...
Expected: 골든 핀이 **깨진다**(`test_roster.py` 등). **정상이다** — 전 유닛의 딜이 오른다. 깨진 테스트 목록을 기록하고 Task 6에서 재기준화한다. 이 태스크에서는 고치지 않는다.
...
- [ ] **Step 3: 깨진 골든 핀을 재기준화한다**

```bash
cd backend && python3 -m pytest -q 2>&1 | tail -30
```

깨진 것이 있으면 각각을 열어 **왜 그 수치가 바뀌었는지 한 줄로 설명할 수 있을 때만** 갱신한다. 설명이 안 되면 버그다.
