...
- **저장값은 엔진 단위.** 환산은 스킬이 판독 시점에 한다. 앱은 화면을 모른다.
  (환산식은 2026-08-08 늦게 정정됐다 — 해상도 기반 일반형은 철회, 비율만
  쓴다. `docs/measurements/accuracy-circle-and-core-px.md` §해석.)
...
- 환산: **`코어_엔진 = 코어_px × (조준원_엔진 / 조준원_px)`** (비율). 해상도
  기반 일반형은 2026-08-08 늦게 철회됐다 — 게임이 창모드라 실제 렌더 폭이
  녹화본과 달랐다. 근거: `docs/measurements/accuracy-circle-and-core-px.md` §해석.
