...
- [ ] **Step 1: 확정 시맨틱 (Fienn 2026-07-19):** Straight Shot의 "2 allies with the highest final ATK"는 **maxwell 자신을 포함**한다. `highest_atk_buff_rule`/`context.top_atk_slugs`는 캐스터를 제외하므로 그대로 못 쓴다 — `_helpers.highest_atk_buff_rule`의 action을 본뜬 커스텀 액션으로, 자신 포함 랭킹(`context.top_atk_slugs`의 구현을 참고해 caster 제외 없이 final ATK 상위 2인을 뽑는 로컬 함수)을 만들어 `slugs:` 스코프로 적용한다. docstring에 Fienn 판정 명시.
...
- [ ] **Step 1: 확정 시맨틱 (Fienn 2026-07-19):** 변형 노멀 틱은 상시 맥스스택 가정으로 **true 고정**(red-hood Glaring 정상상태 선례 승인), 그리고 **틱마다 +11.9% true 추가 히트**가 하나씩 더 붙는다. 구현: 세그먼트 프로필(`damage_type: "true"`)이 22.2% 틱을 방출하고, 같은 93틱 케이던스의 `scheduled_nukes` 스펙(`build_buster_scheduled_nukes(values)` — percent 11.9, damage_type "true")을 `_SCHEDULED_NUKE_BUILDERS["laplace-signature"]`로 등록해 추가 히트를 방출한다(두 경로 모두 `context.burst_times` 앵커라 케이던스 일치).
