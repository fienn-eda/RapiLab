...
- **색: 새 토큰 0개.** `--surface`(바닥) · `--rule`(내부 구획선) ·
  ...
  채움). **`--accent`는 포커스 링 말고는 이 화면에 나오지 않는다** — 팔레트가
  빨강에 허락한 자리가 「활성 탭 · 선택된 칩 · 포커스 링 · 오류」이고, 그중 이
  화면이 쓰는 것은 포커스 링뿐이다. 선택 표시를 빨강으로 하지 않는 이유는
  아래 C절에 있다.
...
## 개정 (2026-08-20, Fienn) — 타임라인이 가이드 문장을 대체한다

E절의 「전투 타임라인 + 자유 문장 textarea」를 **세로 이벤트 타임라인 하나**로
바꾼다. 앱에 띄운 것을 보고 Fienn이 정한 결과다(잠정 딱지를 뗀다).

### 왜 세로인가

Fienn이 준 참고 이미지 셋은 전부 가로축이고, 셋 다 라벨이 겹치지 않게 위아래로
교차 배치하거나 줄기 길이를 엇갈린다 — **가로축의 라벨 자리가 부족하다는 증거가
디자인 안에 들어 있다.** 이 카드에 들어갈 설명은 「탄막 사격 — 엄폐로 넘긴다」
같은 한글 문장이고, 놓일 곳은 세로로 긴 우측 컬럼이다. 세로로 두면 이벤트마다
한 줄을 통째로 써서 길이 제약이 사라지고, 개수가 늘어도 겹치지 않는다.
카운트다운이 위에서 아래로 줄어드는 것도 방향이 같다.

### 잔여시간 표기 — 표시만 바꾸고 데이터는 안 바꾼다

게임 UI가 `3:00 → 0:00`으로 남은 시간을 세므로 타임라인도 같은 형식을 쓴다
(Fienn, 2026-08-20). **저장·입력의 진실은 경과 초다** — `part_destruction_times`와
엔진이 쓰는 단위가 그것이고, 경과로 저장해야 전투 시간을 바꿔도 이벤트가 전투
시작에 그대로 붙어 있는다. 화면에 그릴 때만 `fight_duration - at`을 `mm:ss`로
환산한다. 환산은 `lib/fightClock.ts` 한 곳에만 둔다.

사람이 입력할 때는 잔여시간으로 친다 — 게임에서 눈으로 보는 값이 그쪽이다.
파싱도 같은 파일이 한다.

### 두 종류의 점을 섞되 구분한다

- **파괴 시각** — 보스 설정에서 자동으로 온다. 라벨은 「부위파괴」 고정이고
  **여기서 고칠 수 없다.** 엔진이 읽는 입력이라 두 자리에서 편집하게 하면 진실이
  둘이 된다(2026-08-07 「(회차, 보스)가 키다」와 같은 이유).
- **패턴** — Fienn이 시각과 설명을 직접 적는다. 엔진은 이 값을 읽지 않는다.

둘은 마커 모양으로 갈린다(자동 ◆ / 직접 ◯).

### 자료형

```ts
export interface GuideEvent {
  /** 목록 안에서만 쓰는 식별자. 정렬해도 편집 중인 행이 튀지 않게 한다. */
  id: string
...
`at: null`을 두는 이유: Fienn이 처음 예로 든 가이드 문장 중
「잡몹이 계속 나오므로 홀드 파이어 택틱은 못 쓴다」는 특정 시각에 매이지 않는다.
입력 수단을 둘로 늘리지 않으면서(자유 textarea를 되살리지 않으면서) 담으려면
목록 안에 시각 없는 항목이 있어야 한다. 목록 맨 아래 「상시」로 서고,
**다른 이벤트와 똑같이 편집된다**(Fienn 지정, 2026-08-20).

`hooks/useBossGuides`는 문자열 대신 `GuideEvent[]`를 담는다. 저장돼 있던 값이
문자열이면 `[{ at: null, text: <그 문자열> }]`로 읽어 들인다 — 개정 전에 쓴 글을
버리지 않는다.

### 편집은 타임라인 그 자리에서

읽기용 타임라인과 편집용 목록을 따로 두지 않는다. 행이 곧 입력이다.

```
 3:00 ┬ 전투 시작                              ← 축 끝, 읽기 전용
 2:59 ◆ 부위파괴                               ← 자동, 읽기 전용
[2:40]◯ [탄막 사격 — 엄폐로 넘긴다        ] ×  ← 직접 입력
 1:59 ◆ 부위파괴
 0:54 ◆ 부위파괴
 0:00 ┴ 종료
      + 이벤트 추가
[    ]◯ [잡몹이 계속 나와 홀드 파이어 못 씀] ×  ← 시각이 비면 「상시」
```

정렬은 `at` 오름차순(= 잔여시간 내림차순), `at === null`이 맨 뒤. 시각 칸은
`m:ss`/`mm:ss`가 **온전히** 파싱될 때만 `at`을 갱신한다 — 「2」나 「2:4」에서
행이 튀어 오르지 않게 하는 것이 이 규칙의 목적이다.

### 이 개정이 안 건드리는 것

뱃지 줄 · 토글 칩 · 좌우 배치 · 기본 접힘은 그대로다. 바뀌는 것은 가이드 카드
안쪽뿐이다.
