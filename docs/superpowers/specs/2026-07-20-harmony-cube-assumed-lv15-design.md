...
- **파급 실측 (2026-07-20, 트렁크 `84138af` 병합 상태 1004 passed 기준): 이
  수정으로 깨지는 테스트는 0개다.** 안전해서가 아니라 **커버리지가 없어서**다.
  변이 검증 — `other_elemental_bonus`를 아예 무시하도록 바꿔도 1004개 중 단
  1개만 실패한다(`test_skill_rules_maiden_ice_rose.py::
  test_blessings_fill_buffs_step_up_at_mp_fill_times_vs_water_boss`), 그마저
  **우위 상황** 테스트다. **비우위 상황의 우월코드 동작을 검증하는 테스트는
  현재 존재하지 않으며, 그것이 이 버그가 살아남은 이유다.** 아래 게이팅 회귀
  테스트는 형식적 항목이 아니라 이 구멍을 막는 것이 목적이다.
...
## 알려진 한계 — 수동 입력 경로의 평탄 스탯

스탯이 앱에 들어오는 경로는 둘이고, 큐브 가정이 두 경로에 **똑같이 적용되지
않는다.**

**동기화 경로**는 ATK를 항목별로 계산한다 —
`기초값(레벨, 클래스) + 돌파 + 코어 + 평탄합(장비 + 큐브 + 소장품 + 호감도 +
연구)`. 큐브 항이 우리 손 안에 있으므로 `harmony_cube_lv`를 15로 바꿔 끼우면
가정이 그대로 반영된다.

**수동 입력 경로**는 사용자가 ShiftyPad 화면에서 읽어 타이핑한 **합산된 숫자
하나**를 받는다. 그 안에는 입력 시점의 착용 큐브(또는 미착용)가 이미 녹아
있는데, 항목별로 분해할 수 없으므로 큐브 항만 Lv.15짜리로 교체할 방법이 없다.

결과적으로 수동 경로는 **"효과는 Lv.15 가정, 평탄 스탯은 스냅샷 그대로"** 라는
반쪽 상태가 된다. 큐브 효과(재장전 29.69% / 우월코드 19.09%)는
`assumed_cube_effects`에서 나오므로 정상 적용되지만, 입력 당시 미착용이었던
유닛은 평탄 스탯이 ATK 2,780 · HP 83,400만큼 부족하다. 실측 로스터 159명의
raid400 ATK 중앙값 102,205 기준 **약 2.7% 과소**다. 입력 당시 큐브를 끼고
있었다면 오차는 0이다.

**고치지 않는 이유.** 고치려면 입력 시점의 착용 큐브를 알아야 하는데, 이번
설계에서 그 입력란을 제거한다. 남는 선택지는 "큐브를 뺀 ATK를 입력하세요"라고
사용자에게 산수를 시키는 것뿐이라 더 나쁘다.

이 불일치를 근본적으로 없애는 길은 수동 입력 경로 자체를 없애고 동기화 전용
읽기 전용 로스터로 가는 것이다. 그러나 그것은 `NikkeCard` · 각 Field 컴포넌트 ·
`nikkeDraft` · `App.tsx` 배선과 그 테스트에 걸친 800~1,200 LOC 규모의 재작업이고
(동기화도 같은 draft 모델로 임포트되므로 삭제가 아니라 **읽기 전용 UI 재설계**다),
미지원 유닛 수동 추가와 가정 실험 기능을 포기하는 **제품 방향 결정**이다.
2.7% 오차를 근거로 결정할 사안이 아니므로 **별도 스펙으로 분리한다**
(Fienn 확인, 2026-07-20).
