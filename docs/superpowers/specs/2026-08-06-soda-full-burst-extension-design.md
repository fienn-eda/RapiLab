...
붙어 있다. 확장 쪽은 누적이 **실측으로 확인**됐다(Fienn의 15초 = 10 + 2 + 3). 넉 쪽에
같은 문구를 그대로 적용하면 Ext II 상태의 한 발은 **52.04 + 85.02 = 137.06%**가 되고,
아니라면 85.02%다.

**설계의 기본값은 누적(137.06%)이다** — 같은 스킬의 같은 문구를 두 불릿에서 다르게
읽을 근거가 없고, 확장 쪽은 그 읽기가 실측과 맞는다. 다만 넉 자체의 실측은 없다.
**Fienn의 확인이 필요한 유일한 항목**이고, 답이 바뀌면 상수 한 줄이 아니라 넉의 크기가
1.6배 달라진다.
