...
| 아니스:스타 · 아르카나 · 크라운 · 신데렐라 · 네온:비전아이 | 8.42B | 6.73B | **+25.1%** |
...
- **경계 판정** — 게임이 FB 마지막 틱에서 조건을 보는지 한 프레임 뒤에 보는지. 5초 창
  ...
  갈라진다.
- **닫힌 게이트의 여유가 엔진 상수다.** 「표준 10초 창에서는 게이트가 닫힌다」는 이
  설계의 중심 주장인데, 엔진에서 그것을 지키는 폭은 버스트 스테이지 2와 3 사이의
  간격뿐이다 — 수동 모드는 티어 갭 **0.1초**, 자동 모드는 `FULL_BURST_OPEN_DELAY`
  **1e-6**이고, 그 조각을 「닫힘」으로 만드는 것은 `own_burst_status_active`의 엄격한
  `>` 하나다. 실제 게임에서는 두 버스트 시전이 차례로 끝나야 하므로 같은 간격이 눈에
  보이는 초 단위이고, 게이트는 엔진이 보여줄 수 있는 것보다 훨씬 크게 닫혀 있다 —
  얼마나인지는 아무도 재지 않았다. 열린 쪽(이사벨 5초 창, 여유 4.9초)은 이렇게 얇지
  않다.
