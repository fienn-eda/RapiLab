...
| `grave` | 무한탄약·탄약제거 둘 다 "not modeled" | 짝이 통째로 빠짐 + **방열 지속시간 결함**(아래) |
...
4. **`Reload Ratio ▼50%`는 재장전 속도가 아니다.** 인게임에서 **탄창을 절반씩 두 번**
   장전하는 것으로 작동하며, 사실상 재장전 시간이 두 배가 된다.
5. **방열(Heat Emission) 제거 조건은 둘이다** (인게임 툴팁 「[방열 제거 조건]」):
   ① **재장전을 최대 장탄까지 완료** 시 ② 버스트 스킬 사용 시.

   **①이 ②보다 훨씬 먼저 발동한다.** 0탄약에서 Reload Ratio ▼50%로 장전하면 첫
   장전은 절반까지만 차서 방열이 안 풀리고, **두 번째 장전이 최대 장탄에 닿는 순간
   방열이 스스로 제거된다.** 그것이 판정 4의 「절반씩 두 번」의 정체다. 따라서 방열은
   **재장전 두 번 길이만 산다.**
...
Heat Emission(skills[0]) 원문의 구조가 세 블록이고, 그 구조가 설계를 정한다:

```
■ Prediction 종료 시 발동. 자신.
   Removes {v1}% of bullets.
   Heat Emission: Reload Ratio ▼ {v2}%. 특정 조건에서 방열 제거.
■ 방열 상태일 때만 발동. 자신.       → HP 회복 (생존기, 계속 보류)
■ 방열 상태일 때만 발동. 아군 전체.   → 버스트 게이지 충전속도 ▲(미소비),
                                      Pierce Damage ▲{v5} continuously
```

매니페스트 `source`가 `dotgg`이므로 이 슬롯 번호는 **네이티브 키**이고 그대로 쓸 수 있다
(토큰 순서를 세야 하는 것은 레이·아스카 쪽이다).

**항 1 — 무한탄약.** 세그먼트로는 안 된다. `_segment_shot_records`가 "**Segments never
reload**"이면서 동시에 「명시적 `rate_of_fire` 프로필은 어떤 cadence 버프도 받지 않는다」
는 계약을 갖기 때문에, 세그먼트로 덮으면 정작 그녀 버프가 다 켜져 있는 10초 창에서
**공속이 얼어붙는다.** 대신 **큰 `max_ammo_percent` self 버프**를 창 한정으로 건다 —
기존 스탯이고, 라이브 버프가 전부 유지된다.

**항 2 — 탄약 제거 + 강제 재장전.** 무발사 세그먼트. **길이는 재장전 ×2**다 (판정 4·5:
Reload Ratio ▼50%가 0탄약에서 절반씩 두 번 장전으로 만드는 그 길이).

**항 3 — 방열 지속시간 정정.** 판정 5의 조건 ①이 이 항을 **엔진 확장에서 인코딩 수정으로
바꾼다.** Reload Ratio는 방열이 살아 있는 동안만 걸리고 방열은 두 번째 장전에서 스스로
꺼지므로, **지속적인 재장전 디버프가 아니다** — 따라서 「재장전 시간 배수」 스탯은
필요 없다. 대신 **현재 인코딩이 틀렸다**:

`build_grave_rules`의 `apply_heat_emission`이 스쿼드 Pierce Damage를 `duration=None`
(무기한)으로 걸고 `remove_heat_emission_on_reburst`가 다음 자기 버스트에서 잘라낸다 —
즉 **40초 주기 중 ~30초**를 산다. 조건 ①이면 **재장전 두 번(≈2.3초)** 이다. 스쿼드
버프이므로 **그레이브가 든 모든 덱이 부풀어 있다.**

정정: 무기한 + 리버스트 절단 대신 **유한 지속 = 항 2의 세그먼트 길이**. 두 곳이 같은
숫자를 쓰도록 헬퍼 하나에서 유도한다. `remove_heat_emission_on_reburst`(조건 ②)는
안전망으로 남긴다 — 버스트 쿨 40초라 실제로는 물지 않는다.

**이 정정은 앞선 Fienn 판정을 뒤집는 것이 아니라 갱신하는 것이다.** 독스트링의 「removed
exactly when Grave uses her burst skill AGAIN」은 조건 ②만 보이던 시점의 판정이고,
이번 툴팁이 조건 ①을 더한다.

Prediction 창 자체는 이미 모델돼 있다(`PREDICTION_DURATION = 10.0`, 자기 버스트가 부여,
...
- **제거된 탄약이 아군 소모탄 카운터에 잡히는지는 모른다.** 리틀 머메이드의 Bubble
  ...
  남긴다.
- **그레이브의 무한탄약을 「재장전이 안 일어날 만큼 큰 `max_ammo_percent`」로 근사한다.**
  진짜 무한이 아니라 창 길이 안에서 구별 불가능한 값이다. 그리고 `capacity`는 **탄창
  시작 시점에** 샘플링되므로, Prediction이 시작될 때 진행 중이던 탄창에는 안 걸린다 —
  그 탄창을 비우고 한 번 재장전한 뒤부터 큰 탄창이 된다. 창 안에서 재장전 하나를 더
  치르는 셈이라 **과소 방향**이고, 그것이 정직한 쪽이다.
- **방열 지속시간과 강제 재장전 길이는 파일 재장전 값에서 유도하며 라이브 재장전 속도
  버프를 읽지 않는다.** 밀크의 `_forced_reload_seconds`가 이미 하는 것과 같은 근사다
  (그쪽도 자기 무기값 + 고정 절만 본다). 덱에 재장전 버퍼가 있으면 실제로는 더 짧다.
...
- 그레이브: 무한탄약 창 안에서 **재장전이 없는지**, 창 끝에서 재장전이 도는지, 그리고
  그 재장전 길이가 **평소의 2배**인지
- 그레이브: 스쿼드 Pierce Damage가 **재장전 두 번 뒤에 꺼지는지** — 이 가드는 지금
  코드에서 반드시 빨개져야 한다(현재는 ~30초 산다). **빨개지는 것을 먼저 확인한 뒤**
  구현한다
...
4. **B — 그레이브.** 세 항: 무한탄약 · 강제 재장전(×2 길이) · **방열 지속시간 정정**.
   가장 크고, 유일하게 **기존 인코딩을 고치는** 조각이다. 스쿼드 버프가 짧아지므로
   그레이브가 든 덱의 점수가 내려간다 — 스윕에서 따로 확인한다.
