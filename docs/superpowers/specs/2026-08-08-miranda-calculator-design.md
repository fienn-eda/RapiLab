...
실제로 ATK 라인을 늘리면 **다른 오버로드 라인을 잃는다** — 크확·크댐을 내주고 사는
공격력이다. 이 화면은 그 손실을 모르고, 딜이 오른다고 말하지도 않는다. 답하는
것은 「누가 받는가」 하나다. 화면에 한 줄로 적는다.
...
§5.6의 한 줄은 목록 아래에 고정으로 붙인다:
`오버로드 라인을 공격력으로 바꾸면 크확·크댐을 잃어요 — 이 계산은 그 손실을 보지 않아요.`
