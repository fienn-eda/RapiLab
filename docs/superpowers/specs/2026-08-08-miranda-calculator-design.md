...
**기본이 off이면 `target_grants`에는 아무것도 쌓이지 않는다** -
`top_atk_slugs`가 리스트에 남기는 조건은 `self.target_grants is not None`이다.
다만 `highest_atk_buff_rule`과 `round_buff_rule`(`_resolve_scope`를 통해)은
플래그와 무관하게 매 호출마다 `grant_stats` 튜플 하나를 만든다 -
`round_buff_rule`의 액션은 샷마다 발동할 수 있다. 딜에는 영향이 없고 비용은
무시할 만큼 작다.
...
def miranda_target_report(deck_specs, boss, spec_index, alternatives=None) -> dict
```

`spec_index`는 로스터의 **모든** 로드 가능한 스펙이어야 한다(`{slug: NikkeSpec}`,
`load_roster`의 출력) - 좌석 순서를 정하는 내부 헬퍼 `order_deck`이 엔진이 고른
MODE_VARIANTS 좌석을 `deck_specs`에 없는 변형으로 확정할 수 있어서, 다섯만 담은
색인으로는 되짚을 수 없다.
...
caster로 이미 걸러낸 미란다 자신의 기록 안에서는 top-N 대상형 룰이 파워업!과
웨이크업!3 둘뿐이므로 이 구분에 겹침이 없다(다른 유닛의 top-N 대상형 룰까지
합치면 둘보다 많다 — `highest_atk_buff_rule`이 맥스웰·레오나·나가의 불릿도
만든다, §3). 미란다의 다른
...
`(b의 최종공격력 − a의 최종공격력) / a의 표시공격력`은 **필요치를 어림할 뿐이고,
실측(§5.3)은 이 닫힌형이 과소평가함을 보였다 — 그것도 큰 폭으로.** a의 오버로드가
오르면 파워업!(B1 시전) 판정도 같이 움직여, a가 파워업! 상위 N에 새로 들어가면 그
자리에서 +40.4%를 얻고 밀려난 쪽은 그만큼 잃는다. 계단이 정확히 이 질문이 흥미로운
지점에 서 있다.
...
실측(Task 7, 2026-08-08)은 필요한 오버로드 공격력이 **naive 격차(최종공격력
차이를 유닛의 표시공격력으로 나눈 값)의 대략 2.5~3배**임을 보였다. 이유는 위
경계 그대로다 — 기준이 「매 사이클 받는가」이고 사이클마다 승자가 바뀌므로 가장
빡센 사이클이 값을 정한다. 구체적으로, 한 유닛의 표시 공격력이 나머지보다 20%
높은 덱에서 교차점은 **62~76%**로 나왔다 — 58.52% 오버로드 상한보다 위, 즉
**도달 불가능**이다. 화면에서는 `오버로드 공격력을 상한(58.52%)까지 올려도
매 사이클 받지는 못해요`(§7.4)가 드문 답이 아니라 흔한 답이 된다.
...
| 조건 | 메시지 |
|---|---|
| `len(units) != 5` | `덱은 5명이어야 해요.` |
| 덱 안에 슬러그 중복 | `이 덱에 같은 니케가 겹쳐 들어갔어요: {dups}` |
| 미란다 부재 | `덱에 미란다가 없어요. 미란다를 넣어야 계산할 수 있어요.` — 화면에서는 좌석이 고정이라 닿지 않는 가드다(§7.2). API는 화면 밖에서도 불릴 수 있으므로 남긴다 |
| 미지원/로스터에 없는 슬러그 | `엔진이 쓸 수 없는 슬러그예요: {slug}` |
| `InfeasibleDeck` | `이 다섯으로는 성립하는 버스트 순서가 없어요. 버스트 1·2·3단계 인원 수가 1·1·3, 1·2·2, 2·1·2 중 하나여야 해요.` (`_evaluate_decks_sync`의 문구 재사용) |
...
뱃지의 `n/T`가 「적어도 한 사이클」을 답하는 것과 달리, 이 문구가 재는 경계는
「매 사이클」이다(§5.3) - 부분 수령 행에서는 뱃지와 문구가 한 줄에 같이 뜨므로,
문구가 자기 기준을 스스로 말해야 둘이 부딪히지 않는다.

| `kind` / 값 | 문구 |
|---|---|
| `gain`, 값 있음 | `오버로드 공격력 8.00% → 11.47%면 매 사이클 받아요 (+3.47%p)` |
| `gain`, `null` | `오버로드 공격력을 상한(58.52%)까지 올려도 매 사이클 받지는 못해요` |
| `keep`, 값 > 0 | `오버로드 공격력이 5.90% 밑으로 내려가면 매 사이클은 못 받아요 (지금 8.00%, 여유 2.10%p)` |
| `keep`, 값 = 0 | `오버로드 공격력이 없어도 매 사이클 유지돼요` |
...
- `test_api_miranda_targets.py`(신규) — 422 다섯 가지와 정상 응답 한 건.
