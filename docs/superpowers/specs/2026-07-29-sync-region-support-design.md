...
두 계정 × 5 리전 전수 조회(2026-07-29, Fienn):
...
- **`intl_open_id`는 전역 유일하고, 리전마다 별개의 게임 데이터를 갖는다.** 한
  사람이 여러 서버에 캐릭터를 만들 수 있으므로 **여러 리전이 `code 0`인 것이
  정상이다.** 남의 계정을 물어올 위험은 없다 — 어느 area로 물어도 같은 사람이다.
- 대신 **내 여러 리전 계정 중 엉뚱한 것을 고를 위험**이 있다. A를 자동 판정하면
  186기 JP 대신 10기 KR을 집을 수 있다.
- `1302125 "get info list err"` = 그 리전에 이 계정이 없다.
- **`1303002 "proxy.GetUserShiftyspadPrivacy error"`는 간헐적이다.** 같은
  (B, 81) 조합이 한 번은 1303002, 다음엔 `code 0, 0기`였다. 리전 하나의 일시적
  오류가 동기화 전체를 죽여선 안 된다.

## 결정 (Fienn 확정)

1. **탐색 후, 모호할 때만 묻는다.** 북마크릿이 5개 리전을 조회해 니케가 있는
   리전만 고른다. 하나면 그대로 진행하고(대부분의 유저), 둘 이상이면 앱이
   `Japan 186기 / Korea 10기`를 보여주고 고르게 한다. 짐작하지 않고, 유저가
   리전 번호를 알 필요도 없다.
2. **프로필 실별자를 `(open_id, area)` 쌍으로 바꾼다.** 지금은 `open_id`만
   키라서, 같은 open_id의 다른 리전 로스터를 동기화하면 조용히 덮어쓴다.
...
```ts
// blablalink의 GetRegionList가 주는 다섯 리전. 하드코딩인 이유: 목록이 바뀌는
// 일은 새 서버가 열릴 때뿐이고, 그때는 이 표에 한 줄 더하는 것이 라이브 조회를
// 유지하는 것보다 싸다.
...
1. 5개 리전에 `GetUserCharacters`. **`code !== 0`이거나 캐릭터가 0기면 그 리전은
...
```js
{ open_id, regions: [ { area, nickname, owned, character_details, recycle_room_researches } ] }
```

**구 payload도 계속 받는다.** 기존에 설치된 북마크릿은 `{open_id, nickname,
owned, character_details, recycle_room_researches}`를 보내는데, 그것은 area 81로
조회한 데이터이므로 `regions: [{area: 81, ...}]`으로 승격해 받으면 정확하다.
이렇게 하면 이번 변경이 즉시 재설치를 강제하지 않는다.

북마크릿 헤더의 "얇게 유지" 원칙은 지킨다 — 추가되는 것은 리전 루프와 후보
필터뿐이고, 판단(어느 리전을 쓸지)은 앱이 한다.

### 리전 선택 UI

`SyncRosterPanel`이 payload를 받았을 때:

- `regions.length === 1` → 지금과 동일하게 바로 `assembleRoster` → `onImport`.
- `regions.length >= 2` → 리전 목록을 `Japan (186기)` 형태로 보여주고 선택을
  기다린다. 고른 리전 하나만 `assembleRoster`에 넘긴다 — 나머지는 버린다(백엔드
  왕복을 낭비하지 않는다).

여러 리전을 한 번에 프로필로 만들지 않는다. 한 번의 동기화 = 한 리전.
다른 리전도 원하면 다시 동기화하면 되고, `(open_id, area)` 키라서 둘이 공존한다.
...
- 드롭다운 라벨에 리전을 붙인다: `FIENN (Japan)`. 같은 사람의 두 리전 계정이
  구분돼야 하므로 닉네임만으로는 부족하다.
...
- 여러 리전을 한 번에 여러 프로필로 만들기 — 한 번에 하나면 된다.
...
- `buildBookmarklet`: 생성된 소스가 5개 area를 모두 담고, `code !== 0`인 리전을
  건너뛰며, 후보 0개일 때 사람이 읽을 문구를 내는지.
- payload 수신: 신 shape(`regions`)와 구 shape(area 81 승격) 양쪽.
- 리전 1개면 선택 UI 없이 바로 임포트, 2개 이상이면 선택 전에는 임포트하지 않음.
- 프로필 저장소: `(openId, area)` 키로 두 리전이 공존하고 서로 덮어쓰지 않음.
- 마이그레이션: 구 스키마(`profiles[openId]`, `activeOpenId`)가 `:81` 키로
  옮겨지고 `area: 81`이 채워짐. 이미 새 스키마면 건드리지 않음(멱등).
- 드롭다운이 같은 닉네임의 두 리전을 구분해 보여줌.
