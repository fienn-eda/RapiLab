...
- 디렉토리 RAW 페이로드의 `shot_id`에서 무기 필드 추출 — `ammo` / `weapon_type` /
  `attack_type`은 들어 있으나 엔진이 필요로 하는 `damage` / `reloadTime` /
  `chargeTime` / `chargeDamage`가 없어 수동 스텁을 없애지 못한다. 별건으로 남긴다.
