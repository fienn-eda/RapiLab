...
> **2026-08-14 정정:** 이 드롭 줄은 구현되지 않았다 — Task 2 구현 중 도달 불가능한
> 죽은 코드로 판명됐다: 두 워크 모두 탄창을 **만탄으로** 열고 환급은 **상한 캡**에
> 걸리므로, 지나간 환급을 만탄에 적용해도 `min(capacity, capacity + n) == capacity`로
> 그대로 만탄이다. 판정 2("재장전 중 환급은 버려진다")는 여전히 참이지만, 그것을
> 강제하는 것은 상한 캡이지 시각 비교가 아니다. 아래 `:156-159`와 테스트 표
> `:260`도 같다.
...
> **2026-08-14 정정:** 실제 구현에는 `time < magazine_start` 드롭이 없다 — 위
> 「공유 탄창 워크에는 규칙이 하나 더 필요하다」 단락의 정정과 같은 이유(상한 캡이
> 이미 같은 결과를 강제하므로 도달 불가능한 죽은 코드였다).
...
**2026-08-14 정정:** 위 첫 행의 "깨뜨리는 방법"은 구현되지 않은 `time < magazine_start`
드롭을 가리킨다 — 실제로는 이 행이 말하는 결과(다음 탄창이 평범한 만탄)를 상한 캡
하나가 강제하므로, "드롭을 뺀다"는 조작 자체가 존재하지 않는 코드를 상정하고 있어
더 이상 반증 가능하지 않다. 실제 테스트는
`test_a_stale_refill_finds_no_room_in_a_full_magazine`(`backend/tests/test_ammo_refund.py`)로,
용량 캡만으로 통과한다.
