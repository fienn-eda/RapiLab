...
- 마지막 갱신: 2026-07-19 (**Diesel: Winter Sweets 신규 인코딩 — Intro/Highlight 2슬러그,
  유닛별 버스트 스케줄 정책(`burst_delay`) 엔진 확장 소비**). 2026-07-17에 미뤄뒀던
  "Diesel은 짝수 사이클 버스트가 강한데 엔진이 '한 사이클 거르기'를 표현 못 함" 갭을
  드디어 해소 — `burst_cycle` 멤버가 `{"skip_cycles": N}` / `{"not_before": T}` /
  `{"min_interval": S}` 를 가질 수 있게 됐다. **핵심 판정(Fienn 2026-07-19)**: Intro/Highlight는
  첫 풀버스트에서 확정되어 **전투 종료까지 고정**이고 중간 변경 불가. Highlight 지속딜 버프가
  235.03% vs Intro 60.19%로 약 4배라, 1사이클을 거르는 대가가 충분히 남는다.
  Bready와 달리 **상태가 엔진이 실제 시뮬레이션하는 버스트 스케줄로 갈리므로**, 정적
  슬러그만 만들면 1사이클에 버스트하면서 Highlight 버프까지 챙기는 과대평가가 된다 —
  그래서 Highlight 슬러그에 진짜 `skip_cycles: 1`을 걸었다. 같은 확장으로 **Elegg의
  버스트도 고스트 캡까지 보류**(78.2/136.2초 2회, 둘 다 13스택 = Fienn 플레이 서술과 일치;
  이전엔 5회 중 4회가 6히트 분기였다). **테이블 행 72 / `ENCODED_SLUGS` 75개.**
  이전 갱신: 2026-07-19 (**Elegg: Boom and Shock · Mihara: Bonding Chain 신규 인코딩 —
  gap #2 Pattern B 소진**). 착수 전 검증에서 **둘 다 Pattern B가 아니었음이 확인됨**:
  스킬 텍스트에 시간감쇠 게이지도 임계치 변신도 없고, 결정론적 fill을 가진 평범한
  자원(Pattern A) 유닛이었다 — red-hood와 같은 종류의 낡은 분류. 엔진 확장 4건 소비:
  자원 reset의 `value_fn`(소비량이 pre-reset 카운트를 읽음) · `dynamic_hit_count_nukes`의
  `hit_count_fn`(카운트→히트수 분기) · **다중 소스 fill**(`fill`이 (fill spec, amount) 리스트를
  받아 한 자원을 서로 다른 비율/증가량으로 급유) · `scheduled_nukes`의 `resource_gate`
  (전투 전체 DoT를 스택수로 스케일). Elegg의 "6초창 스쿼드 누적 100히트"는 그녀가 MG(초당 60발)라
  항상 충족(Fienn 판정)이라 `("periodic", 6)`으로 근사. Mihara는 Fienn 실측(버스트 시 사슬감기
  20스택)으로 방출 타이밍 해석을 확정 — 툴팁의 3개 방출 트리거 중 레이드에서 유효한 건
  배틀스타트와 "풀버스트 종료 후" 둘뿐이고, 신규 fill kind는 창 끝 직후에 놓아
  동시각 Bonding Pain reset에 먹히지 않게 했다. 테이블 행 70 / `ENCODED_SLUGS` 71개, 매니페스트·API 로더블 71/71.
  더 이전 갱신: 2026-07-19 (**무기변형 v1 착지 — red-hood 세그먼트 마이그레이션
...
| Elegg: Boom and Shock | `elegg-boom-and-shock` | Attacker | MG | Water | ✅ | (신규 2026-07-19) **Pattern B 오분류 정정 — 감쇠 게이지/변신 없음, 결정론적 fill의 자원 유닛.** Ghosts 자원(캡13, 6초마다 +1)·유령수 임계 버프 2종(≥1: Water아군 flat ATK=자ATK 16.2% / ≥4: Elemental Advantage Attack Damage +35%, 둘 다 상시·`element:Water`라 자신 포함)·Ghostbuster 버스트 자ATK+40%/10초 + **캡에서 포획 시 1100% 오버플로 넉**(`scheduled_nukes`가 각 포획틱 직전 카운트를 읽어 캡이면 발동)·13 Ghosts 버스트 **분기**(캡13이면 800%×13연타·유령−9, 미만이면 800%×6연타·유령−6, 최소 1 유지 — 신규 `hit_count_fn`+reset `value_fn`) 모델됨. E2E(180초 5인덱): 버스트 t=1.00/57.83(6히트) → 114.66/171.49(**13히트**)로 분기 양쪽 실발동, **(2026-07-19 갱신) 이후 `burst_delay`로 실제 운용에 맞춤** — `not_before`=캡13×6초=78초,
`min_interval`=캡소비9×6초=54초(둘 다 그녀 자신의 fill 값에서 유도). 재충전 54초가 쿨 40초보다 길어
실효 케이던스가 쿨이 아니라 재충전이다. 결과 78.2/136.2초 **2회, 둘 다 13히트 분기** — Fienn의
'전투 중 2번, 13스택 쌓인 후' 서술과 일치(이전 자연 로테이션은 5회 중 4회가 6히트 분기였다). 보류: '6초창 스쿼드 누적 100히트' 요구치는 그녀가 MG(초당 60발)라 항상 충족으로 근사(Fienn 판정) — SR/RL 위주 덱에선 이론상 미달 가능·Possession 타겟팅(보스 1기)·`other_elemental_bonus`는 보스 원소 게이팅 없음(guillotine 선례 관례) |
