...
- Consequences: Emitting a type-gated buff is now live only when the deck also produces an instance of that type (the buff is a multiplier needing something to multiply). Mint's projectile-explosion buff now reaches RL allies; Rapi's burst nuke benefits from it. Provisional, pending Fienn confirmation: `true` damage keeps the nikke.gg-formula DEF subtraction (no bypass); `attack_damage_up` stays global across all types. Deferred: shield_damage_up and the major-modifier terms remain inert.
