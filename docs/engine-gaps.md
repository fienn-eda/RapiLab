...
- **무엇:** 큐브의 부가효과(재장전 속도 / 우월 코드 대미지)가 시뮬에 전혀 반영되지 않는다.
  다른 갭과 달리 특정 유닛이 막히는 게 아니라 **로스터 전체가 영향받는다**.
- **왜 막힘:** 배선은 있는데 값을 공급하는 쪽이 없다. `cube_effects.cube_to_effects`는
  `reload_speed_percent`/`superior_code_damage_percent`를 받아 Effect를 만들지만,
  `roster.py:62`가 `spec.cube.get(...)`으로 넘기는 그 키들이 `PveCube`(`models.py:23`,
  필드는 `name`/`level` 둘뿐)에 존재하지 않는다 → 항상 `None` → 항상 빈 리스트.
  자세한 추적은 `docs/insights.md` 참조.
- **평탄 스탯은 정상:** 동기화 경로의 ATK/HP는 `harmony_cube_lv`를 `cube_atk`/`cube_hp`에
  넣어 계산하므로 착용 큐브가 반영된다(종류 무관·레벨 의존, lv15 = +2,780 ATK/+83,400 HP).
  즉 큐브는 **평탄 스탯만 살아있고 효과는 죽어있는 반쪽 배선** 상태다.
- **데이터 상황:** `tables.json`의 `cube_sample`에 실제 수치가 있다 —
  `harmonycube_skill_group[].description_value_list`(퀵 리로드 HC / 안티 코드 HC)와
  큐브레벨 1~15 → 스킬레벨 사다리(`level1`/`level2`/`level3`). 다만 **큐브 한 종류(id
  1000303)만 샘플링돼 있고**, 값 리스트의 인덱싱이 단순 레벨 사다리가 아니라
  (퀵 리로드가 14.84·22.27·29.69 뒤에 17.5·20·22.5…로 불연속) **인게임 표시값과 대조해
  확정해야 한다**. 지어내면 안 되는 지점.
- **필요한 작업:** 큐브 카탈로그 수집 + 인덱싱 실측 검증 → 정규화 모델 결정 → 배선.
  규모 중. 오버로드 값 테이블을 로제타 역산으로 확정했던 것과 같은 성격의 작업이다.
- **모델 결정이 함께 필요 (Fienn 제안, 2026-07-19):** 수집 시점의 착용 상태를 쓰지 말고
  **전원 lv15 큐브 착용을 가정**하고 UI에 그 가정을 명시하자는 안. 근거는 게임 규칙 —
  한 큐브 종류를 최대 12명까지 동시 착용 가능하고 덱 전투 사이에 재장착할 수 있어
  **실질적으로 25명 전원이 lv15 큐브를 낀 채로 싸운다**(Fienn 확인). 따라서 스냅샷 시점의
  착용/미착용 차이는 실제 전투 양상과 무관한 노이즈다. 미결 쟁점: 가정 큐브를 전역 1종으로
  할지, 유닛별로 둘지, 아니면 **추천 엔진이 유닛마다 최적 큐브를 고르게** 할지(큐브가 자유
  재장착이라면 이는 입력이 아니라 덱빌딩의 결정변수다). 전역 1종 가정은 재장전 이득을 보는
  유닛 쪽으로 랭킹을 치우치게 한다 — 차지형 유닛은 받을 이득을 못 받고 안 받을 이득을 받는다.
