...
**물리는 조건: 발사 공백이 1.16667초보다 짧을 때만.** 그 구간의 크기는 세 번째 판독이
직접 쟀다 — **크라운 + 재장전 큐브 15레벨**에서 공백 63프레임(1.05초), 램프 **81프레임**
(콜드 137). **잔존 40.9%, 즉 이런 덱의 MG는 매 탄창 56프레임(0.933초)을 엔진에게 잘못
청구받는다.**

**선형 감쇠는 이 판독으로 기각됐다.** 공백 63프레임이면 선형은 잔존 10%·램프 123프레임을
예측하는데 관측은 81프레임이다. 열은 초반에 훨씬 느리게 빠진다 — 두 판독을 동시에
만족하는 모양은 「약 52.9프레임 **유지** 후 17.1프레임에 걸쳐 방출」이지만, **관측점 2개에
미지수 2개라 자유도가 0이고 모양이 맞다는 증거가 아니다.**
...
**닫기 전에 아직 재야 할 것 — 세 번째 공백 길이 하나.** 유지/방출 모양은 두 점을 지나는
해가 유일하지 않다. 지금 있는 공백은 63프레임(잔존 41%)과 124프레임(잔존 0%) 둘뿐이라,
**그 사이나 그보다 짧은 공백** 하나가 더 있어야 모양이 검정된다. 재장전 속도를 더 쌓거나
덜 쌓은 조건 하나면 된다.

**같이 재면 좋을 것:** 그 램프 구간의 **잔여 탄환 수**. 지금 잔존 41%는 「램프 = 일정한
낮은 연사」라는 엔진 가정 위에서만 나오는 값이라, 발수가 붙으면 그 가정 없이 직접
읽힌다(첫 판독이 잔여 탄환을 같이 기록해 48발/137프레임을 확정한 것과 같은 이유).
