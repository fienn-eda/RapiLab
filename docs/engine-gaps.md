...
- 마지막 갱신: 2026-07-27 (**gap #17 해소 — 소장품 스킬 효과 배선.** 데이터 테이블
...
- **SMG·RL 무기군은 데이터가 전혀 없다.** `tables.json["collectibles"]`에 항목 자체가
  없고, 리졸버는 미등록 group_id를 조용히 버린다(안전한 기본값 0). 이 두 무기군의
  소장품을 낀 유닛은 그 스킬 효과가 여전히 0으로 계산된다.
- **SR/SG/AR 값은 게임 CDN에서 캡처한 게 아니라 인게임 툴팁을 손으로 유도한 것이다.**
  Task 1에서 라이브 Playwright 캡처(`collect.js --collectibles`)를 시도했으나
  `connectOverCDP: ECONNREFUSED` — 로그인된 blablalink Chrome 세션이 이 환경에
  없어서 실패했고, 브라우저 자체는 CDN에 닿는 것을 확인했다(세션만 없음). 대신
  Ade(SR)·Helm·Zwei·Tove의 실측 최대값에서 사다리 하위 단을 `max/6×[3,4,5,6]`으로
  역산했다 — 실제 게임 데이터가 아니라 **추정치**다.
  MG 무기군만 `collectible_sample`(Flora로 인게임 검증됨)과 완전히 일치하는 실제값이다.
- **R 등급(일반 소장품)의 사다리는 알려져 있지 않다.** 현재 테이블은 전부 SR 기준이고,
  R 등급 소장품을 낀 유닛의 계수는 미지수다.
- **소장품 낀 유닛의 노출 범위는 8기보다 훨씬 넓다.** `-signature` 슬러그로 애장품(SSR)
  보유가 드러나는 유닛은 **8/159**뿐이지만, 에이드처럼 **일반 소장품**을 낀 유닛은
  슬러그가 안 바뀌어 안 보인다 — 이 배선이 실제로 몇 명을 움직일지는 로스터 재동기화
  전까지 알 수 없다.

**캘리브레이션에는 아직 반영 안 됨.** 실제 로스터(`tools/collect-blablalink/
roster-drafts.json`)는 `NikkeDraft[]` 형태라 `collectible_tid` 필드 자체가 없고,
`scripts/roster_fixture.py::_state_from_draft`도 그 필드를 참조하지 않아 pydantic
기본값 0으로 떨어진다 — 즉 `measure_record_calibration.py` 수치는 이 배선 전후로
**완전히 동일**(합계 0.914x)하다. 이건 회귀가 아니라 배선이 전 구간에서 정확히
무해하다는 확인이다. 실제 캘리브레이션 변화는 **Fienn의 로스터 재동기화** 이후에나
관측 가능하다. 상세는 `docs/decisions.md`의 관련 ADR, 캘리브레이션 To-Do는
`docs/roadmap.md`.
