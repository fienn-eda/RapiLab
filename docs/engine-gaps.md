...
- **무엇:** 재장전을 실측대로 고치자 합계가 **1.015x → 1.055x**로 멀어졌다(±15% 이내
  21/25 → 19/25). 지금까지의 균형은 **너무 느린 재장전 위에서** 맞춰져 있었고,
  재장전이 빨라지자 가려져 있던 과대가 드러났다.
- **어디에 몰리나 — 평타 신호:**
  ...
  | rei-ayanami | 0.936 | **1.008** ✓ 개선 |
  | scarlet-black-shadow (차지) | 0.999 | 0.999 불변 |

  **재장전이 빨라지면 늘어나는 것은 평타 발수뿐**이므로 과대 항은 평타 경로에 있다.
  차지 위주 덱(덱4 1.007 · 덱5 1.043)은 거의 안 움직이고, 탄창 무기가 많은
  덱1·2·3이 크게 뛴다는 것이 같은 말이다.
...
- **같은 형태의 선례:** 택티컬 베어 탄환 환급(항목 12-b)도 흑련을 0.981 → 0.999로
  ...
  가능성이 높다** — 둘 다 "정확해질수록 합계가 나빠진다"이고, 둘 다 평타를 늘린다.
- **어디서 시작하나 (착수 전 정해둔 첫 수):** 최악 덱인 **deck3(1.100x)의 소스별
  분해**를 먼저 본다 —

      python scripts/measure_deck_breakdown.py --deck rapi-red-hood-b1,crown,\
          rei-ayanami-tentative-name,asuka-shikinami-langley-wille,helm-signature

  `normal_attack` 항이 과대인지 한 번에 드러난다. 덱 총합만 보는 캘리브레이션
  스크립트로는 **어느 소스가 부풀었는지 못 가르므로** 이 분해가 선행이다.
- **후보를 가르는 법 (오늘 세 번 쓴 기법):** 엔진 상수·항을 하나씩 **0으로 죽여
  보고** 실기록을 재채점하면, 어느 유닛이 얼마나 움직이는지가 그 항의 지분이다.
  모듈 속성을 몽키패치하고 `measure_record_calibration.measure()`를 다시 부르는
  방식이면 코드를 안 고치고 A/B가 된다(택티컬 베어 큐브·신데렐라 판별·아핀 재장전
  세 번 다 이 방식으로 착수 전에 크기를 쟀다). **착수 판단을 추측이 아니라 측정으로
  하는 것이 이 항목의 전제다** — 재장전이 그랬듯 "고치면 합계가 나빠지는" 항일 수도
  있기 때문이다.
