...
**남은 것 — 세그먼트의 트루뎀 타이핑은 못 게이트한다.** `weapon_mode_schedules`의 프로필은
`damage_type`을 **세그먼트를 만들 때** 고정하는데, 세그먼트는 **발사 타임라인을 만드는 도중**에
지어진다 — 읽을 자원이 아직 없다. 창을 「타이핑된 앞부분 + 일반 뒷부분」으로 쪼개려면 스케줄이
자기 소유자의 발사를 볼 수 있어야 하고 그건 다른 엔진 조각이다.
**남기는 비용은 잰 값으로 약 0.6%다**(타이핑 자체가 DEF 보스에서 +3.54%, 그 중 닫혀야 할 17.5%).
라이더(+7.34%)가 더 큰 쪽이고 그건 닫았다.

**주의 — 스윕은 이 항을 못 본다.** `sweep_slug_damage.py`의 보스는 `enemy_def = 0.0`이라
**트루뎀과 일반뎀이 수치적으로 같다.** 타이핑을 스윕으로 재면 언제나 0이 나오는데 그건 게임에
대해 아무 말도 안 하는 것이다. 이 표의 「−4.72%/−1.23%」는 실기록 보스의 DEF를 넣고 잰 값이다.
