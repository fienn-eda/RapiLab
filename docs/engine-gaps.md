...
**효과 — 캘리브레이션이 나빠진다, 그게 맞다.** `moran-signature` **0.962x → 0.821x**
(그녀의 창모드 코어히트율이 0.53 → 0.13). 덱5 총합은 1.012x → **1.000x**로 좋아지고,
합계는 1.043x → 1.041x, ±15% 이내는 19/25 → **18/25**다. 실측이 근사를 대체한 것이므로
숫자가 나빠진 쪽이 더 정확하다 — 그리고 이제 **그녀가 왜 0.821x인지**가 다음 질문이다.
