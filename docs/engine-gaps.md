...
1. ~~**실기록 보스의 코어 지름 — 미측정.**~~ **✅ 해소 (2026-08-14).** Fienn이 전투 중
   ...
   **이 항목이 「막고 있다」던 둘이 이제 문다.** 「MG 정확도 예열」은 탄창 앞 29발이
   ...
   배선해도 inert였다), 「무기변형 세그먼트 탄착군」은 나유타에서 이미 값이 나온다
...
- **재장전 속도 100% 초과에서 고정 구간 0.148초가 살아남는지.** 현행은 0으로 클립하고
  이 배선은 그 선택을 바꾸지 않았다. 살아남는다면 공백이 21프레임으로 늘어 효과가 줄어든다.
...
## 새 갭: MG 정확도 예열 — 탄창 앞 29발이 코어를 놓친다 (2026-08-07)
...
| — | MG 정확도 예열(탄창 앞 29발 코어 손실) | 미집계(실기록 MG 6유닛에 실제로 문다, 2026-08-14~) | 미착수 — 발 인덱스 배선 필요 | 발사 타임라인 |
