...
**막는 것:** 선언되지 않은 무기변형 세그먼트(라플라스: 얼티메이트 히어로, 신데렐라:
크리스탈 웨이브, 맥스웰 등)가 기저 무기와 다른 탄착군을 갖는다면 그 차이.
**필요한 것:** 유닛별로 「변형 중에 코어를 항상 맞히는가」 한 줄 — 나유타에 물어본
것과 같은 질문이고, 답이 예면 프로필에 한 줄 추가로 끝난다.
