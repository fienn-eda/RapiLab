...
**필요한 것:** 「변형 중에 코어를 항상 맞히는가」 한 줄을 **모란·츠바이·그레이브·
질: 발렌타인·스노우화이트**에 물으면 된다. 답이 예면 프로필에 `always_core_hit: True`
한 줄이다.

**실측 진행 (Fienn, 2026-08-15):** 나유타(변신 시)와 신데렐라: 크리스탈 웨이브가
**둘 다 코어히트 100%**다. 둘 다 **엔진이 이미 그렇게 주고 있다** — 나유타는 선언으로,
신데렐라 CW 스나이프는 SR 프로필(10px)이 코어보다 좁아서다. 즉 이 실측은 갭을
가리키는 게 아니라 **모델을 확증한다.** 실기록에 앉는 `cinderella-crystal-wave-mg`는
변형이 아니라 **MG 모드 고정 프로필**이라 이 갭의 대상이 아니고, 그녀의 1.237x는
다른 원인이다.
...
| — | 무기변형 세그먼트 탄착군 — 기저 무기로 근사, 미측정 | **7** (세그먼트 20슬러그 중 기저가 AR/SMG/SG인 것만; 나유타는 선언 완료) — 츠바이 2종(SG, 코어율 0.038) · 모란 2종·그레이브·질: 발렌타인·스노우화이트(AR, 0.425) | 미착수 — **재집계 2026-08-15**: 기저가 SR/RL·수렴 MG인 12슬러그는 코어율이 어차피 1.0이라 근사가 무의미하다. 실기록에 앉은 건 moran-signature 하나(0.962x 과소, 방향 맞음). 필요한 건 다섯 유닛에 「변형 중 코어 100%인가」 한 줄 | 발사 타임라인 |
