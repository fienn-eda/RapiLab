...
- 마지막 갱신: 2026-07-27 (**gap #15 해소 — 소장품 스킬 효과 배선.** 데이터 테이블
...
| 14 | **적정사거리(effective range) 보너스** — 배수는 **실측 확정(+0.30)**, 배선만 없음 | 1 (Ade: Agent Bunny) | 미착수 — **모델링 결정 필요**(아래) | 스탯 배선 |
| ~~15~~ | ~~소장품 스킬 효과 미모델~~ (무기군별 스탯 보너스; 에이드 = 차지대미지 6.31% 배율) | 8+ 확인, **실제로는 소장품 낀 전원** | **해소 (2026-07-27)** — 데이터 테이블 + 리졸버 + 유닛별 배선 + 데미지 적용 전부 완료. SMG·RL 무기군 데이터 없음, SR/SG/AR은 CDN 캡처가 아니라 손으로 유도한 값, R 등급 사다리 미상. **실제 로스터가 소장품을 안 실어 캘리브레이션은 아직 무변동** — Fienn 재동기화 대기 | 신규 스탯 소스 |
...
### 14. 적정사거리 보너스 — 배수는 확정, **"언제 사거리 안인가"가 미결** (2026-07-27)
...
### 15. **소장품(collectible/애장품) 스킬 효과** — ✅ 해소 (2026-07-27, 데이터 갭 잔존)
...
일치) · 크리 **+0.5**에 다른 크리대미지원 **0** · 적정사거리 **+0.30**(gap #14) ·
