...
**아스카는 정확히 그 유닛이다.** 그녀의 강제 재장전은 Emergency Repair의 「고정 60%
증가」로 **1.08초**, 자연 재장전 2.478초의 절반도 안 된다. 즉 위 9pt 중 얼마는 엔진이
과청구한 몫이고, 그 크기는 감쇠율을 재기 전에는 모른다 — 아래 「새 갭: MG 예열은
재장전 중에 시간을 갖고 빠진다」 참고.
...
**막는 것 (물리는 곳이 큰 순서):**
- **아스카: WILLE** — 강제 재장전이 「고정 60% 증가」로 **1.08초**, 자연 재장전
  2.478초의 절반도 안 된다. 그녀는 이 갭에 가장 크게 물리는 유닛이고, 실기록에서
  0.938x로 과소 읽히는 몫의 일부가 여기서 온다.
- **재장전 속도 버프를 받는 모든 MG** — 크라운·프리바티·Resilience 큐브가 붙은 덱에서
  재장전이 짧아질수록 엔진의 과청구가 커진다. 지금 MG 무기군 평균이 1.094x로 **과대**인
  것과 방향이 반대라, 이 갭을 닫으면 MG는 더 과대해진다.
- 자연 재장전만 하는 MG는 재장전이 길어(로산나 1.67초 툴팁) 상대적으로 덜 물린다.

**필요한 측정 — 기존 영상에 이미 있을 수 있다.** `docs/measurements/mg-spinup.md`의
원본은 한 탄창에서 끝난다(첫 발 863 → 비움 1256 → 재장전 완료 1367). 그 **다음 탄창의
첫 발 프레임과 예열 완료 프레임** 두 개면, 콜드 137프레임과 대조해 재장전 111프레임
(1.85초) 동안 빠진 양이 나온다. 「완전 방출까지 D초」 하나로 못박히면 배선은
`magazine_shot_offset`이 램프를 0이 아니라 k번째 구간에서 시작하게 하는 것이고,
`k`는 직전 재장전 길이의 함수다 — 이미 탄창 워크가 `actual_reload_time`을 알고 있으므로
새 정보를 만들 필요가 없다.

**아직 모르는 것:** 감쇠가 선형인지, 그리고 「MG heating up speed ▲/▼」가 **차오르는
속도만** 바꾸는지 **빠지는 속도까지** 바꾸는지. 후자라면 아스카의 ▼100%는 재장전 중
보존에도 영향을 준다. 재장전 속도가 다른 두 조건에서 재면 첫 질문이, 아스카/레이가
붙은 조건에서 재면 두 번째가 갈린다.
