...
- 마지막 갱신: 2026-07-31 (**gap #19 해소 — 클립형 재장전. 분할 수는 실측이
  ...
  **실기록 5덱에 클립 유닛이 없다 — 테이블을 비우고 다시 재면 5덱 배수가 표시
  ...
  시뮬과 어긋난다(**14슬러그**, 센티 60초 **48 vs 37**). Fienn이 분리 결정.
  단 `closed_form`은 **쓰이지 않는 모듈**이라 이 어긋남은 지금 아무것도 안 바꾼다.
...
### 20. `closed_form`의 발수가 차지 모션 딜레이를 무시한다 — 미착수, 사실상 무해 (2026-07-31)

- **무엇:** `closed_form._shot_count`가 부르는 `attack_rate.generate_shot_times`에는
  **차지 모션 딜레이를 받는 인자가 없다**. 캐시 키도 무기·탄창·재장전·차지시간·환급뿐
  이다. 시뮬레이션 경로(`_base_shot_records`)는 `weapon_stats["charge_motion_delay"]`를
...
- **★ 그런데 `closed_form`은 어디에도 안 쓰인다.** `backend/app/` 어느 모듈도
  임포트하지 않는다 — 임포트하는 건 테스트와 분석 스크립트 둘
  (`measure_unmodeled_damage_share.py` · `validate_surrogate_recall.py`)뿐이다.
  캐스케이드 Phase 1은 `surrogate.py`(샘플 회귀)를 쓰고, **닫힌 수식 스코어러는
  옵션 B로 측정 후 기각됐다**(Spearman 0.54, top-100까지 시뮬해도 최적 미발견 —
  `docs/decisions.md`). 따라서 이 수정은 **덱 탐색 순위도 캘리브레이션도 안 움직인다.**
- **그래서 진짜 선택지는 둘이다:** ① 딜레이를 배선해 두 경로를 일치시킨다(죽은
  모듈을 정확하게 유지) ② 모듈을 지운다(기각된 접근이고, 남아 있으면 살아 있는
  경로로 오인된다 — 이 항목이 "덱 탐색 순위가 움직인다"고 잘못 적혔던 이유가 그것이다).
  분석 스크립트 둘이 기각 근거를 재생산하는 용도라 ②는 그 둘의 운명과 같이 정해야 한다.
- **왜 갭 #19와 분리했나:** 두 수정이 섞이면 어느 쪽이 무엇을 바꿨는지 갈리지
  않는다(Fienn, 2026-07-31). 이 이유는 그대로 유효하다.
- **주의 — 이 항목에 잘못 적혀 있던 숫자(2026-07-31 검증):** "13슬러그"는 14가
  맞고, "센티 41 vs 37 / 약 11% 과대"는 **이 갭이 아니라 갭 #19의 전/후**(발당
  1.45 → 1.6167초)다. 두 경로 차이는 센티에서 48 vs 37이다. 인용됐던 합계
  1.010x도 지금은 1.015x다.
- 참고: `backend/app/closed_form.py:168-187` · `backend/app/attack_rate.py:335-347`.
