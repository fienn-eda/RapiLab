...
**남은 열린 질문 — 소장품 레벨 0의 의미 (Fienn 확인 필요).** 실계정에서 레벨 0
착용이 **31유닛**(인코딩돼 실제 계산에 드는 것 9유닛)인데, **스탯 쪽은 레벨 0을
'미착용'으로 보고 0을 주고**(실측 근거: 그 유닛들의 raid400 ATK가 커브값만큼 높지
않다) **스킬 쪽은 사다리 1단을 준다**(`test_level_zero_is_a_real_level_not_an_empty_slot`).
게임 클라이언트 자신도 `atk.at(0)`·`level1.at(0)`를 그대로 읽으므로 번들을 읽어서는
못 가린다.

**크기는 작지 않다.** R 등급은 1.56~5.67%지만 **SR 등급은 레벨 0이 이미 사다리
1단의 큰 값**이다 — 헬름: 아쿠아마린 코어 대미지 **+10.22%**, 볼륨·리터 평타 배율
**×1.0473**, D: 킬러 와이프 차지 배율 **×1.0474**. 볼륨은 실기록 덱4 편성이고
리터는 주력 버퍼다. 인게임에서 레벨 0 소장품의 스킬 줄이 활성으로 보이는지 확인하면
닫힌다(걸린 값이 가장 큰 헬름: 아쿠아마린이 확인 대상으로 좋다).
**그 전까지 현재 동작을 유지한다.**

레벨 0 착용 9유닛(인코딩분): 헬름: 아쿠아마린(SR AR) · 볼륨 · 리터(SR SMG) ·
D: 킬러 와이프(SR SR) · 기요틴: 윈터슬레이어 · 줄리아 · 팬텀(R AR) ·
플로라 · 로잔나(R MG).
