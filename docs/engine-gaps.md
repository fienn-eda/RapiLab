...
Happy Memories, 스택당 +1)는 **대미지에 영향이 없음이 확인됐다**(Fienn 2026-07-28
판독). 같은 샷 총합을 더 많은 펠릿이 나눠 가질 뿐이라 발당 비율만 내려간다
(`0.992143 = 1.091357/1.1` 등, 실측과 잔차 4e-06 이내로 일치 — 상세는
`docs/decisions.md`의 SG·SMG 소장품 버킷 결정). 엔진에 펠릿 개념이 없는 것은
여전하지만(`dorothy_serendipity.py`), 그 미모델이 막는 것은 **명중 안정성·코어
커버리지**이지 데미지가 아니다. 실제로 남는 데미지 갭은 펠릿 **개수를 세는 트리거**
하나뿐이다 — 도로시: 세렌디피티 Flash의 「80/160 펠릿 명중 시」(Pierce·Attack
Damage·Pierce range 발동). 막힌 유닛은 여전히 그녀 1명이지만, 성격은 "펠릿 대미지
모델 없음"이 아니라 "펠릿 카운터 트리거 없음"이다.

### 참고 — 무기변환 세그먼트는 소장품의 「무기」 배치 스탯을 못 받는다

갭으로 등록하지 않고 기록만 남긴다. `weapon_mode_schedule`로 짓는 세그먼트(무기변환
창)의 대체 프로필은 캐스터 자신의 스킬 데이터만으로 조립되는 정적 dict라
`weapon_stats`를 전혀 읽지 않는다(예: `nayuta.py`의
`build_memory_incineration_weapon_mode_schedule`). SG·SMG 소장품의 「일반 공격
대미지 배율」을 2026-08-03에 무기 스탯 버킷에서 가산 버킷
...
무기 스탯은 세그먼트 밖에서만 유효하다. **같은 사각지대가 RL·SR의
`charge_damage_percent`(에이드 사격장 실측으로 확정된 무기 스탯 배치)에도 그대로
남아 있다** — `weapon_mode_schedule`을 쓰는 유닛이 그 소장품을 착용하면 세그먼트
구간의 차지댐이 소장품 보너스를 놓친다. 오늘은 이 조합(무기변환 세그먼트 보유 +
RL/SR 차지대미지 소장품 착용)에 걸리는 유닛이 없다 — 확인만 해 두고 고치지 않는다.
