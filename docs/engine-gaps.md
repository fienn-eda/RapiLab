...
4. ~~**Pattern B (시간감쇠 게이지·다중소스 스택) — 5슬러그.**~~ **✅ 대부분 해소
   (2026-08-14). 진짜 남은 건 `phantom-signature` 하나다.** 「5슬러그」는 한 갭이 아니라
   **서로 다른 네 가지**를 한 라벨에 묶은 것이었고, 원문과 코드를 대조하니 넷 중 셋은
   ...
   - `phantom-signature` — **유일하게 남은 진짜 갭.** 대거가 **cap 도달 시 자기를 소비**
     하고 그 소비가 다른 소스의 스케줄을 바꾼다(순환). 값어치는 **저 한 자리**다.
...
### 4. `phantom-signature` — 유일하게 남은 진짜 갭

애장품이 「평타 30발마다」라는 **두 번째 대거 소스**를 준다. 여기까진 기존 프리미티브로 된다
(다중소스 fill + 5초 개별 만료 + cap 3). 못 하는 건 **자기소비**다:

- Thief's Vision의 셋째 불릿이 「최대 스택일 때 **스택을 제거**하고」 250% 분산딜을 낸다 —
  자원이 **cap에 닿으면 0으로 떨어진다.** 지금 reset 트리거는 battle_start·own_burst·
  own_burst_delayed·full_burst_end뿐이고 **「cap 도달」이 없다.**
- 게다가 첫째 불릿이 **Calling Card를 제거**하는데, 그게 **다른 대거 소스를 다시 여는 조건**이다.
  즉 한 소스의 fill 스케줄이 **소비 시각에 의존**한다. 지금의 해소 패스는 소스별 스케줄을
  독립으로 만들어 합치므로 이 순환을 표현할 수 없다 — **순차 walk**가 필요하다
  (`_resolve_squad_burst_cycle_resource`가 다른 자원에 이미 쓰는 모양).

**값어치는 저 한 자리다 (측정, 2026-08-14).** 그녀의 실제 발사율은 **11.16발/초**라 현행 상수
60발은 **5.38초**인데 Calling Card 소스는 **5.00초** 시계로 돈다 — 진짜 주기는 상수보다 **짧고**,
즉 현행 인코딩은 프록을 **과소**로 센다. 주기 민감도는 55발 **+1.54%** · 50발 +3.37% ·
45발 +5.44% · 70발 −1.50%다.

**명중률 쪽은 지금 값이 0이다.** 대거 스택을 1→2→3으로 올려도 **딜이 0.00% 움직인다** — 명중률은
`BossProfile.core_diameter_px`를 통해서만 딜에 닿는데 그 필드는 opt-in이고 **제품이 덱을 짜는
보스 중 그걸 가진 것이 하나도 없다**(`data/raid-rotations.json` 0/6, 기본값 `None`). 실기록
캘리브레이션에서만 살아 있고 `phantom-signature`는 그 로스터에 없다. **레이드 보스에 실측
코어 지름이 들어가는 날 이 항이 깨어난다.**
