...
**막던 15슬러그 중 11은 엔진 쪽 블로커가 풀렸고, 4는 여전히 inert이나 사유가
바뀐다**(자기 자신은 RL/MG라 기본 탄착군 10px가 이미 코어 안 — 단 그 중 셋(디젤·
모더니아·앵커)의 명중 버프는 원문상 「아군 전체」 대상이라 SG/SMG/AR 아군에게는
무해하지 않다, 자기 한정은 마스트뿐). **11 쪽은 엔진 능력(capability) 레벨의 해소이지 유닛
레벨은 아니다** — 15개 모듈 어디에도 `Effect("hit_rate", ...)` 등록이 없어(전수
grep 확인) 재인코딩 전에는 여전히 딜에 영향이 없다. 슬러그별 갈림과 재인코딩
필요 여부는 `docs/encoded-nikkes.md`.
...
**막는 것:** 세그먼트가 기저 무기와 실제로 다른 탄착군을 갖는다면 그 차이. 코어
50px에서 나유타의 SR 프로필 변형샷은 세그먼트 라벨(SR, 10px, p=1.0)이 아니라 기저
무기(SMG, 110px, p=0.207)로 과금되는데, 그녀의 실측 덱 배율(0.7126x)은 순정
SMG(0.7145x)와 거의 같아 지금 선택이 틀렸다는 신호는 없다. **필요한 것:** 무기변형
유닛의 코어히트율 인게임 실측 — 세그먼트 프로필이 실제로 다른 조준원을 갖는지 그것만
답한다.
