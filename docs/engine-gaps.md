...
**캘리브레이션 (2026-08-14): 1.036x · 20/25 · 과대 합계 +1.523B / 과소 −0.409B**
(SR 1.126x · MG 1.078x · RL 1.035x · SMG 0.997x · AR 0.958x, 캐비엇 2유닛 제외).
...
1. ~~**실기록 보스의 코어 지름 — 미측정.**~~ **✅ 해소 (2026-08-14).** Fienn이 전투 중
   ...
   **이 항목이 「막고 있다」던 둘이 이제 문다.** 「MG 정확도 예열」은 탄창 앞 29발이
   250px에서 시작하므로 배선하면 MG 1.078x를 끌어내리는 방향이고(코어가 없던 동안에는
...
## 새 갭: MG 예열은 재장전 중에 **시간을 갖고** 빠진다 (2026-08-14)
...
**이제 필요한 것은 측정이 아니라 설계 판단이다.** 배선하려면 `magazine_shot_offset`이
일정한 낮은 연사 대신 **측정된 곡선**을 쓰고, 시작 위치를 직전 발사 공백에서 유도해야
한다. 그런데 곡선으로 바꾸면 **콜드 램프의 발사 시각 분포도 같이 바뀐다** — 총합은
같지만 그 안의 48발이 재배치되므로, 발수 카운터·first/last-bullet 마커·풀버스트 창
경계에 걸친 모든 MG 유닛이 움직인다. 이 배치의 범위를 넘고 자체 설계가 필요하다.

**방향 충돌은 그대로다:** MG 무기군은 이미 1.094x로 **과대**인데, 이 갭을 닫으면 발사
수가 늘어 더 과대해진다. 배선 전에 그 상호작용을 어떻게 볼지부터 정해야 한다.

**남은 미측정은 하나뿐:** 위치 2~24 사이의 모양(지금은 두 점 사이 직선, 평균
2.5프레임/발). 그 구간 안에서도 앞이 느릴 가능성이 높다.

**배선은 자리가 있다:** `magazine_shot_offset`이 램프를 0번이 아니라 k번째 구간에서
시작하게 하고, `k`는 직전 발사 공백의 함수다 — 탄창 워크가 이미 `actual_reload_time`을
알고 있으므로 새 정보를 만들 필요가 없다.

**또 하나 모르는 것:** 「MG heating up speed ▲/▼」가 **차오르는 속도만** 바꾸는지
**빠지는 속도까지** 바꾸는지. 후자라면 아스카의 ▼100%는 재장전 중 보존에도 영향을 준다.
