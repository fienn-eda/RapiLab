...
| — | ~~위치 기반 스코프~~ (「자신과 양 옆 아군 2명」 — squad로 근사) | 2 (rouge 착륙 · flora-signature 2불릿 보류) | **해소 (2026-08-13)** — `SquadContext.adjacency` + `neighbor_slugs` → `slugs:` 스코프, `registry.SEATED_BUFF_SLUGS`. 좌석은 **버스트 우선순위와 다른 축**이라 덱 리스트 인덱스로 읽으면 안 된다. 탐색은 결정적 정책(최고 ATK 2명), 보고는 C(4,2)=6 전수 최적(`evaluate_deck_best_seating`)이고 고른 좌석을 `seating`으로 싣는다. 실측: 5인 덱 하나에서 최적이 정책보다 **+2.01%**, 정책은 6개 중 4등 | 신규 스코프 |
