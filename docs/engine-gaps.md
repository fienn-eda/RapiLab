...
4. ~~**Pattern B (시간감쇠 게이지·다중소스 스택) — 5슬러그.**~~ **✅ 완전 해소
   ...
   **서로 다른 네 가지**를 한 라벨에 묶은 것이었고, 원문과 코드를 대조하니 넷 중 셋은
   ...
   - `phantom-signature` — **갭 아님.** 착수하려고 walk를 실제 발사 시각에 돌려 보니
     **인코딩된 60발 상수가 근사가 아니라 정확값**이었다. 베이스를 1스택에 못 박는
     그 대칭이 이 빌드의 소스 1에도 그대로 걸려, 애장품의 30발 메트로놈만 누적된다 →
     **30 × (3−1) = 60**. 발사율 6.1~40발/초 전 구간에서 간격이 예외 없이 60이다.
...
**남는 것은 명중률 하나뿐이고, 그 값은 지금 0이다.** 그녀의 Hit Rate는 1스택으로 박혀 있는데
진짜 카운트는 톱니이고 시간평균이 **1.48스택**이다(같은 walk). 그런데 대거를 1→2→3으로 올려도
**딜이 0.00% 움직인다** — 명중률은 `BossProfile.core_diameter_px`를 통해서만 딜에 닿는데 그
필드는 opt-in이고 **제품이 덱을 짜는 보스 중 그걸 가진 것이 하나도 없다**
(`data/raid-rotations.json` 0/6, 기본값 `None`). 실기록 캘리브레이션에서만 살아 있고
`phantom-signature`는 그 로스터에 없다(Fienn이 그녀의 애장품을 안 갖고 있다 —
`collectible_tid=100101 · level=0`, 보유 유닛은 `2xxxxx · level=2`).

**그리고 고칠 때 1.48을 박으면 안 된다.** 명중률은 **면적비**로 딜에 들어가므로 「카운트의
평균」과 「그 결과 코어히트율의 평균」은 다른 수다 — 비선형 경로에 시간평균 카운트를 넣는 것은
아무도 안 잰 방향으로 틀리는 짓이다. 올바른 처방은 **계단함수**(대거를 진짜 자원으로 만들어
count-scaled `ResourceBuff`를 물리는 것)이고, **레이드 보스에 실측 코어 지름이 들어가는 날**
지을 일이지 그 전에 할 일이 아니다.
