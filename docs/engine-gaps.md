...
### 참고 — 펠릿 수 자체는 데미지 갭이 아니다, 남는 것은 펠릿 개수 트리거뿐
...
SG·SMG 소장품 버킷 결정). 엔진에 펠릿 개념이 없는 것은 여전하지만(`dorothy_serendipity.py`), 그
미모델이 막는 것은 **명중 안정성·코어 커버리지**이지 데미지가 아니다. 실제로 남는
데미지 갭은 펠릿 **개수를 세는 트리거** 하나뿐이다 — 도로시: 세렌디피티 Flash의
「80/160 펠릿 명중 시」(Pierce·Attack Damage·Pierce range 발동). 막힌 유닛은 여전히
