...
판독, 상세는 `docs/measurements/arcana-fortune-mate-happy-memories.md`). 같은 샷
총합을 더 많은 펠릿이 나눠 가질 뿐이라 발당 비율만 내려간다(`0.992143 = 1.091357/1.1`
등, 실측과 잔차 3e-07 이내로 일치 — 상세는 `docs/decisions.md`의 SG·SMG 소장품
버킷 결정). 엔진에 펠릿 개념이 없는 것은 여전하지만(`dorothy_serendipity.py`), 그
