...
**도로시: 세렌디피티는 표가 약속한 p=100%에 아직 못 간다.** 그 값은 Flash
(+98.18%)가 있어야 나오는데 트리거가 80펠릿 카운터이고, 엔진에 펠릿 모델이 없다.
Radiant Wings(+40.68%)만으로는 10.1%다. → 아래 「펠릿 카운터」 갭.
...
## 새 갭: 펠릿 카운터 — 도로시의 Flash가 명중 모델의 최대 항인데 못 닿는다 (2026-08-07)
...
110% 특이점을 넘어 SG 탄착군이 0이 되고 코어히트율이 100%가 된다.** 설계문서 §6이
그녀를 이 모델의 최대 수혜자로 적은 근거가 정확히 이 불릿이다.

**막는 것:** 도로시: 세렌디피티의 Flash 전량(명중·공격데미지·Pierce·펠릿 고정).
현재 인코딩은 Radiant Wings만이라 코어 50px에서 10.1%에 머문다(100%가 아니라).

**필요한 것은 「펠릿 카운터」 하나가 아니다.** 데이터에 발당 펠릿 수는 있다
(`shot_detail.shot_count`, SG 전원 10 · 츠바이만 5)이므로 80펠릿 = 8발로 환산은
되지만, 그 환산이 성립하려면 **10발이 전부 표적에 맞는다**고 가정해야 한다. 그것은
결정론이 아니라 플레이 조건이고 — gap #21이 `p_조준`이라 부르는 바로 그 항이다 —
탄착군이 250px인 샷건에서 특히 크게 틀린다. 게다가 발동 후 3 round는 펠릿이 1로
고정되어 다음 80펠릿까지 걸리는 발수가 달라지므로, 정상상태 주기 자체가 이
가정에 의존한다.

**그래서 값은 실측이 정한다:** 인게임에서 Flash가 몇 초마다 터지는지 한 번 재면
(소다의 골든칩 바닥값을 정한 것과 같은 방식) 그 주기를 상수로 박아 per-shot 룰로
끝난다. 펠릿 시뮬레이션은 필요 없다.

Fienn 판정 2026-08-07: 이번 배치에서는 **보류 유지**, 설계문서의 「최대 수혜자」
주장을 정정.
