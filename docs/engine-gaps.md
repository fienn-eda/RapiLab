...
| 19 | **클립형 재장전** (탄창을 N등분해 여러 번 장전 — Centi는 6발을 2발씩 3회) | 2 확인 + **런처·샷건 일부 미확인** | 미착수 — 분할 수가 데이터에 없어 실측 상수 테이블 필요(아래) | 발사 타임라인 |
...
### 19. 클립형 재장전 (탄창을 N등분해 여러 번 장전) — 미착수 (2026-07-30)

- **무엇:** 일부 런처·샷건 니케는 탄창을 **한 번에** 채우지 않는다. Centi는 6발
  탄창을 **정수 단위로 3등분**해 2발씩, 0.5초(30프레임) 장전을 **세 번** 한다
  (Fienn, 2026-07-30). 게임 데이터 파일에는 `maxAmmo=6`, `reloadTime=0.5`만 있어
  분할 횟수가 어디에도 없다.
- **왜 막혔나:** `attack_rate`의 발사 타임라인은 "탄창을 다 쓰면 `reload_time` 한 번"
  이라는 단일 모델이다. 클립형은 같은 탄창에서 재장전을 여러 번 하므로 실제
  케이던스가 더 느리다 — Centi 기준 발당 1.45초(현 모델) vs 1.617초(실제),
  약 11% 낙관.
- **막힌 유닛:** 확인 **2** (`centi`, `centi-signature`). Fienn에 따르면 런처·샷건
  니케 중 **일부**가 같은 방식이며, 명단은 아직 확정되지 않았다 — Centi 인코딩
  이후 **모아서 처리**하기로 함(Fienn, 2026-07-30). 데이터 파일이 분할 횟수를 담지
  않으므로, 갭 규모를 세려면 유닛별 확인이 선행돼야 한다.
- **파급:** 순수한 케이던스 문제로 보이지만 **버프 가동률로 전파된다**.
  `centi-signature`의 유효 쿨다운(풀차지 1발당 스킬2 쿨 9.16% 감소)은 그녀의 발당
  간격에서 유도되므로, 간격이 낙관적이면 그녀의 스쿼드 ATK 버프 가동률도 낙관적이다.
  `last_bullet`/`first_bullet` per-shot 마커도 "탄창당 1회"를 전제하므로 클립형에서
  의미가 달라진다(현재 소비하는 클립형 유닛은 없음).
- **확장 방향(미확정):** 발사 타임라인에 유닛별 `reload_splits`(기본 1)를 두고
  탄창을 균등 분할해 각 경계마다 `reload_time`을 넣는 것이 가장 작은 변경으로 보인다
  — 단, 분할 수는 데이터에 없으므로 실측 기반 상수 테이블이 필요하다
  (`TIMED_CHARGE_MOTION_DELAY`와 같은 성격).
- 참고: `backend/app/skill_rules/centi.py` docstring · `backend/app/attack_rate.py`.
