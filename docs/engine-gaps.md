...
## 새 갭: 코어 보너스 배율이 유닛별로 다르다 — 4유닛 2.5배, 엔진은 전원 2.0배 (2026-08-07)

`shot_detail.core_damage_rate`가 미란다·퀀시: 이스케이프 퀸·리틀 머메이드·치사토
니시키기 넷은 `25000`(2.5배)인데, 엔진의 `raid_simulator.CORE_HIT_BONUS = 1.0`
(major modifier에 +1.0 = 2.0배)은 전원에게 균일하게 쓰인다. 이 넷은 코어 보너스가
**과소** 계상 중이다. 이 항목은 명중률 작업(코어히트 *확률*)과는 섞이지 않는다 —
이건 코어 보너스의 *크기*가 유닛별로 다르다는 별개 결함이다.

**막는 것:** 위 4유닛의 코어 히트 대미지. **필요한 것:** `CORE_HIT_BONUS`를
유닛별 상수로 바꾸고(`shot_detail.core_damage_rate` 소스), 기본값 2.0배(=1.0)를
유지한 채 이 넷만 오버라이드.
...
| — | `core_damage_rate` 유닛별 상이(2.5배 vs 엔진 2.0배) | 4(미란다·퀀시:이스케이프퀸·리틀머메이드·치사토) | 미착수 — `CORE_HIT_BONUS` 유닛별화 | 스탯 상수 |
