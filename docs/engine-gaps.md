...
| 15 | **에이드 평타가 균일하게 1.06배 부족** — 버킷은 무기 계수/차지로 좁혔고 출처 미결 | 1 (Ade) | 미착수 — **Fienn 확인 필요**(큐브 종류 · 무기 툴팁) | 데이터 결함? |
...
### 15. 에이드 평타가 균일하게 **1.06배** 부족하다 — 버킷은 좁혔고 출처가 미결 (2026-07-27)

**실측 7개가 전부 같은 비율로 어긋난다.** Fienn의 사격장 측정(1인, ATK 305,667
오버로드 별도, 스킬 **10/7/10**, 표적 DEF 100, 버스트 미사용)을 엔진 모델로 재현하면
크리 on/off · 사거리 in/out · 스파이렌즈 max/미만의 **7조합 전부가 model/measured =
0.94315**로 **1e-4 안에서 동일**하다.

**이 균일성이 곧 구조 검증이다.** 세 축이 서로 다른 버킷을 건드리므로, 어느 하나라도
틀렸다면 균일성이 깨진다 — 실제로 후보 3개를 넣어보고 전부 기각했다:
- **major 버킷**(코어 보너스·크리대미지원): 사거리 on/off 행이 서로 어긋남 → 기각.
  따라서 **코어히트 = 1.0, 크리대미지원 = 0**이 확정된다.
- **ATK 쪽**: 스파이렌즈 행에만 있는 `flat_atk`(Agent's Gaze) 때문에 어긋남 → 기각.
- **damage-up 버킷**: 같은 버킷의 `pierce_damage_up` 15.29%와 가산이라 어긋남 → 기각.

**⚠ 스킬레벨 함정:** 그녀는 2스킬이 **레벨 7**이라 ATK 13.78% · 관통대미지 15.29%다
(레벨 10이면 16% · 18.36%). 최대레벨 값으로 실측을 대조하면 조용히 틀린다.

**남은 곳은 전 행에서 상수인 버킷뿐** — **공격계수(무기 damage%)** 또는 **차지 대미지**,
둘 다 무기 속성이다. 필요한 값은 계수 69.04% → **73.20%**, 또는 차지 250%+11.81% →
**277.6%**. 출처 후보 두 가지, 아직 못 가림:
1. **무기 수치가 유닛별로 다른데 dotgg가 일반값을 준다** — 로스터 SR 14기 중 **13기가
   69.04/250/6으로 완전히 동일**하고 rouge만 71.07이다. 다만 그녀만 6% 낮은 것이라면
   전역 SR 문제는 아니다(기록에서 prika 1.069x · snow-white 1.29x로 SR이 낮지 않다).
2. **하모니 큐브** — 엔진은 전원 Resilience Lv.15(재장전 29.69% + 우월코드 19.09%)를
   가정하는데 둘 다 이 샷에 안 붙는다. Fienn의 실제 큐브가 차지 대미지 계열이면 정확히
   이 자리다. **데이터에 `resilience_cube` 하나뿐이라 로컬 확인이 불가능하다.**

**Fienn에게 물을 것:** 사격장의 에이드가 낀 **큐브 종류**, 그리고 인게임 무기 툴팁의
**대미지% / 차지 대미지%**. 둘 중 하나면 닫힌다.

`backend/tests/test_damage_formula.py`가 **균일성을 고정**한다(상수가 아니라). 누가
빠진 항을 찾으면 그 테스트가 시끄럽게 깨지고 상수가 1.0이 된다 — 조용한 6%보다 낫다.
