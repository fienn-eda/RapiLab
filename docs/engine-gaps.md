...
- 마지막 갱신: 2026-07-28 (**"추가 대미지" per-shot 넉의 FB 보너스 누락 — 리베랄리오 0.922x → 1.006x.** per-shot 넉 38개 중 21개가 `full_burst_bonus_eligible`을 안 넘기고 있었고, 헬퍼 독스트링이 **2026-07-12의 옛 문구 규칙**을 들고 있는 게 원인. 07-26에 "연산 시점으로 판단"으로 재구성됐지만 넉 호출부는 되짚지 않았다(평타 때와 **같은 형태의 누락**). **두 규칙이 합의하는 5유닛 6개 호출**에만 적용 — 덱1 0.946 → 0.977, 덱2~5 불변, 미달 −1.903B → −1.552B. **plain "as damage" 넉 15개는 미해결**: 일괄 적용 시 합계 1.025x지만 스노우화이트 1.386x, 반면 그녀 실측은 "as damage" 스윕도 보너스를 받는다고 말한다. 가르는 측정법은 `docs/insights.md`.
