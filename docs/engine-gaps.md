...
**그 9pt가 실제 비용인지는 물어서 닫혔다 (2026-08-14, Fienn 인게임): MG는 매 재장전
이후 예열이 필요하고, 스킬이 일으킨 강제 재장전도 마찬가지다.** 엔진의 탄창당 예열
모델이 맞고, 아스카가 버스트마다 치르는 그 예열은 게임이 실제로 청구하는 값이다.
`docs/measurements/mg-spinup.md`가 「이 측정이 가르지 못한 것」으로 열어 두었던 항목이
이 배치 때문에 값비싸져서(자연 탄창 소진이 아니라 버스트마다 청구된다) 닫았다.
