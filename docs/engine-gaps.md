...
| 목단(모란) · 모란 시그니처 | 100% **아님**. 창모드 탄착군 **100px** | ⏸ **막힘** — 아래 |
...
**남은 것 — 목단 2슬러그: 100px이 어느 단위인지.** 엔진의 `WEAPON_SPREAD_DIAMETER`는
게임 내부 값(`shot_detail.start_accuracy_circle_scale`)이고, 화면 px에서 그리로 가는
**일반형은 없다**(`docs/measurements/accuracy-circle-and-core-px.md`에서 한 번 틀렸고
철회됐다). 쓸 수 있는 건 **같은 프레임에서 잰 두 길이의 비**뿐이다:

    탄착군_엔진 = 탄착군_px × (기준_엔진 / 기준_px)

그러니 창모드 판독 100px과 **같은 프레임·같은 창 설정**에서 잰 기준 길이가 하나 필요하다
— 가장 쉬운 건 **같은 조건의 그녀 평소 AR 조준원**(엔진 75)이다. 2026-08-08 설정
(창 2333×1312을 2560×1440으로 녹화, k=0.7485)과 동일하다면 100 ÷ 0.7485 = **133.6**이
되지만, **그 가정을 확인 없이 쓰면 안 된다.** 값이 정해지면 세그먼트 프로필에
측정된 탄착군을 싣는 새 키가 필요하다(`always_core_hit`과 같은 모양의 선언).
...
| — | 무기변형 세그먼트 탄착군 — 기저 무기로 근사 | **2** (모란 2종) — 재집계+실측으로 20→7→2 | **거의 해소 (2026-08-15)** — 기저가 SR/RL·수렴 MG인 12슬러그는 코어율이 어차피 1.0이라 근사가 무의미. 나머지 7을 Fienn이 인게임으로 판독: 츠바이 2종·스노우화이트는 코어 100%라 `always_core_hit` 선언(스노우화이트 +15.9% · 츠바이 +7.4%, 실기록 불변 — 둘 다 덱에 없다), 그레이브·질: 발렌타인은 100%가 아니라 **현행 근사가 맞다**(질의 명중률 버프는 이미 인코딩됨). **남은 건 모란 2종** — 창모드 탄착군 100px의 단위 확인 필요(화면 px → 엔진 단위 일반형은 없다) | 발사 타임라인 |
