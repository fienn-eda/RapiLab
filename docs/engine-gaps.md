...
| 15 | **소장품 스킬 효과 미모델** (무기군별 스탯 보너스; 에이드 = 차지대미지 6.31% 배율) | 8+ 확인, **실제로는 소장품 낀 전원** | 미착수 — 테이블 수집 + 입력 모델 확장 | 신규 스탯 소스 |
...
### 15. **소장품(collectible/애장품) 스킬 효과가 전혀 모델링되지 않았다** (2026-07-27)
...
**엔진에는 표현 자체가 없다:**
- `stat_assembly`는 `collectible_sample`에서 **`atk`/`hp` 커브만** 읽는다
  (`collectible_atk`/`collectible_hp`). 스킬 효과는 아무도 안 읽는다.
- `collection_skill_group_data`(실제 효과)는 테이블에 **있는데 소비자가 없고**,
  `favoriteitem_skill_group_data`는 **비어 있다**(SSR 애장품 값 미수집).
- 테이블에 샘플이 **MG 무기군 하나뿐**이다. 에이드에게 필요한 건 **SR 무기군**이다.
- `UserNikkeState`에 **item tid도 level도 없다**. 동기화 경로(`roster_assembly`)가
  플랫 ATK/HP만 뽑아 쓰고 **버린다** — 즉 엔진은 소장품이 꼈는지조차 모른다.

**노출 범위는 8기보다 훨씬 넓다.** `-signature` 슬러그로 애장품(SSR) 보유가 드러나는
유닛은 **8/159**뿐이지만, 에이드처럼 **일반 소장품(SR)** 을 낀 유닛은 슬러그가 안 바뀌어
**완전히 안 보인다**. 그리고 방향이 중요하다 — 이 갭은 엔진을 **낮추는** 쪽이고, 현재
전체 캘리브레이션이 **0.914x(미달)** 이다. 미달 유닛들(아니스:스타 0.594 · 미하라 0.697 ·
리틀머메이드 0.759)이 여기 걸려 있을 수 있다.

**착수 전제 (Fienn과 상의할 것):** ① **무기군 6종 × 등급(SR/SSR) × 레벨**의 소장품 스킬
테이블 수집 ② `UserNikkeState`에 item tid/level을 실어 보내도록 입력 모델과 동기화 경로
확장. 둘 다 데이터·계약 변경이라 단순 배선이 아니다.
