...
- **보류로 남는 것:** Soda: Twinkling Bunny의 "Full Burst Duration ▲2/3초"는 이
  ...
  **2026-08-06 재판단(칩 소비를 감산으로 고친 뒤), 그리고 같은 날의 정정:** 1차
  결론은 「보류 유지」였다 — 소다가 **매 사이클** 버스트하는 시뮬 궤적에서 칩이
  50→43→35→28→20→13→10으로 떨어져 임계 20을 t=141.40에 가로지르므로 슬러그당
  고정값 자리에 못 올린다는 것이었다. **Fienn의 실측이 이를 뒤집었다**
  (`docs/measurements/soda-golden-chip-in-play.md`): 실전에서 그녀는 **격 사이클로**
  버스트하고, 그러면 칩이 33~50에서 **순환**하며 풀 버스트는 **항상 15초**다. 칩 ≥20
  → +5초 → 창당 fill 9~10 → 격번 소비 17을 상쇄, 자기일관적이므로 **그 운영에서는
  상수로 접힌다.** 미모델의 대가는 이 덱에서 **+9.3%**(덱 전원이 FB 창을 50% 더 갖는다).
  남은 설계 과제는 조건의 모양이다 — 원문이 `entering Burst Stage 3 / Affects all
  allies`라 **소다가 버스트할 필요가 없고**, 지금의 `FULL_BURST_DURATION_DELTA`가
  읽는 「사이클을 연 B3」와 다르다.
