...
| 16 | **적정사거리(effective range) 보너스** — 배수 실측 확정(+0.30) | 1 (Ade: Agent Bunny) | **배선 완료 (2026-07-31)** — `BossProfile.in_effective_range`, **기본 False**(거리는 스테이지가 정한다 · Fienn). 남은 건 "실기록 보스가 사거리 안이었나" 하나 | 스탯 배선 |
...
**✅ 배선 완료 — `BossProfile.in_effective_range`, 기본 False (2026-07-31).**
"사거리 안인가"는 **거리를 누가 정하느냐**의 문제였고, Fienn의 판정은
...
기본값 False는 배선 전과 **정확히 같은 동작**이다(테스트 1672 → 1678 passed, 기존
1672건 전부 무변동). 배선을 켜지 않은 채 남기는 것은 의도된 것이다 — 다음 측정이
**필드 하나**로 끝나게 하려는 것이고, 죽은 채로 방치하지 않으려고
`backend/tests/test_effective_range.py` 6건이 켰을 때의 동작을 고정한다.

**켰을 때의 크기 (측정함):** 합계 **1.055x → 1.098x**, ±15% 이내 **19/25 → 14/25**.
평타 위주 유닛이 크게 뛰고(helm 1.35 → 1.45 · mast 1.31 → 1.45 · crown 1.23 → 1.37)
RL 유닛은 불변이라 **무기군 간 상대 평가가 바뀐다** — 추천 결과가 움직이는 변경이다.

**남은 미결 — 이제 하나뿐이다:** 실기록의 보스(Annihilio)를 **사거리 안에서 싸웠는가**.
그 답이 실기록 채점의 `in_effective_range`를 정한다. 로스터 전체가 아니라 **인카운터
하나**에 대한 질문이 됐다는 것이 이 항목이 좁혀진 지점이다.
...
- **배제한 후보 — 다시 파지 말 것:**
  ...
  | 발수 과대 | 발수가 틀리면 `per_shot_nuke`도 같이 틀려야 한다. 그렇게 채점하면 **악화**(log-sd 0.085 vs 0.078, ±15% 17 vs 21) |
