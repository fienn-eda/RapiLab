...
## ★ 재장전 공식이 실측과 다르다 — 모델 확정, 착륙 보류 (2026-07-29)
...
**왜 아직 안 넣었나**: 넣으면 실기록 캘리브레이션이 **1.010x → 1.044x**로 초과하고
±15% 이내가 21/25 → 20/25가 된다(deck3 크라운 1.027→1.092, deck2 프리바티
1.019→1.051). 즉 **지금 캘리브레이션이 너무 느린 재장전 위에서 균형이 맞춰져
있었다** — 가려진 과대 항이 따로 있고, 재장전만 고치면 그것이 드러난다. 다음 작업은
그 과대 항을 찾는 것이며, 재장전 수정은 그와 함께 착륙해야 한다.
...
- 마지막 갱신: 2026-07-31 (**gap #19 해소 — 클립형 재장전. 분할 수는 실측이
  ...
  **실기록 5덱에 클립 유닛이 없다 — 테이블을 비우고 다시 재면 5덱 배수가 표시
  ...
  같이 열린 것: **gap #20** — `closed_form`의 발수가 차지 모션 딜레이를 안 접어
  시뮬과 어긋난다(13슬러그, 센티 41 vs 37). Fienn이 분리 결정.
...
| 16 | **적정사거리(effective range) 보너스** — 배수는 **실측 확정(+0.30)**, 배선만 없음 | 1 (Ade: Agent Bunny) | 미착수 — **모델링 결정 필요**(아래) | 스탯 배선 |
...
| 20 | **덱 탐색 해석 경로가 차지 모션 딜레이를 무시** (`closed_form`과 시뮬의 발수가 다르다) | 13 (`TIMED_CHARGE_MOTION_DELAY` 전원) | 미착수 — 갭 #19와 분리 결정(아래) | 발사 타임라인 |
...
**그런데 배선이 자명하지 않다 — 결정이 필요하다.** "사거리 안인가"는 **포지셔닝**
문제이고 엔진에는 거리 개념이 없다. 그냥 상시 True로 켜면 **전 유닛이 +15%(코어히트
기준)** 를 받는데, 현재 합계가 이미 0.914x라 그건 명백히 과대다. 반대로 지금처럼 상시
False면 사거리 안에서 쏘는 만큼을 통째로 잃는다. 필요한 것은 "실제 플레이에서 샷의
몇 %가 적정사거리인가"이고 그건 **Fienn에게 물어야 하는 값**이다.
...
- **해결:** `user_roster.load_nikke_spec`이 `weapon_stats["reload_time"]`에
  ...
  (덱 시뮬 · `closed_form` 발수 · 차지창 계산기 · `caster_weapon_stats`)이 전부
...
- **미결 — 아핀 재장전 모델과의 상호작용:** 위 ★ 항목의
  ...
  재장전당 한 번인지 갈라야 한다. 센티 실측(클립당 0.5초, 사이클 9.7초)은 클립마다
  붙지 **않는** 쪽과 맞는다. 지금의 곱셈 접기는 현행 곱셈 모델에서만 정확하다.
...
### 20. 덱 탐색의 해석 경로가 차지 모션 딜레이를 무시한다 — 미착수 (2026-07-31)

- **무엇:** `closed_form._shot_count`가 부르는 `attack_rate.generate_shot_times`에는
  **차지 모션 딜레이를 받는 인자가 없다**. 캐시 키도 무기·탄창·재장전·차지시간뿐이다.
  시뮬레이션 경로(`_base_shot_records`)는 `weapon_stats["charge_motion_delay"]`를
  제대로 접으므로, **두 경로가 같은 유닛에 서로 다른 발수를 준다**.
- **규모:** `registry.TIMED_CHARGE_MOTION_DELAY`에 든 **13슬러그**. 센티 기준
  60초에 해석 경로 41발 vs 시뮬 37발(약 11% 과대).
- **왜 아직 안 고쳤나:** 갭 #19(클립 재장전)와 분리하기로 결정(Fienn, 2026-07-31).
  두 수정이 섞이면 어느 쪽이 무엇을 바꿨는지 갈리지 않는다. 이 수정은 **덱 탐색의
  후보 순위를 움직이고**, 캘리브레이션 5덱에도 차지 유닛이 있어 합계 1.010x가
  움직일 수 있다.
- 참고: `backend/app/closed_form.py:176-186` · `backend/app/attack_rate.py:277`.
