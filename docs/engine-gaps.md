...
| **18** | **평타가 Full Burst 보너스를 못 받는다** — `record(...,"normal_attack")`이 `full_burst_bonus_eligible`을 안 넘겨 기본값 False | **전원** (실측 덱에서 평타의 77~89%가 창 안) | **미결 — Fienn 판단 대기**(아래) | 누락된 오픈인 |
...
**닫는 방법은 측정 하나다.** 같은 유닛의 평타 1발을 **풀버스트 창 안과 밖에서** 각각
읽으면 된다(다른 조건 동일, 크리/코어 상태 맞출 것). 비율이 major modifier에 +0.5가
더해진 값이면 확정이다 — 예를 들어 코어·논크리라면 창 밖 대비 창 안이
`(1+1.0+0.5)/(1+1.0) = 1.25`배여야 한다. **그 전까지 엔진은 안 건드렸다.**
