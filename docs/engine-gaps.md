...
1. **실기록 보스의 코어 지름 — 미측정.** 막힌 건 유닛이 아니라 정확도 전체다.
   절대오차 상위 넷(나유타 +0.352B · 리틀머메이드 +0.321B · 리베랄리오 +0.278B ·
   신데렐라:CW-MG +0.238B = **+1.19B, 추적 가능한 과대의 61%**)이 전부 평타 비중이
   큰 유닛이고 무기군이 제각각인데 잔차는 무기군 순으로 정렬돼 있다. `core_diameter_px`가
   opt-in이라 `RECORD_BOSS`는 코어 적격 평타를 전부 p=1.0으로 받는다. **코드는 이미 다
   있고 필요한 건 인게임 측정 하나**다. 아래 「MG 정확도 예열」과 「무기변형 세그먼트
   탄착군」도 이걸 켜야 비로소 문다.
...
코어 50px를 덮고 35발째에 수렴값 10px에 닿는다) — 연사 예열(`attack_rate.MG_SPINUP`)
과 같은 형태의 **정확도** 예열이다. `accuracy.WEAPON_SPREAD_DIAMETER["MG"]`는 수렴값
10px만 담아 이 구간을 모델하지 않는다.
