...
**남은 열린 질문 — 소장품 레벨 0의 의미 (Fienn 확인 필요).** 실계정 9유닛이 레벨
0으로 소장품을 착용하고 있는데, **스탯 쪽은 레벨 0을 '미착용'으로 보고 0을 주고**
(실측 근거: 그 유닛들의 raid400 ATK가 커브값만큼 높지 않다) **스킬 쪽은 사다리
1단을 준다**(`test_level_zero_is_a_real_level_not_an_empty_slot`). 게임 클라이언트
자신도 `atk.at(0)`·`level1.at(0)`를 그대로 읽으므로 번들을 읽어서는 못 가린다.
값 자체는 작지만(1.56~1.58%) 둘 중 하나는 틀렸다. 인게임에서 레벨 0 소장품의
스킬 줄이 활성으로 보이는지 확인하면 닫힌다. **그 전까지 현재 동작을 유지한다.**
