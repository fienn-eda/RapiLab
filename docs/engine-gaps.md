...
**따라서 물리는 조건은 명확하다: 발사 공백이 1.16667초보다 짧을 때만.** 선형 잔존을
가정했을 때:
...
**닫기 전에 아직 재야 할 것 — 부분 잔존의 곡선.** 이번 판독의 공백은 감쇠보다 **길어서**
그 사이 구간을 재지 않았다. 아는 것은 양 끝점뿐이고(공백 ≥ 1.16667초 → 램프 전체,
공백 0 → 램프 없음), 위 표의 선형은 **가정이다**. 재장전 속도를 쌓은 조건에서 한 번
읽으면 갈린다.
