...
- [ ] **신데렐라의 floor 자체가 2.7σ 빠르다 — 미해결로 남김 (2026-08-15).**
      실측 19.583프레임이 floor 20프레임(180발/분)보다 **0.417프레임 빠르고**,
      암시 연사는 **183.8발/분**이다. 지금 반영하지 않는 이유 셋: (1) 0.007초라
      판독 정밀도 근처다, (2) 반영하면 발수가 2.1% 늘어 그녀가 1.095x → 약 1.12x로
      **더 과대**가 된다 — 이미 과대인 유닛을 판독 하나로 밀어올릴 일이 아니다,
      (3) **가를 방법이 있다**: floor를 갖는 다른 넷(아니스: 스타 120 · 리베랄리오
      200 · 라플라스: UH 300 · 네온 300)을 같은 방식으로 재서 **전부 같은 방향으로
      2% 빠른지** 보면 판독 편향인지 그녀 데이터가 틀린 것인지 갈린다.
