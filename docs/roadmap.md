...
- [ ] **To-Do (Fienn 확인 필요): 소장품 레벨 0은 스킬을 주는가?** 엔진이 지금
      ...
      그대로 읽어서 **읽어서는 못 가린다**. 실계정 레벨 0 착용 **31유닛**(인코딩돼
      계산에 드는 것 9유닛)이고 **크기는 작지 않다** — SR 등급은 레벨 0이 이미
      사다리 1단이라 헬름: 아쿠아마린 코어 대미지 **+10.22%**, 볼륨·리터 평타 배율
      **×1.0473**, D: 킬러 와이프 차지 배율 ×1.0474다(볼륨은 실기록 덱4 편성).
      R 등급은 1.56~5.67%. 인게임에서 레벨 0 소장품의 스킬 줄이 활성으로 보이는지
      확인하면 닫힌다 — 걸린 값이 가장 큰 **헬름: 아쿠아마린**으로 보는 게 좋다.
      그 전까지 현재 동작 유지.
