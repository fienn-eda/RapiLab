...
- [ ] **탄창이 빈 상태(재장전 중)에서 카운터가 10에 닿으면 그 3발이 어디로 가나.**
      ...
      탄창이 더 작은 유닛에서는 갈린다.
- [ ] **합계가 1.011x → 1.015x로 멀어진 것을 추적한다.** 흑련의 −0.124B가 다른 유닛의
      과대를 가려주고 있었다. 재장전 공식 아핀 수정(engine-gaps 최상단)이 드러낸 것과
      **같은 종류의 가려진 과대 항**이며, 둘은 같이 봐야 한다.
...
- [ ] **먼저 정할 것: `closed_form`을 고칠 것인가 지울 것인가 (engine-gap #20).**
      `closed_form._shot_count`가 부르는 `generate_shot_times`에는 차지 모션 딜레이
      인자가 없고 캐시 키에도 없어, 시뮬 경로(`_base_shot_records`)와 **같은 유닛에
      다른 발수**를 준다 — `TIMED_CHARGE_MOTION_DELAY` **14슬러그**, 센티 60초 기준
      **48발 vs 37발**(+30%), 흑련 180초 346 vs 189(+83%).
      **단 `closed_form`은 프로덕션 어디에서도 안 쓰인다** — 캐스케이드는
      `surrogate.py`를 쓰고 닫힌 수식 스코어러(옵션 B)는 측정 후 기각됐다. 그래서
      이 수정은 덱 탐색 순위도 캘리브레이션도 **안 움직인다**(2026-07-31 검증).
      - [ ] 삭제를 고른다면 분석 스크립트 둘(`measure_unmodeled_damage_share.py` ·
            `validate_surrogate_recall.py`)의 운명도 같이 정한다 — 기각 근거를
            재생산하는 용도다.
      - [ ] 유지를 고른다면 딜레이를 배선하고 두 경로가 같은 발수를 내는 테스트를
            건다. 캘리브레이션 재측정은 불필요하다(경로가 죽어 있다).
...
- [x] **차지 창 계산기(`차지` 탭 · `POST /api/charge-window`) — 완료 (2026-07-29).**
      ...
      두 타수와 각 확률로 낸다. **알려진 한계 둘**: 재장전 모델 수정 미착륙은
      `docs/engine-gaps.md` 참조. 오버로드 차지속도 줄의 집계 규칙은 미결이라 계산기는
      엔진과 같은 생합계 규칙을 쓰고, 프레임 경계 1.10%p 안이면 경고만 띄운다 — 상세는
