...
- [ ] **②-b 북마클릿을 로컬 POST로 — 착수 가능.** 남은 설계 둘: 북마클릿이 후보
      포트를 어떻게 훑을지, 그리고 **앱이 수신을 어떻게 알아챌지**(현재는
      `postMessage`를 프론트가 받는데 네이티브 창에는 안 닿는다 — 폴링이냐 SSE냐).
      북마클릿은 CSP 때문에 인라인 강제라 **고치면 전 유저가 재설치**해야 한다.
      지금은 유저 0이라 바꾸기 좋은 시점이다.
