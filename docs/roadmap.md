...
- **버스트 게이지가 덱 속성이 됐다 (2026-08-22).** 엔진의 타이밍 축 하나가 통째로
  ...
  - **★ 덱1 실측이 상수를 확증하고 정확도 갭 셋을 열었다 (2026-08-22).** Fienn이
    ...
    **7~13%가 미상으로 남는다**. 그날 F4(아니스 발당 27% 과소)와 「개전이 발사
    ...
    - [ ] 다음: 미상 7~13%를 낼 채움원 찾기. 발사 모델 검증은 **단독편성**으로
      해야 한다(창이 안 열리는 유일한 조건).
    - [ ] **F5 — 아니스의 게이지 충전 속도 6%가 단독편성 9발과 안 맞는다**(10발째가
      된다; 허용 범위 6.20~19.47%). 상한까지 올려도 덱1은 1.20에서 멈추므로
      **미상 채움원과는 별개 항**이다. 가르는 판독: **아니스 단독편성 8발 후
      게이지 px**(6% → 156px · 15% → 169px, 176px 척도).
