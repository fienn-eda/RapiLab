...
### 앨리스 인코딩 + 수동 톡톡이 케이던스 (2026-08-19, Fienn 제보)

- [x] **앨리스 수집·인코딩** — ShiftyPad 정본(rid 191) + lootandwaifus 원문. SR ·
      버스트 3 · Fire · 차지 1.5초 · **풀차지 350%**(전 차지무기 중 최고) · 6발 ·
      재장전 2초. Energizing Carrot(FB진입, top-2 ATK, **캐스터 기준** 차속
      0.17505초 + 차지댐 7%) · Healthy Carrot(`has_pierce` 영구) · Wonderland(자기
      차속 +80.15% / ATK +55.12%, 10초, 버스트 딜 없음).
- [x] **톡톡이를 floor로 표현** — `registry.MANUAL_TAP_FIRE_INTERVAL` 17프레임
      (0.28333초 = FB 10초에 35발). **엔진 산술 무변경**:
      `shot_interval_with_speed`의 `max(잔여차지, floor)`가 이미 「차지가 짧으면
      손이, 길면 차지가 상한」이라, 톡톡이가 그녀 버스트 창 안에서만 걸리는 것이
      저절로 나온다(창 안 35발 / 창 밖 7발).
- [x] **측정 표 둘은 안 건드렸다** — `TIMED_CHARGE_MOTION_DELAY`는 실측 표이고,
      `CHARGE_ROUNDS_PER_MINUTE`는 `audit_rate_of_fire.py`가 `input_type`과 대조해
      **`UP`인 앨리스를 거부한다**. 플레이 스타일 가정은 자기 이름의 상수로 분리.
- [x] **UI 안내문구** — 덱 결과에 「자기 버스트 10초 동안 톡톡이」. 누가 해당되는지는
      엔진이 `tap_fire_slugs`로 실어 보낸다(`hold_burst_slugs`와 같은 계약).
- [x] **35발은 측정이 아니라 판단이다** — Fienn 실측은 자동 23발 / 수동 40발+
      (재장전 0 조건)이고 그 사이에서 고른 값. `docs/measurements/alice-tap-fire.md`.
- [x] 백엔드 **2540 passed / 3 skipped** · 프론트 **942 passed / 73 files** · 타입에러 0.
- [ ] **부분 차지 대미지 규칙 판독 1회** — 무차지 샷이 100%인지 게이지 비례인지
      미측정이고, 그 둘이 앨리스 창 밖 이득을 **1.38배 대 2.09배**로 가른다. 재는 법:
      버스트 창 밖에서 톡톡이 한 발과 풀차지 한 발의 대미지 숫자 하나씩(크리는 같은
      종류끼리 비교). **이 한 판독이 밀크: 블루밍 바니·아인까지 같이 푼다**(둘은
      동일 SR 무기). engine-gaps 「부분 차지 샷」 갭.
- [ ] **앨리스 창 밖 수동 재입력 간격** — 현재 창 밖 발 간격을 차지시간 그대로 두는데
      실제로는 다시 누르는 손의 간격이 얹힌다. `audit_charge_motion_delay.py`가
      앨리스를 `UNVERIFIED`로 계속 띄우는 이유. 창 밖이 전투의 약 72%다.
