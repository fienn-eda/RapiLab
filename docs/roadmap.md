...
- 마지막 갱신: 2026-08-06
- 브랜치: **`wip/soda-full-burst-extension` 진행 중**(소다의 FB 확장 + 조건부 넉,
  트렁크 미병합). 기준선 **백엔드 1935 passed, 3 skipped · 프론트 523 passed** ·
  실기록 캘리 **1.079x · 17/25**.

  캘리는 이 브랜치 전후로 한 자리도 안 움직인다 — 트렁크 `19afe0ac`에서 재봐도
  1.079x다. 이 문서와 `docs/engine-gaps.md`에 오래 적혀 있던 **1.078x는 낡은
  값**이었고, 2026-08-06 재측정으로 정정한다.
