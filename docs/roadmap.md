...
- [ ] **차지속도가 적용될 때만 예측보다 1.3~2.1% 빠르다 — 방향 확정, 모델 미정
      (2026-08-16).** 신데렐라의 2.7σ가 판독 편향인지 가르려고 넷을 쟀고,
      **편향은 기각**됐다. 갈린 축은 판독이 아니라 **차지속도**다:
      ...
      - **차속을 타는 쪽만 빠르다** — 리베랄리오(6.09%) **−1.59%, 6.0σ** ·
        네온(17.03%) **−1.34%, 4.2σ** · 신데렐라 −2.08%. 역산한 실효 차속은
        명목보다 **+1.49%p · +1.11%p** 크다.
      - **고치지 않는 이유:** 데이터점이 둘뿐이라 형태가 안 정해진다 —
        비례(k≈0.984)도 고정 오프셋도 프레임 내림도 **둘 중 하나만** 설명한다.
        **차속 값이 다른 유닛을 몇 더 재야** 갈린다.
