...
- [ ] **Stage 0-a: 큐브 질문** — ShiftyPad 표시 hp/atk/def에 PVE 큐브가 포함인가 별도인가
      (`models.py:38-39`). 오버로드는 물어봐서 해결된 선례가 있다(`overload_effects.py:3-5`).
      **모든 경로의 정확성을 막음** — 스크래핑은 표시값을 통째로 삼키므로.
