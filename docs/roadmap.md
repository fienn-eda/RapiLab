...
- 마지막 갱신: 2026-08-08
- **6인 인코딩 배치 (2026-08-08)**: 돌라 · 델타: 닌자 시프 · 레오나 · 나가 ·
  은화: 택티컬 업그레이드 · 엠마: 택티컬 업그레이드. `ENCODED_SLUGS` 95→**101**.
...
- [x] **게임 데이터의 `accuracy_circle_scale`이 1920×1080 화면 픽셀임을 실측으로
      확정했다.** 사격장에서 잰 조준원 셋(SG 187 · AR 57 · SMG 82 @2560×1440)이
      데이터값 250/75/110에 **원점을 지나는 하나의 비례식으로 1px 이내**에 얹힌다
      (기울기 0.7485 vs 1920/2560, 0.21% 차이). 환산은 `화면px × (가로해상도/1920)`.
