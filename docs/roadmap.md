...
- [x] **스왑 예산이 덱 1에서 다 새고 있었다 — 공평 배분 + 상한 180초, 완료.** 제보:
      ...
      **+3.46%**). `_try_swaps` 호출 전수 추적: **15개 작업 항목 중 앞 5개**(전부 덱 1의
      ...
      `SWAP_TIME_BUDGET_SEC = 180`. 측정은 `--elemental-interrupt` 신설 후
...
- [ ] **⚠ 게이지 충전은 덱 속성인데 상수로 모델링돼 있다 — 기본값 2.0 유지.**
      ...
      **2026-08-05 반증 — 위 근거의 전제가 거꾸로다.** "아니스: 스타 같은 CDR 유닛이
      낀 덱에서는 게이지가 병목이 아니다"라고 적혀 있지만, **아니스: 스타가 들어간**
      실제 추천 덱(아니스: 스타 · 아르카나 · 크라운 · 신데렐라 · 프리바티)의 버스트
      타임라인을 뽑아 보니 **사이클의 절반이 게이지 병목**이었다(간격 12.20/12.52 교대,
      하한 = FB 10.0 + 게이지 2.0 = 12.0). 방향이 반대다 — **CDR이 클수록 쿨다운이
      먼저 해소되어 게이지가 병목이 된다.** 그 덱의 사이클당 CDR은 아니스 7.48초 +
      아르카나 6초 = **13.48초**로, 이 항목이 "알려진 오차" 사례로 든 볼륨(8.21초)보다
      크다. 즉 그런 덱은 2.0이 안전한 쪽이 아니라 **2.0이 가장 크게 틀리는 쪽**이다.
      영향은 개별 유닛 오차가 아니라 **덱 선택**에 이른다: 같은 두 덱을 게이지만 바꿔
      채점하면 CDR 집약 덱은 **-16.6%**(11.29B → 9.41B) 떨어지는데 CDR 없는 덱은
      **-1.0%**만 떨어져, 둘의 격차가 **+6.2% → +26.2%로 4배** 벌어진다. 아르카나의
      "버스트 쿨다운 -6초"는 게이지 2.5에서 총딜을 **자릿수까지 동일하게** 남긴다
      (효과 정확히 0) — 그런데 게이지 2.0에서는 **+13.6%**를 번다.
      상세와 측정법은 `docs/insights.md`.
