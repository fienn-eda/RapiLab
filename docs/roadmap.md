...
- 마지막 갱신: 2026-08-02
- 브랜치: `worktree-cross-tier-swaps`(`wip/scaffolding` `d78a61f` 기준에서 분기 —
  백엔드 **1750 passed, 3 skipped**, 이 항목 시점에 트렁크 미병합). 배분 언덕오르기가
  버스트 티어를 넘어 교환한다 — 상세는 아래 "배분 탐색 품질" To-Do.
...
- [ ] **후보 풀이 CDR 유닛을 못 본다 — 미착수.** 솔린은 `prune_candidate_pool`(11개)
...
- [ ] **`WEAPON_SYNERGY_ANCHORS` 슬러그가 애장품 빌드를 놓친다 — 미착수.** 앵커가
      `"tove"`인데 Fienn의 토브는 애장품이라 `tove-signature` → `if anchor in slugs`가
      **항상 False**, 즉 이 안전장치는 한 번도 작동한 적이 없다. `character_of()`로
      키를 잡아야 한다. 대상이 `burst_tier == 3` 한정인 것도 함께 볼 것(B1 CDR 유닛은
      슬러그를 고쳐도 안 들어온다).
- [ ] **단조성 위반: 로스터를 늘리면 답이 나빠진다 — 미착수.** 같은 보스에서 58유닛
      **41.79B** < 55유닛 **42.62B**(55는 58의 부분집합). 선택지를 더 줬는데 결과가
      나빠지는 건 그리디 필링의 불안정성이고, 교차-티어 수정으로는 안 없어졌다.
- [x] **스왑 예산은 병목이 아니었다 — 측정 완료, 45초 유지.** 신규
...
- [ ] **덱3의 모더니아 좌석 — 미착수, 위 감사가 가리키는 곳.** 여지가 전부
      "모더니아가 덱3에 잘못 있다"로 수렴했고, 덱3은 4.20B로 가장 약한 덱이다.
      이동집합을 넓히는 대신 **버퍼 좌석 규칙(`_BUFFER_SEAT_SLUGS`)이 덱 배치를
      왜곡하는지**를 보는 편이 싸고 표적이다.
...
**미해결:** 앱 실행 몇 초 뒤 검은 화면이 **한 번** 관측됐고 재현되지 않았다.
브라우저로 같은 페이지를 열어 두면 콘솔 에러가 0건이므로 프론트가 주기적으로
터지는 종류는 아니다. 재발하면 `RapiLab.exe --debug` → 창 우클릭 → 개발자 도구.
