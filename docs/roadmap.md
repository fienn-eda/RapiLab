...
- [ ] **스왑 단계가 45초 안에 수렴하지 않는다 — 판단 필요(Fienn).** 교차-티어 도입
      **전에도 이미 데드라인에 걸리고 있었다**(이득 곡선이 t=45.0에서도 상승 중).
      공간이 pass당 616 → 963으로 넓어져 커버리지는 300% → 193%로 떨어졌지만 이득은
      +10.46% → **+11.27%**로 올랐다. 예산을 올릴지(응답 지연 ↔ 품질)는 Fienn 결정.
