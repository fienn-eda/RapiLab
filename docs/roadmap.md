...
- [ ] **mihara-bonding-chain 0.910x — 지속딜이 캡에 포화된 채 17% 부족.** 그녀 딜은
      ...
      **남은 후보 (Fienn 확인 필요):** 원문이 타겟을 계속 **복수형**으로 쓴다
      ("random enemy **units**", "the same enemy **units**", "Mirrors the stack count
      of Ensnaring Chains **on each target**"). 기록의 보스는 **파괴 가능 파츠**를
      가졌으므로, 파츠가 별도 타겟이면 지속딜이 본체 외에 파츠에도 들어간다.
      그렇다면 미달의 방향과 크기가 맞는다.
