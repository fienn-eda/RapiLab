...
- [ ] **덱4 편성과 유닛별 실기록이 저장소에 없다 — 신데렐라 비율을 재현할 수 없다.**
      이전 캘리브레이션 세션들이 대조 하네스를 대화 중에 즉석으로 만들고 버려서,
      "신데렐라 0.61x"를 저장소만으로 되짚을 수 없다. 이번에
      `scripts/measure_deck_breakdown.py`(덱을 슬러그로 지정 → 유닛별·소스별 분해,
      `--record`로 비율까지)를 남겼다. **Fienn에게 덱4 편성 5인과 신데렐라의 기록
      데미지를 받아 픽스처로 못박을 것.**
