...
- [x] **볼륨 CDR 티어는 누적이다 — 확정·반영 (Fienn, 2026-07-27).**
      ...
      11 → **14회**.
- [ ] **⚠ 게이지 충전은 덱 속성인데 상수로 모델링돼 있다 — 기본값 2.0 유지.**
      한때 덱4 실측(**풀버스트 14회, 14번째 2:57** → 게이지 **2.65초**)으로 기본값을
      올렸다가 **되돌렸다**. Fienn 근거: 실제 게이지는 **가한 대미지/발사 수**로 차므로
      편성마다 다르고, **아니스: 스타 · 목단 · 라피: 레드후드 B1(CDR 7.48초)** 같은
      유닛이 낀 대부분의 덱에서는 **게이지가 쿨타임보다 빨리 차서 병목이 아니다**.
      2.0은 "웬만하면 안 걸리는" 값으로 **의도적으로** 고른 것이고, 2.65로 올리면
      그 덱들에 **없는 제약을 발명**한다. 상수 하나로는 덱마다 틀릴 수밖에 없으므로
      제약을 가장 적게 만드는 쪽에 둔다.
      **알려진 오차**: 덱4처럼 CDR이 큰 편성(볼륨 누적 8.21초)에서는 게이지가 실제로
      병목이고, 엔진의 14번째 버스트가 **169초 vs 실측 177초**로 어긋난다.
...
- [ ] **"버스트 N단계 진입 시"를 `own_burst_activate`로 인코딩한 유닛 6기 잔존.**
      ein · maiden-ice-rose · mast-romantic-maid(3단계) · mint · rei-ayanami ·
      snow-white-heavy-arms. `snow_white_heavy_arms.py`가 그 근거를 명시하는데
      **그 근거가 틀렸다** — "버스트 위치 3번 고정이니 매 사이클 발동"은 B3를 2기
      앉히면 깨진다(실측 11사이클 6/5 분할). 신데렐라와 같은 수정이지만 유닛마다
      불릿 단위 확인이 필요하다.
...
- [ ] **To-Do: 스노우화이트 — 회복 시간의 모드별 배분만 남았다.**
      44.1초가 두 모드에 어떻게 갈리는지가 미해결이다: **발당 균일 0.526초**와
      **볼리 길이 비례**(기본 0.394 / Fully Active 1.18)가 둘 다 총량에 맞는다.
      **연속 기본 평타 2발의 간격 하나면 갈린다**(차지 1.2초 + 회복이므로 약 1.6~1.7초
      예상). 그 값 없이 총량에 상수를 맞추지 말 것 — 일반화도 주의다:
      아니스:스타(RL, 0.739x로 이미 미달)에 RL 0.345초를 그냥 더하면 더 나빠진다.
      맞는 수정이 그 밑의 반대 오차를 드러냈다. 그녀의 **발당 모델은 실측 확정**이므로
      남은 과대분은 **발수·가동률·덱 문맥** 층이다. 배제된 후보: 장탄 수(덱4에 그녀에게
      걸리는 차지속도 버프가 없어 0.2초 틱 6회 → 5장전으로 모델과 일치) · 발수(113발/180초,
      SR 6장탄·1.2초·재장전 2초에서 예상 ≈117) · **버스트 3단계 트리거**(다른 아군 B3
      사이클에도 켜지는 것을 Fienn이 인게임 확인, 현행 모델이 맞다).
      이어지는 순서: 아니스:스타 −0.358B · 네온 +0.355B · 아크레인저 +0.272B · 에이드 +0.194B.
      `measure_record_calibration.py`가 이제 절대 오차 상위 10개를 출력한다
      (배수 순위로 고르면 로스터의 가장 작은 기여자만 쫓게 된다 — `docs/insights.md`).
      이어지는 순서: 아니스:스타 −0.408B · 네온 +0.355B · 아크레인저 +0.317B ·
      리틀머메이드 −0.313B · 리베랄리오 −0.302B · 스노우화이트 +0.256B.
      **아스카 잔여 +0.155B(1.055x)의 유력 후보:** Emergency Repair Effect 2
      「탄약 100% 제거」가 미모델링이다. Annihilation(버스트+9초)마다 탄창을
      강제로 비우므로 발수가 준다 — 640발 탄창·재장전 2.14초 기준 거칠게 −3~5%로,
      잔여 오차와 자릿수가 맞는다. 다만 **강제 탄약 소진**은 새 엔진 능력이라
      단독으로 넣지 않았다(Effect 4의 「재장전 속도 +60% 고정」도 함께 필요).
      **감사 방법은 정착됐다:** 유닛의 발당 모델은 **사격장 한 판**이면 계수·코어·크리·
      버프를 전부 분리해 확정된다(같은 구간 안의 **두 계수 비**를 읽으면 ATK·버프가
      상쇄된다). 발당이 맞는데 배수가 틀리면 그건 계수가 아니라 **발수·가동률·
      덱 문맥** 층이다(에이드·아니스가 그 사례).
