...
- [ ] **신데렐라 버스트 불릿1의 계수가 1.2714x 과대다 — 원인 미상.**
      사격장 재현에서 **불릿2(Beautiful 미러)는 0.9952x로 맞는데** 불릿1(10회 순차공격)만
      1.2714x다. 둘은 **같은 offense·같은 `attack_damage_up`** 을 쓰므로 버프 체인은
      전부 검증된 셈이고, 오차는 **계수 하나**에 갇혀 있다 — 실측이 함의하는 히트당 값은
      **1,074.3%**, 스킬 표기는 **1365.92%**(비 0.7865). 확인 필요: 10히트가 모두 같은
      표기값이었는지(첫 히트만 다를 가능성).
      ※ 한때 크라운의 One for All을 범인으로 적었으나 **Fienn 확인으로 철회**했다 —
      "직전에 버스트를 사용한 아군"은 그 사이클의 B1·B2·B3 전원이고 엔진이 맞다.
