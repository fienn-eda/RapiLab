...
- [x] **Scarlet 인게임 실측으로 공식 확정 + 차지시간 대형 오류 발견 (2026-07-20)**
      ...
      느려서 클램프에 안 걸린다.
- [ ] **Liberalio Calm Depths 차지속도 버프 인코딩** — 위 분석으로 값 해석이
      거의 풀렸다("12.74% of the skill user's Charge Speed" ≈ 25.48%p, 기본
      차지속도 200% 가정). 남은 선행 작업 2건: **최저 ATK B3 랭킹**(`top_atk_slugs`의
      거울) 신설, 그리고 **200% 기본값 가정을 Fienn이 확인**. 확인 방법: 다른
      차지속도 버프원(또는 Liberalio 스킬 레벨 변경)으로 2번째 배율을 측정하면
      2배 계수가 상수인지 검증된다.
- [x] **표본 측정 완료 (2026-07-21) — 계통 오류 아님, '검 쓰는 RL' 한정.**
      ...
      **딜 영향은 구조에 따라 정반대**: Raven은 발수 −45%인데 딜 −10.9%뿐(Shock Wave가
      ...
      트리거라 발수에 직결).
- [ ] **차지속도 공식: 남은 미결 — '기본값의 %' vs '절대 초'** (2026-07-21 정정)
      2026-07-20에 `×(1−s)` "확정"이라 적었으나 **과했다**. 현재까지 확실한 것:
      **옛 `÷(1+s)`는 배제**(Cinderella +100%면 20발을 예측하는데 실측 29~30발).
      그러나 남은 두 후보를 **우리 데이터로는 구분할 수 없다** — 버프 크기를 정확히
      아는 유닛(Neon 9.47%, Cinderella 100%)이 **전부 기본 차지 1.0초**라, 거기서는
      `기본×(1−s)`와 `기본 − s초`가 수치적으로 동일하기 때문이다.
      커뮤니티가 말하는 프레임 규칙("차지속도 ~1.67%마다 1프레임 감소")을 양자화해
      대입하면 Neon에 **−0.07프레임**으로 거의 정확히 맞는다(연속 `×(1−s)`는 −0.75,
      `÷(1+s)`는 −0.26프레임). 즉 실제 규칙은 프레임 양자화일 가능성이 높지만,
      **어느 쪽을 기준으로 양자화하는지가 미결**이라 지금 구현하면 근거 없이 한쪽을
      못박는 셈이 된다. 현 구현 `×(1−s)`는 기본 1.0초 유닛에서 1프레임 이내로 맞으므로
      유지한다.
      **판별 실험**: 기본 차지가 1.0초가 **아닌** 유닛에 크기를 아는 차지속도 오버로드를
      달고 재면 즉시 갈린다. 예) Raven(기본 2.03초)에 오버로드 10% → `기본×(1−s)`는
      1.827초, `기본 − s초`는 1.93초 — **6프레임 차이**라 명확히 측정된다.
      부수 미결: Liberalio의 "캐스터 Charge Speed의 12.74%"가 실효 몇 %인지
      (모델에 따라 19.0% 또는 25.9%로 갈림, 어느 쪽도 12.74의 깔끔한 배수가 아님).
