...
- [x] **나유타 풀차지 넉에 FB 보너스 부여 — 완료 (2026-07-26).** 530.46%(=150% +
      ...
      **나유타 0.79x → 0.85x.**
- [ ] **"시전자 ATK의 X%" 버프의 기준: 기본 ATK인가 라이브 ATK인가 — Fienn 확인 필요.**
      나유타 스킬1 2번 불릿은 3초마다(기억 흡수 발동마다) 갱신되고, 갱신이라면 그때의
      **현재 ATK**로 다시 계산돼야 한다. 실측: 덱2에서 **나유타의 라이브 ATK는 기본의
      2.05배**(리틀머메이드 1.99 · 신데렐라 2.37 · 벨벳 1.88 · 프리바티 1.93).
      현재 엔진은 `caster_atk` = 기본 ATK로 **30여 유닛이 공유하는 관례**다.
      민감도: caster ATK ×1.2 → 덱2 0.87x, ×1.4 → 0.90x, ×1.6 → 0.92x.
      **다만 모양이 안 맞는다** — 지금 정확한 벨벳(1.00)·프리바티(0.97)가 ×1.4에서
      1.07·1.03으로 넘어간다. 전면 전환은 로스터 전체를 움직이므로 판정 필요.
