    ...
    **7~13%가 미상으로 남는다**. 그날 F4(아니스 발당 27% 과소)와 「개전이 발사
    ...
    - [ ] **F6 — 라피의 부착형 유탄에 MG값(500)을 주고 있다 (실측 확정,
      ...
      **X ∈ [11,429, 13,333)**로 가뒀다(500이면 84.7%라 한참 모자란다). 덱1
      ...
      - [ ] **고치기: 표현 방법을 정해야 한다.** 스킬 타격의 에너지를 유닛별로
        줄 경로가 엔진에 없다. (A) `gauge_hits` 배수로 25타 세기 — 변경 최소,
        의미 왜곡. (B) `damage_log` 행에 게이지 에너지를 실어 넘기기 — 의미 정확,
        확장 가능, `skill_hits_by_slug` 형식 변경. **B 추천.**
