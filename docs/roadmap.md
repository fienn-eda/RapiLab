...
- [ ] **차지속도 판독이 요구하는 총량이 엔진보다 정확히 +2%p 크다 — 형태 확정,
      ...
      **편향은 기각**됐다. 갈린 축은 판독이 아니라 **차지속도**다:
      ...
      - **남은 것은 구조적 차이 하나 — 브래디만 단독 편성이고 단독만 0%p다.**
        스킬 텍스트에 안 적히는 무언가(풀버스트 자체 등)가 남는지는 이 판독들이
        못 가른다. **Fienn 재측정 예정**이고, `charge-speed-scaling.md`의
        **「재측정 프로토콜」**이 그 축을 가르도록 설계돼 있다(실험 1: 리베랄리오
        단독 = 엔진 85f 대 실측 83f — **제안 전에 엔진에 물어 갈리는 것을 확인함**).
