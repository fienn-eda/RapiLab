...
- [x] **볼륨 CDR 티어는 누적이다 — 확정·반영 (Fienn, 2026-07-27).**
      ...
      11 → **14회**.
- [x] **게이지 충전 시간을 실측으로 고정 — 완료 (2026-07-27).** Fienn 사격장 실측
      (덱4 편성): **풀버스트 14회, 14번째가 2:57**. 누적 CDR이 쿨타임을 앞지르면서
      정상상태 사이클이 `10 + 게이지 + 티어간격`이 되므로 이 두 관측이 게이지를
      고정한다 — **2.65초**(2.90초면 14번째가 180초 밖으로 나가는 타이트한 적합).
      기본값 2.0은 **한 번도 측정된 적 없는 추정치**였다. 덱4 1.53x → **1.50x**.
- [ ] **⚠ 게이지는 사실 덱마다 다르다 — 상수 모델이 이제 큰 근사가 됐다.**
      실제 버스트 게이지는 **가한 대미지/발사 수**로 차므로 편성·무기에 따라 달라진다.
      엔진은 이를 `BossProfile.gauge_charge_time` 상수 하나로 두는데, **inert하던
      시절엔 무해했지만 이제는 사이클 속도를 정하므로 전 유닛 딜에 선형으로 곱해진다.**
      2.65초는 **덱4 한 편성의 실측 1점**이다. 다른 덱에서 풀버스트 횟수를 세면
      편차를 알 수 있다 — 크면 게이지를 덱 속성으로 승격해야 한다.
      (`burst_gauge_fill_speed_percent`(Anis: Star)가 "의도된 미구현"으로 남아 있는
      것도 같은 뿌리다.)
