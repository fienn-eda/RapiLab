...
- [ ] **미해결: Scarlet의 차지속도 하한** — 그녀의 기본 차지가 **0.3초**로 측정된
      하한(0.345초)보다 빠르다. 무조건 하한을 걸면 버프가 없어도 그녀가 느려지는
      회귀라 하한을 `min(기본, FLOOR)`로 뒀는데, 그 결과 **그녀는 차지속도 버프로
      0.3초 아래로 못 내려간다**(보수적 선택). 실제로 더 빨라지는지 인게임 확인 필요.
      두 값이 어긋난다는 사실 자체가 하한이 단일 전역 상수가 아님을 시사한다.
