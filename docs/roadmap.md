...
- **Pattern B 해체 + Hero Vision 인코딩 (2026-08-14, 브랜치
  ...
  **이미 다 있었다** — [[stale-defers-need-the-catalog-not-the-docstring]]의 네 번째 사례.
  ...
  - **`phantom-signature` — 유일하게 남은 진짜 갭.** 대거가 **cap 도달 시 자기를 소비**
    하고 그 소비가 다른 소스의 fill 스케줄을 바꾼다(순환) → 순차 walk 필요. 값어치는
    저 한 자리이고(11.16발/초라 60발=5.38초 vs 5.00초 시계), **명중률 쪽은 지금 0이다**
    — `core_diameter_px`가 opt-in이라 제품 레이드 보스 **0/6**에서 inert.
