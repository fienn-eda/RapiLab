...
- [ ] **★ 밀크의 수동 멈춤을 재야 한다 — 지금 어긋나 있다.** 그녀는 톡톡이 후보(=손으로
      치는 좌석)인데 `optimal_full_charges`에 넘어가는 풀차지 멈춤이 **자동** 값
      22프레임이다. 아인의 수동 환산값 17.643f를 대입해 보면 정상상태 DPS 160.56 →
      **163.04**, 톡톡이 이득 10.76% → **7.74%**다(케이던스 선택 자체는 안 바뀐다 —
      Pierce 창이 매거진당 풀차지 1발을 강제한다). **실측 전까지 건드리지 않았다.**
      함께 정할 것: 수동 멈춤에 **육안 반응시간을 넣을지**(게임 잠금 14.143f 대 실제
      케이던스 재현 17.643f). 톡톡이는 게이지를 볼 필요가 없어 반응시간이 0이므로,
      풀차지에만 잠금값을 주면 비교가 풀차지에 불리해진다.
...
- [x] 검증: 백엔드 **2598 passed / 3 skipped**(기준선 2597에서 +1 — 이 변경이 더한
      회귀 테스트, 값을 수동 판독으로 바꿔 실제로 빨개지는 것 확인) ·
      `scripts/audit_charge_motion_delay.py` **exit 0**(17 timed / 15 stand-in).
