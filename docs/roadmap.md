...
- [x] **홍련(스칼렛: 블랙 섀도우) 조사 완료 — 결함 없음 (2026-07-27).** 절대 오차
      ...
      넉에도 그대로 적용된다.
- [ ] **차지 대미지가 스킬 넉에 붙는가 — 미결, Fienn 판정 필요 (2026-07-27).**
      nikke.gg와 프로젝트 자체 레퍼런스는 **평타 전용**이라고 명시하는데, 적용하면
      홍련 0.62x · 신데렐라 0.61x · liberalio 0.80x로 세 유닛이 동시에 무너진다
      (합계 0.950x → 0.83x). 되돌렸고 `git stash` 태그
      `charge-normal-attack-only-2026-07-27`에 보관. 상세는 `docs/decisions.md`.
- [ ] **홍련의 실기록 6.5B는 기준으로 못 쓸 수 있다 (2026-07-27).**
      2026-06-23 분배 대미지 이슈로 **솔로 레이드가 중단**됐고, 원인은 "홍련·일레그를
      **마스트: 로맨틱 메이드** 같은 분배 버퍼와 함께 쓸 때" 발생하는 최대 **6.8%**
      비정상 상승이었다 — **덱1이 정확히 그 조합이다.** 이어 7월 2일 수정에서 홍련
      스킬1 계수가 **+13% 상향**됐고(250.47/500/750.47 → 283.03/565/848.03),
      수집 데이터는 수정 후 값이다. 옛 계수를 적용하면 현행 모델은 **1.03x**가 된다.
      → 다른 덱의 기록 시점도 같은 이유로 점검이 필요하다.
