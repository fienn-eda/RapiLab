...
- [ ] **blessed 기본 export를 정할 것 (Fienn).** `roster-drafts.json`(모든 스크립트의
      기본값)은 굴림이 없고 48유닛의 ATK가 낮은 **옛 export**다. 같은 계정의
      `roster-drafts-jp-fienn.json`이 굴림 전부 + 더 최신이다. 캘리브레이션이 어느 쪽을
      기준으로 삼을지가 걸려 있다.
