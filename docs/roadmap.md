...
- **버스트 게이지가 덱 속성이 됐다 (2026-08-22).** 엔진의 타이밍 축 하나가 통째로
  ...
  - **★ 덱1 실측이 상수를 확증하고 정확도 갭 셋을 열었다 (2026-08-22).** Fienn이
    ...
    **F1이 주범으로 좁혀졌다 (2026-08-22 후속 분석)** — 천장 논증이 「무기 밖
    채움원이 반드시 있다」를 부등식으로 확정했고, 크기가 별똥별 착탄율과 1.5%
    안에서 같다. **다음 한 수는 코드가 아니라 판독이다**: 버충 구간에 별똥별이
    계속 떨어지는지를 영상에서 확인하면 F1이 확정되고 비행 시간 상수도 같이
    나온다. 재현·천장: `scripts/check_gauge_ceiling.py`.
