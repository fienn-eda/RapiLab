...
- [ ] **차지속도 판독이 요구하는 총량이 엔진보다 정확히 +2%p 크다 — 형태 확정,
      ...
      **편향은 기각**됐다. 갈린 축은 판독이 아니라 **차지속도**다:
      ...
      - **출처 후보 넷이 죽었다.** ① **차속 큐브 — 기각**(Fienn, 2026-08-16: 안 꼈다).
        ...
        **합계 자체가 달라야 한다.**
      - **⑤ 남은 후보: 측정 당시 오버로드가 문서 값이 아니었다.** 독립 증거 —
        **로스터의 네온은 차속 9.47%**(굴림 2.28+2.86+4.33, 2026-08-11 실계정 대조)인데
        측정 문서는 **17.03%**다. 그 사이 오버로드가 움직였고 어느 시점 값이 판독에
        대응하는지가 기록에 없다. **Fienn 확인 필요:** 그 두 판독 때 **인게임 화면의
        차속 표기가 몇 %**였는가. 위 범위 안이면 어긋남은 통째로 사라지고 엔진은
        처음부터 옳았던 것이 된다.
      - **★ 앞으로 차지 판독을 받을 때는 인게임 오버로드 표기값을 같이 받을 것.**
        로스터는 그 사이에 움직이므로 나중에 되짚을 수 없다.
