...
- [ ] **라플라스: 얼티메이트 히어로 — Warm Up 차속이 엔진에 없다 (2026-08-16).
      ⚠ 판독 재측정 대기(2026-08-17).** 차속 0인데 한 구간 안에서 발 간격이
      **56 → 53 → 49 → 47**로 계속 준다(첫 차지는 60프레임 정각 4연속이라 무기
      데이터는 맞다). 전체 −13.44%로 이 조사에서 나온 가장 큰 어긋남이다.
      - **원문이 메커니즘을 확정한다:** skills[0] 2번 불릿 —
        ...
        (실측 빌드 구간 약 **4.4초**).
      - **다만 실측 스택당 실효는 ~5%로 읽힌다**(7.08 / 5.63 / 5.56 / 5.14%),
        원문 10%의 절반이다. 스택당 5% 모델이 실측과 평균 0.8프레임 차이로 맞는다.
        **★ 이는 위 항목(실효가 명목보다 크다)과 방향이 반대**라 하나로 묶으면
        안 된다.
      - 변형 **주기**도 어긋나 보인다 — 차지 시작 간격 평균 **16.1초** 대 엔진
        **12.5초**. 다만 그 주기는 `[Max Ammo Increase]`에 비례하고 이 판독은
        **부계정**이라 단정 불가.
      - **판독이 프레임드랍/부하로 오염됐을 수 있어 Fienn이 재측정한다.** 위
        숫자는 전부 잠정이다. 다만 **판독 편향 기각(위 항목)은 이것이 빠져도
        아니스 ±0.000f와 밀크 +0.61%만으로 성립한다.**
