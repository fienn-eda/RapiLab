...
- [ ] **차지속도 판독이 요구하는 총량이 엔진보다 정확히 +2%p 크다 — 형태 확정,
      ...
      **편향은 기각**됐다. 갈린 축은 판독이 아니라 **차지속도**다:
      ...
      - **유력 후보는 차속 큐브 — 엔진에 없다.** `cube_effects.py`는 렐릭 베어와
        택티컬 베어 둘만 알고 전원 렐릭 베어 Lv15를 가정한다. 차속 큐브는 실재한다
        (arca 175948581 댓글). **대조군이 지탱한다** — 큐브를 명시한 유일한 판독인
        브래디(재장전 큐브 Lv15)만 0%p다. **Fienn 확인 필요:** 그 두 판독에서
        차속 큐브를 꼈는가, Lv15가 몇 %인가.
