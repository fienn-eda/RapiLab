...
- [ ] **⚠ 아래 캘리브레이션 표는 폐기 대상이다 — 잘못된 보스에서 측정됐다.**
      덱별/유닛별 숫자 전부가 Electric 약점 보스 기준이므로, **Electric·Wind 유닛이
      낀 칸은 전부 무효**다(신데렐라·볼륨이 대표적). 사격장 단발 측정들은 표적이
      "우월코드 미적용"이라 **영향 없다**. 재측정하려면 **덱1·2·3·5의 편성과
      유닛별 기록이 필요한데 리포지토리 어디에도 남아 있지 않다** — 이전 세션들이
      대화 중에 만들고 버렸다. Fienn에게 다시 받아 **픽스처로 못박을 것**.
- [ ] **캘리브레이션 현황 (2026-07-27 기준, 세션 종료 시점) — ⚠ 위 항목 참조, 무효.**
