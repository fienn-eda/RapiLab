...
- [x] **볼륨 CDR 티어는 누적이다 — 확정·반영 (Fienn, 2026-07-27).**
      ...
      11 → **14회**.
- [ ] **⚠ 180초 풀버스트 횟수가 검증 안 됐다 — 사이클 모델 전체가 여기 걸려 있다.**
      누적 CDR을 켜자 엔진이 180초에 **14회**를 돌린다(사이클 ~12.9초). 이 수치는
      **모든 유닛의 모든 딜에 선형으로 곱해지므로** 캘리브레이션에서 가장 큰 단일
      레버다. 실기록 대비 덱4가 1.53x인 것과 방향이 맞지 않는다 — 게임에서 실제
      횟수가 더 적다면 사이클 모델 쪽에 캡이 빠져 있다는 뜻이다(게이지 충전이 실제
      병목일 가능성, `gauge_charge_time` 기본 2.0초). **Fienn에게 180초 솔로레이드
      1회의 풀버스트 횟수를 받아 못박을 것** — 세기 쉽고 즉시 갈린다.
