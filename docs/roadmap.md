...
- [ ] **덱2가 유일하게 남은 구조적 이상치 (0.83x).** 세 딜러가 거의 균일하게 낮고
      (리틀 머메이드 0.76 · 나유타 0.79 · 신데렐라 MG 0.84) 두 서포터는 정확하다
      (프리바티 0.97 · 벨벳 1.00). 이 균일함은 유닛별 인코딩 결함이 아니라 **공유
      배수 하나가 빠진 모양**이다. 덱2에만 있는 것은 **벨벳**이고, 차지 게이팅 이후
      그녀의 간판 스쿼드 버프(Charge Damage +100.8%)는 자기 자신과 나유타의 10초
      변형 창 외에는 아무에게도 닿지 않는다. 검증 완료 후보(모두 레버 아님):
      버스트 게이지 충전(±2%) · 신데렐라 스나이프 모드(±1%) · 나유타 변형 창(정상 동작).
      → **Fienn 확인 필요**: 벨벳을 넣은 덱2가 실제로 7.74B를 낸 이유는 무엇인가.
