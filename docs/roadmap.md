...
- [ ] **★★★★ 답 나옴 (2026-08-16): 양자화는 엔진이 맞고 리베랄리오의 데이터
      ...
      - **흑련 차분이 리베랄리오 base를 쟀다** (본계정, 흑련 차속 없음, **편성만 바꾼
        ...
        **`base=88f`만 두 독립 측정을 동시에 만족한다.** grant 단독으로는 87.5±1.4f로
        ...
        같은 형태의 수집 데이터 오류다.
      - **남은 것:** ① **엔진 반영 판단** — 리베랄리오는 실기록 덱에 있고 차지가
        2프레임 짧아지면 딜이 오른다(캘리 방향 확인 후 결정). ② **네온**은 같은 형태로
        base 59f면 0.7σ가 되지만 **grant 같은 독립 측정이 없어 추정으로만** 둔다.
        ③ **신데렐라 −2.09%**는 floor(`rate_of_fire`) 경로라 이 축과 별개.
