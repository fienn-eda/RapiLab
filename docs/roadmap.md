...
- [ ] **Liberalio Calm Depths 차지속도 버프 인코딩** — 위 분석으로 값 해석이
      ...
      2배 계수가 상수인지 검증된다.
- [ ] **다른 RL/SR의 차지시간도 수집 데이터가 맞는지 표본 확인** — Scarlet에서
      2.4배 오류가 나왔으므로 단발 오류인지 계통 오류인지 모른다. 기본 차지 1.0초로
      기록된 RL(neon-vision-eye·raven 등) 하나를 같은 방법으로 재면 판별된다.
      계통 오류라면 29개 차지무기 유닛 전부의 발수가 틀린 것이다.
