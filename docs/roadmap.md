...
- [ ] **차지속도 판독이 요구하는 총량이 엔진보다 정확히 +2%p 크다 — 형태 확정,
      ...
      **편향은 기각**됐다. 갈린 축은 판독이 아니라 **차지속도**다:
      ...
      - **남은 질문: `input_type`이 축인가 판독 방식이 축인가** (표에서 완전히 겹친다 —
        UP 둘은 게이지 관찰, DOWN 셋은 발-발 간격). **실험 A: 리베랄리오의 차지
        게이지를 관찰해 `차지시작 → 발사`를 잰다.** 85f면 차지는 옳고 **겹침 항이
        없는 것**(엔진 갭), 83f면 DOWN_Charge의 차지 계산 자체가 다르다.
