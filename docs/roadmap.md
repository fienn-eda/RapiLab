...
- [x] **게이지 충전이 덱 속성이 됐다 — 해소 (2026-08-22).** 게이지가 보스 상수가
      ...
      **틀렸던 전제 — 지우지 않는다.** 이 항목은 오래 「실제 게이지는 **가한 대미지**로
      ...
      > 은 전부 그 파일 안에서 닫혔다.)
- [x] **게이지 착륙 후 열린 결정 둘 — 둘 다 Fienn이 2026-08-22에 승인했다.**
      ...
      - **② 히스테리시스 — 착륙했고, 오늘 실덱에서는 한 번도 안 걸린다.**
        ...
        다르면 **감쇠하지 않고 경고를 낸다**. 처방으로 적혀 있던 「새 값이 한 격자
        이내면 현재 값 유지」는 **기각됐다** — 고정점을 향해 0.1초씩 걸어 내려가는
        덱이 첫 값에서 얼어붙어 1.0초 오차를 「수렴」이라 답한다.
      - **★ 그 과정에서 이 항목의 전제가 틀렸다는 것이 드러났다.** 「사이클 두 개의
        값이 격자 한 칸 0.1초씩 번갈아 뛴다」가 아니다 — 실제로 재 보니 안 멈추는
        **3덱 전부**가 사이클마다 **격자 4~9칸(0.4~0.9초)** 벌어진 채 번갈아 뛴다.
        그래서 셋 다 감쇠 가드에 걸리고 **감쇠 경로는 0덱**이다(수치도 비용도 착륙
        전과 동일: 397 수렴 / 0 감쇠 / 3 정지, 최대 25패스, 4.59배).
        폭이 큰 이유는 접두사 안정화의 뒷면이다 — 이른 사이클이 0.1초 밀리면 창
        종료·재장전 위상이 밀려 뒤 사이클에서 한 매거진이 통째로 경계를 넘나든다.
        **가드를 넓힐지는 Fienn의 결정으로 남긴다** — 넓히면 그 3덱이 결정적인 답을
        받지만 오차가 격자가 아니라 **0.9초**로 묶여, 이 기능이 사려던 성질을 잃는다.
        숫자를 움직이려고 구현자가 임의로 넓히지는 않았다. 수열 재현은
