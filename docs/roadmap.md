...
- [ ] **볼륨 CDR 티어가 누적인지 대체인지 미확정 — Fienn 확인 필요.**
      원문은 두 불릿 모두 *"Each subsequent effect triggers all effects before
      it"* 인데, 사격장 측정은 **에스컬레이션은 증명하지만**(1회차 = 티어1)
      **3회차가 합인지 대체인지는 못 가른다**(1회차에선 둘이 같다). 현재는 **보수적**인
      대체(2.34 → 2.7 → 3.17)로 두어 기존 정상상태를 유지한다.
      **누적(2.34 → 5.04 → 8.21)으로 켜면 실측 덱4 1.24x → 1.53x, 180초 풀버스트
      11 → 14회.** 큰 레버라 검증 없이 안 넣었다 — `volume.CDR_TIERS_ARE_CUMULATIVE`
      한 줄로 뒤집힌다. **확인 방법**: 3회차 이후 풀버스트에서 표시되는 쿨감 수치, 또는
      180초 레이드의 풀버스트 횟수.
