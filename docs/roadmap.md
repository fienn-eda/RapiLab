...
- [ ] **오버로드 ATK%를 `caster_atk`에 포함할지는 미결.** 오버로드 옵션은 장비 스탯이지만
      엔진은 이것만 전투 시작 Effect로 넣고 `caster_atk`에서는 뺀다(장비·큐브·소장품의
      **플랫** ATK는 `stat_assembly`가 이미 `base_stats`에 넣으므로 이중 계상은 없다).
      실측 오버로드 ATK%: 나유타 +37.4% · 프리바티 +36.1% · 신데렐라 +19.2% ·
      리틀머메이드 +17.3% · 벨벳 +11.8%. **포함시키면 캘리브레이션이 나빠진다** —
      유닛별 평균 절대오차 0.178 → 0.192, 덱별 0.98/0.85/0.98/1.01/1.14 →
      1.04/0.90/1.07/1.01/1.20, 그리고 지금 정확한 유닛들이 전부 1.0을 넘는다
      (벨벳 1.00→1.09 · 크라운 0.96→1.06 · 목단 0.97→1.07). 되돌렸다. → **Fienn 확인 필요.**
- [ ] **덱2가 유일하게 남은 구조적 이상치 (0.84x).** 세 딜러가 거의 균일하게 낮고
      (리틀 머메이드 0.76 · 나유타 0.84 · 신데렐라 MG 0.84) 두 서포터는 정확하다
      (프리바티 0.97 · 벨벳 1.00). 이 균일함은 유닛별 인코딩 결함이 아니라 **공유
      배수 하나가 빠진 모양**이다. **벨벳은 배제됐다 (Fienn, 2026-07-26)**: 그녀의 역할은
      (1) 시전자 기준 ATK 버프 → 전체 강화, 차지 대미지 → 나유타 강화, (2) 파우치
      탄환 소모 → 거품난사 가속이며, **셋 다 이미 모델링돼 있다.** 검증 완료
      후보(모두 레버 아님): 버스트 게이지 충전(±2%) · 신데렐라 스나이프 모드(±1%) ·
      나유타 변형 창(정상 동작). 남은 것은 세 딜러 본인의 인코딩이다.
