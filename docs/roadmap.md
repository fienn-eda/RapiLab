...
- [ ] **M1 마나: Metal σ 재획득 시점 — Fienn 확인 필요.** 원문 "Activates if the
      skill user has cast Burst Skill **before Full Burst ends**"를 재획득이
      ①창 종료 시점이냐 ②캐스트 시점이냐로 읽을 수 있고, **B3를 둘 앉힌 덱에서만**
      갈린다(①이면 그녀가 직접 쏜 사이클엔 버프가 없고 모듈은 정반대를 준다).
      단일 B3 덱에서는 두 해석 모두 현재 코드와 같다. 모듈 docstring의 「티어당
      공격수 한 명」 전제는 사실이 아니다 — `burst_cycle`은 같은 티어 2인을
      번갈아 발사시킨다. **확인 방법: B3 둘인 덱에서 그녀의 Metal σ 버프가 그녀가
      버스트한 사이클에 붙는지, 다음 사이클에 붙는지 한 번 보면 닫힌다.**
