...
- [ ] **볼륨의 Drop the Beat 크리 대미지가 처음부터 최대치다 — 미수정.**
      `volume.py`가 세 에스컬레이션 단계를 **합**(7.74+9.42+11.29 = **28.45%**)해서
      **전투 시작부터** 스쿼드에 건다. 게임은 첫 발동에 **7.74%**(1단계)만 준다 —
      Fienn의 사격장 측정에서 역산한 값과 **소수점까지 일치**한다. 고치면 크리 배율이
      1.8607 → **1.6536**으로 실측과 정확히 맞는다. **스쿼드 버프라 볼륨이 낀 덱
      전원**이 초반 사이클에서 부풀어 있다(덱4가 그렇다). 마스트 취기와 같은 유형이고
      `docs/insights.md`의 "에스컬레이션은 사이클별로" 규칙이 이미 답을 준다.
- [ ] **크라운의 One for All이 풀버스트를 연 B3에게도 ATK 분기를 준다 — 미수정.**
      원문은 *"at the start of Full Burst ... allies who **previously used** their
      Burst Skills"* 인데, 엔진은 `full_burst_enter` 시점의 `burst_used_this_cycle`을
      읽어 **B3 본인까지 포함**한다. B3는 DEF 분기로 가야 한다. 사격장 측정이 네 조합
      중 **유일해**로 이것을 지목했고(맞는 조합이 Last Kingdom의 정확히 36.24%에 떨어짐),
      두 수정(볼륨 + 크라운)을 넣으면 측정값이 **1.0000x로 완전 재현**된다.
      영향: **크라운이 낀 덱의 B3 딜러 전원**(이 측정에서 기본 ATK의 +68%).
      덱4에는 크라운이 없으므로 덱4 과대는 볼륨 쪽이다.
...
- [ ] **캘리브레이션 현황 (2026-07-27 기준, 세션 종료 시점).**
