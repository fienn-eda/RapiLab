...
- [ ] **★ 좁은 풀이 품질을 떠받치고 있다 — 넓히면 −11.32% (2026-08-06 측정).**
      ...
      **이 사실이 바로 위 두 항목과 충돌한다**: 「후보 풀 캡의 상한 = 0」(전 캡이
      ...
      유닛을 못 본다」를 이유로 캡을 넓히면 **조용히 11%를 잃는다**. 캡을 지키는
      테스트는 없다. 먼저 할 일은 값을 못박는 것이 아니라 **캡·K가 함께 움직여야
      한다는 사실을 회귀로 잡는 것**.
- [ ] **단조성 위반의 크기 — 재측정 중 (2026-08-06).** 「앉지 않은 벤치 3명을
      제외해도 답이 바뀐다」를 서로 다른 조합으로 재니 **+0.28% · +0.22%**였다
      (기록된 2.09%보다 훨씬 작다 — 그 쌍의 제외 목록은 어디에도 안 적혀 있어
      재현이 안 된다. **다음부터 측정의 입력을 적을 것**). 세 번째 조합에서
      무기변형 창 겹침 크래시를 밟아 중단됐고, 그 크래시는 이 세션에서 고쳤다.
