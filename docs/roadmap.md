...
- [x] **★ 좁은 풀이 품질을 떠받치고 있다 — 넓히면 −11.32%. 회귀로 막았다
      (2026-08-06).** `test_the_pool_width_and_the_shortlist_width_are_one_setting`이
      `WIDE_TIER_CAPS`와 `DEFAULT_TOP_K`를 **한 쌍으로** 못박고, 실패 메시지가 아래
      스윕 표와 `measure_pool_caps.py` 재실행을 지시한다(뮤테이션 확인: 6/9/18에서
      실패). **캡 3/5/10이 같은 시간에 +1.27%로 현행보다 좋다** — 로스터 하나·보스
      하나라 값은 안 바꿨다. 두 번째 조건에서 재보는 것이 다음 수.
      같은 명령의 캡 스윕: 4/6/12에서 41.056B인데 **6/9/18에서 36.408B(K=20)**,
      8/12/24도 같은 값이다. K=100으로 올리면 둘 다 40.846B(−0.51%)로 회복하므로,
      절벽의 정체는 **대리모델이 넓은 후보를 잘못 순위 매기고 상위 20이 그 오답으로
      채워지는 것**이다. 지금 프로덕션이 좋은 쪽에 앉아 있는 이유는 풀이 좁아서다.
      **이 사실이 바로 위 두 항목과 충돌한다**: 「후보 풀 캡의 상한 = 0」(전 캡이
      같은 값이었다는 2026-08-02 측정)은 더 이상 성립하지 않고, 「후보 풀이 CDR
      유닛을 못 본다」를 이유로 캡을 넓히면 **조용히 11%를 잃는다**. 두 항목을 다시
      읽을 사람이 이 사실을 먼저 만나도록, 값을 못박는 대신 **캡·K가 한 쌍이라는
      것**을 테스트가 들고 있게 했다.
