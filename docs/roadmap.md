...
- [x] **Rei의 590.64% 넉 배선.** 「18발마다, Anti A.T. Field 상태의 타깃에게」.
      ...
      스킬 데이터에 못박는다.
- [x] **「Anti A.T. Field stacks ▲10」은 의도적으로 안 만들었다.** 아스카가 창마다
      **혼자 30 상한에 붙으므로**(위 절) 여유가 0이다. 배선하면 크로스유닛 fill과
      순환 의존(그녀의 fill 시각이 자기가 바꾸는 카운트에 의존)을 사는데 딜은 한 자리도
      안 움직인다. 회귀 테스트가 그 부재를 지킨다.
