...
- [x] **`soda_twinkling_bunny.py`의 `resets: {"trigger": "own_burst", "value": 17}`
      ...
      **+4.97%**(807.9M → 848.0M), 덱 총딜 +0.86%. 예상대로 **과소** 방향이었다.
      ...
      기록 덱 5개에 소다가 없어 **불변**.
- [x] **Beginner's Rewards의 FB 지속 확장(+2/+3초) 재판단 — 결론: 계속 보류,
      다만 이제 근거가 추측이 아니라 궤적이다.** 위 실측에서 그녀는 임계 20을
      **t=141.40에 실제로 가로지른다**(5사이클 +5초 → 2사이클 +2초). 바닥 1 덕분에
      10 아래로는 안 떨어져 「확장 없음」구간은 안 생긴다. 상수 하나로 접을 수 없다는
      뜻이므로 `FULL_BURST_DURATION_DELTA`(슬러그당 고정값)에 그대로 올릴 수 없고,
      **순환**도 그대로다 — 칩은 FB 중 사격으로 차는데 사격은 FB 창이 정해져야
      존재하고, FB 창 길이를 정하는 게 다시 칩이다(확장을 모델링하면 FB 15초 → 창당
      fill 5 → 7로 늘어 이탈 시점이 뒤로 밀린다). 끊는 두 방법은 고정점 반복
      (`evaluate_deck` 2~3회 — 탐색 벽시계 예산 여유가 3.4초뿐이라 위험)과 스케줄러
      안에서 칩 궤적을 재구현(자원 머신 이중 구현 — 이 저장소가 두 번 데인 패턴)인데
      둘 다 대가가 크다. 그녀 혼자를 위해 치를 값은 아니다.
