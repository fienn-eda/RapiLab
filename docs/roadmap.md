...
- [ ] **후보 풀이 CDR 유닛을 못 본다 — 미착수(우선순위 낮아짐).** 위 상한 측정이
      "풀 컷은 이 로스터에서 합계에 영향이 없다"를 보였으므로, 이 항목도 큰 로스터가
      아니면 값이 없을 가능성이 높다. 착수 전에 `measure_pool_caps.py`로 먼저 잴 것.
      원래 근거: 솔린은 `prune_candidate_pool`(11개)
