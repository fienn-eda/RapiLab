...
- [ ] **신데렐라 논크리 절대값이 아직 1.131x다.** 사격장 재현에서 엔진 14,330,988 vs
      실측 12,674,307. **크라운의 스킬 레벨이 미확인**이라 그녀의 `attack_damage_up`
      (엔진 0.3171)과 ATK 버프를 고정하지 못했다. 측정이 함의하는 값은 **0.16482**.
      크라운 레벨을 받으면 바로 갈린다.
