...
- [ ] **17유닛이 1유닛으로 줄었다 — 밀크만 실측 대기 (2026-08-15).**
      ...
      **신규 `scripts/measure_charge_delay_sensitivity.py`가 16개를 닫았다** —
      ...
      - **남은 실측 대상은 `milk-blooming-bunny` 하나(3.07%)**, 그리고 별개로
        `cinderella`(inferred 0, 아무도 그녀를 직접 재지 않았다). audit은 이제 이 둘만
        묻고, 나머지는 「stand-in (accepted)」로 별도 보고하며 **exit 코드에서 빠진다**
        — 항상 빨간 도구는 읽히지 않으므로.
      - 밀크 측정법: 차지 탄이 나가는 순간과 **다음 차지 게이지가 차기 시작하는
        순간**의 간격. 아군 차지속도 버프가 없는 편성, 강제 재장전 구간 밖에서.
        검산은 발-발 간격 = **1.00초(그녀 차지) + 딜레이**.
      ...
      - 백엔드 **2415 → 2421 passed / 3 skipped**.
