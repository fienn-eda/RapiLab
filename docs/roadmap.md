...
- [ ] **라플라스: 얼티메이트 히어로 — 이어 쏠수록 빨라지는데 모델에 없다
      (2026-08-16).** 차속이 0인데 한 구간 안에서 발 간격이 **56 → 53 → 49 → 47**로
      계속 준다(첫 차지는 60프레임 정각이라 무기 데이터는 맞다). 전체 −13.44%로
      이 조사에서 나온 가장 큰 어긋남이다. 그녀 킷이 Warm Up 스택 → 무기 변형
      루프라 스택이 출처로 유력하고, **원문 확인이 먼저다.**
      - 변형 **주기**도 어긋나 보인다 — 차지 시작 간격이 972/908/1024프레임
        = 평균 **16.1초**인데 엔진 모델은 **12.5초**다. 다만 그 주기는
        `[Max Ammo Increase]`에 비례하고 이 판독은 **부계정**이라 단정 불가.
