...
- **버스트 게이지가 덱 속성이 됐다 (2026-08-22).** 엔진의 타이밍 축 하나가 통째로
  ...
  - **★ 덱1 실측이 상수를 확증하고 정확도 갭 셋을 열었다 (2026-08-22).** Fienn이
    ...
    **7~13%가 미상으로 남는다**. 그날 F4(아니스 발당 27% 과소)와 「개전이 발사
    ...
    - [ ] 다음: 미상분을 낼 채움원 찾기. 알려진 것을 전부 되넣으면(브리드·라피의
      창 밖 스킬 타격 + 별똥별 착탄) **24.7% → 4.9~10.8%**로 좁혀지고, 그 폭을
      가르는 것은 **착탄 수가 1~3개 중 어디쯤인가**다 — 사이클마다 세어 평균을
      내면 「거의 닫혔다」와 「10% 남았다」가 갈린다. 발사 모델 검증은
      **단독편성**으로 해야 한다(창이 안 열리는 유일한 조건).
