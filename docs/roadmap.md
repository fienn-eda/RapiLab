...
- 마지막 갱신: 2026-08-22
- **랭킹 시뮬이 고정점까지 안 간다 — 탐색 2.35~2.48배 (2026-08-22).** 탐색 비용은
  (시뮬 수) x (시뮬당 패스 수)인데 `SEARCH_SIM_BUDGET`은 앞 항만 잡고 있었다. 게이지가
  수렴축이 된 뒤로 뒤 항이 비용을 지배한다 — **실제 로스터 400덱에서 덱당 887ms ·
  평균 5.96패스**(중앙값 2, 최대 32)이고, 패스마다 180초 전투를 t=0부터 통째로 다시
  돈다. 분포가 극단적으로 치우쳐 400덱 중 215덱이 2패스에 끝나고 나머지가 상한까지
  퍼진다 — 소수의 덱이 벽시계 대부분을 먹는다. `simulate_raid(max_passes=)`를 더하고
  **덱을 비교하기만 하는 경로만** `RANKING_MAX_PASSES = 3`으로 끊는다
  (`ranking_damage`가 유일한 관문 — 대리모델도 이 함수로 적합되므로 학습과 랭킹이
  같은 척도다). 플레이어에게 나가는 숫자는 예전대로 고정점까지 간다
  (`evaluate_deck_best_seating` · `_report` · `summarize_many`).
  - **3인 이유가 둘이다.** 2 대비 속도를 18% 잃고 오차를 절반 이하로 산다(46유닛
    부분집합: 최대 대미지 오차 17.51% → 2.75%, 최대 순위 이동 20 → 6). 그리고
    **4는 더 안 좋아진다**(2.75% → 4.91%) — 뒤 패스가 고정점을 지나쳤다 돌아오므로
    정확도가 캡에 단조가 아니고, 3은 곡선 위의 점이 아니라 국소 최적이다.
  - **1은 탈락.** 전체 로스터에서 상위 5를 깨뜨린다. 1패스는 언제나 빈 게이지 표로
    채점하고 그 표를 버려서 400덱 중 정확히 채점된 것이 0개다.
  - **좁은 표본은 이 판단을 못 한다** — 46유닛 부분집합에서는 캡 1도 통과했고, 합성
    로스터 30덱은 오차를 3배 과소평가했다(0.189% vs 0.560%). 측정은
    `scripts/measure_ranking_pass_cap.py`(`--exclude`로 시즌 셋 확인, `--end-to-end`로
    추천 편성 대조). **백엔드 2705 passed / 3 skipped.**
  - 남은 지렛대 둘(미착수): **샷 그룹핑** — 대미지가 계수에 정확히 선형이고 한 시뮬의
    21.9만 번 스탯 조회가 서로 다른 번들 **964개**뿐이라, 같은 (번들, 플래그)의 샷은
    계수만 합쳐 한 번에 계산할 수 있다(근사 아님). **evaluate_deck 메모** — 할당 런
    한 판에서 중복 호출이 15.2%였다(무손실).
- **버스트 게이지가 덱 속성이 됐다 (2026-08-22).** 엔진의 타이밍 축 하나가 통째로
  바뀌었다 — 게이지는 이제 보스 상수가 아니라 그 덱의 발사 타임라인에서 나오고,
  `simulate_raid`의 고정점이 **세 번째 수렴축**으로 푼다(게이지 → 사이클 길이 →
  재장전 위치 → 게이지). **무엇이 배선됐는지는 `docs/engine-gaps.md`의
  「버스트 게이지 충전」 행 하나에만 적혀 있다**(채움원·남는 보류 셋 포함).
  결정 넷과 기각한 대안은 `docs/decisions.md`, 교훈은 `docs/insights.md`.
  아래는 그 문서들에 없는 것만 적는다.
  - **비용: 4.68배.** 같은 프로세스·같은 100덱 표본으로 두 팔을 나란히 잰 값이다.

    | | ms/덱 | 패스/덱 | 배수 |
    |---|---|---|---|
    | before 팔 | 169.4 | 1.24 | — |
    | Task 6까지 | 815.4 | 6.12 | 4.89x |
    | **Task 7 이후** | **792.6** | **5.97** | **4.68x** |

    탐색 148초 기준 **약 693초(11.5분)**. **Task 7(스킬 타격 배선)이 오히려 싸게
    만들었다** — 게이지가 빨리 차서 고정점이 덜 돈다. 최대 패스가 18 → 25로 늘었는데
    평균이 준 것은 모순이 아니다(꼬리가 길어지고 몸통이 짧아졌다).
  - **사람의 결정 둘은 2026-08-22에 둘 다 승인됐다** — 비용과 히스테리시스. 아래
    To-Do의 「게이지 착륙 후 열린 결정 둘」 항목에 결과가 있고, **히스테리시스는
    오늘 실덱에서 0번 걸린다**(진동 폭이 격자 한 칸이 아니라 4~9칸이었다).
  - **캘리브레이션 재기준선: 합계 1.051x · ±15% 이내 17/25.** **지표가 아니라 회귀
    감시다**(애니힐리오 기록은 2026-08-17에 채점지표에서 내려갔다 — 아래 절).
    직전 기록값은 **1.075x · 18/25**인데 그것은 **2026-08-16**의 것이고 그 사이
    톡톡이·아인·파츠 파괴 시각 등이 따로 착륙했으므로, **이 차이를 게이지 작업
    하나에 귀속하면 안 된다.** **크게 움직인 덱은 없다** — 덱3 1.166x → **1.157x**
    (크라운 1.393 → 1.357 · 헬름시그 1.332 → 1.327), 아스카 1.171x → **1.175x**로
    네 값 전부 제자리다. 밴드를 벗어난 유닛이 하나 늘었는데(18/25 → 17/25) 가장
    가까운 후보는 경계에 앉은 **미하라 0.850x**다 — **확정이 아니다**(유닛별 직전
    값이 기록돼 있지 않아 어느 유닛이 넘어갔는지 못 가른다).
  - **★ 덱1 실측이 상수를 확증하고 정확도 갭 셋을 열었다 (2026-08-22).** Fienn이
    사격장에서 덱1의 **14사이클 전부**와 버충 구간의 게이지 곡선을 쟀다.
    **확증된 것:** 발당 에너지 상수와 총량 500,000(단독편성 디젤 **14발 예측
    정확히 일치** · 아니스 9발 0.2%) · **아니스의 6% 충전속도 버프가 실제로
    걸린다**(빼면 실측과 5.8% 어긋난다) · 「게이지는 창 밖에서만 찬다」의
    경계(FB 종료 시 0에서 시작). **열린 것:** 채움이 실측보다 **31.5% 느리다** —
    투사체 비행 시간 미모델(F1, 신규 갭) · 아니스의 「0.7초 고정」 창 밖
    누수(F2, 버그) · 정상 상태 발사율 11.5% 과소(F3, F1과 별개 항인지 미확정).
    **주인이 아직 없다 (2026-08-22).** 천장 논증이 「무기 밖 채움원이 반드시
    있다」를 부등식으로 확정했다 — 케이던스 100% + 발당 라이더로도 실측/천장
    **1.19**. Fienn 판독(창 밖 착탄 1~3개)이 F1을 확정했지만 크기는 3.0~8.9%라
    **7~13%가 미상으로 남는다**. 그날 F4(아니스 발당 27% 과소)와 「개전이 발사
    모델을 무죄로 만든다」를 각각 적었다가 **둘 다 철회했다**(추댐은 이미 세어지고
    있었고, 개전 완충은 창 안 누적을 포함한 값이었다).
    - [x] 착탄 수 판독 완료(2026-08-22): `[1,0,0,0,0,2,3,1,1,0,0,1,0,0,0]` →
      평균 **0.60타** = 게이지의 **1.78%**. 「1~3개」를 사이클마다로 읽은 앞선
      추정(4.9~10.8%)은 과대였다.
    - [ ] **F6 — 라피의 부착형 유탄에 MG값(500)을 주고 있다 (실측 확정,
      2026-08-22).** 단독편성 840발의 7번째 유탄 부착 직전/직후가 값을
      **X ∈ [11,429, 13,333)**로 가뒀다(500이면 84.7%라 한참 모자란다). 덱1
      미상이 12.0% → **8.8~9.3%**가 된다.
      - [x] 부착 직전 판독 완료: **174px** → **X ∈ [12,150, 12,623]**, 그 안의
        유일한 RL 실재값은 **12,500**(미상 9.0%).
      - [x] **고쳤다 (B 경로, Fienn 승인, TDD).** `scheduled_nuke` 스펙의
        `gauge_energy` 키 → `record()` → 로그 행 → `_gauge_skill_hits`의 3튜플 →
        `fill_times`. `ATTACHMENT_GAUGE_ENERGY = 12_500`. 덱1 채움 2.692 →
        **2.662초**, 회계상 미상 12.0% → **9.0%**.
      - [ ] **census: 자기 무기와 다른 종류로 발사되는 스킬 타격을 가진 슬러그가
        또 있는가.** 오늘 선언하는 것은 라피 하나뿐이다.
    - **★ 남은 갭은 결과를 바꾼다 (2026-08-22 측정).** 미상 9%가 채움을 밀림
      문턱(2.3~2.4초) 위로 올려 **덱1의 사이클을 거의 전부 밀고 있고**(지금
      10/13 · 4.18초), 실측 채움을 강제하면 밀림이 0이 되며 총딜이 **+1.37%**
      오른다. 실측 원본의 「덱1은 사이클 안 밀림」과 엔진이 정면으로 어긋난다.
      ⚠ 다만 총딜은 **비단조**다 — 2.4초(밀림 2)가 2.0~2.3초(밀림 0)보다 +1%p
      높아, 「빨라질수록 좋다」로 읽으면 안 된다(기전 미조사).
      - [ ] 파급: 자기 무기와 다른 종류로 발사되는 스킬 타격을 가진 슬러그 census.
    - [ ] **다음: 미상 12.0%를 낼 채움원 찾기.** 알려진 것을 전부 되넣은 값이다
      (무기 천장 + 발당 라이더 + 브리드·라피의 창 밖 스킬 + 별똥별 착탄 0.60타 →
      실측/천장 **1.14**). 게임 케이던스로 F2까지 고치면 **18.6%**. F1·F5는
      크기가 닫혔고 F4는 없었으므로 **남은 것은 아직 이름이 없다.** 발사 모델
      검증은 **단독편성**으로 해야 한다(창이 안 열리는 유일한 조건).
    - [x] **F5 — 아니스의 게이지 충전 속도는 6%가 아니라 7.25%다** (측정 완료,
      2026-08-22: 단독편성 8발 후 **158px**). 차이가 1~3%p뿐이라 **덱 결과를 안
      바꾸고**(8%로 놓아도 덱1 1.30 대 1.31) 데이터값 `6`을 판독 한 점으로 덮을
      근거도 안 되므로 **고치지 않고 기록만 한다**. 같은 판독이 「추댐은 이미
      세어진다」(F4 철회)를 독립 확증했다.
    재현·천장: `scripts/check_gauge_ceiling.py`.
    **그런데 사이클 길이와 풀버스트 횟수는 맞는다** — 수동 발동 지연(실측
    0.657초 대 엔진 티어갭 0.20초)이 반대 방향으로 거의 정확히 상쇄하기
    때문이고, 그래서 이 결함이 캘리브레이션에 안 보였다.
    **덱1에 한해 이 브랜치는 상수 시절보다 나빠졌다** — 옛 상수 2.4초가 실측
    2.048초 대비 +17%였는데 새 모델은 2.692초로 **+31%**다. 덱3(판독 대비
    +15.6% → −6.8%)과 덱4·5(이제 정확히 0/15 구속)의 개선이 훨씬 크지만
    **덱1이 진 것은 진 것이다.** 셋의 상태·다시 여는 조건·재현은
    `docs/engine-gaps.md`의 「덱1 실측이 연 셋」, 원본 판독과 유도는
    `docs/measurements/burst-gauge-fill.md`.
- **문서 정정 8건 (2026-08-16).** 「엔진 완성도에 뭐가 남았나」를 세다가 **문서가
  이미 끝난 일을 미착수로 들고 있는 것**을 여덟 찾아 고쳤다(일곱은 이 문서와
  `engine-gaps.md`, 하나는 모듈 독스트링):
  ...
  **7슬러그 열림**에서 멈춰 있던 것(바로 다음 절에서 7이 전부 닫혔다) ·
  ...
  · ⑧ 같은 날 후속: `bready.py` 독스트링의 「엔진이 덱의 버프 종류와 모드 선택을
  잇지 못한다」 → **2026-08-05 `e334f872`로 이미 닫혀 있었다**(`TASTE_INDUCER_SLUGS`
  + `_taste_induced_valid`). census 갭 **89 → 88**. **이 낡은 문장 하나가 「추천이
  인게임에서 재현 안 되는 구멍」이라는 없는 일감을 만들어 냈고, 착수 직전 코드
  확인에서 죽었다** — 갭 목록의 다음 항목에 손대기 전에 **독스트링 말고 코드를**
  볼 것.
  덤으로 **베이스라인 숫자도 정정**됐다: 조건부 3건이 워크트리에서만 skip되므로
  메인 체크아웃 기준선은 **2427 passed / 0 skipped**이지 「2424/3」이 아니다.
  census·캘리 숫자도 실측값으로 갱신했다(**131건 · 갭 88 / 스코프 43** — ⑧ 반영 후,
  **1.050x · 19/25**), 그리고 **갭 88을 「할 일 88」로 읽지 말라**는 분류 주석을
  ...
  **착수 가능한 건 열 남짓**이다).
  ...
  안 고친 문장도 다음 세션에 「아직 안 됐다」로 읽힌다.** 여덟 중 넷은
  **닫은 그날 그 문서의 다른 절만 고치고 요약·백로그·착수순서를 안 고친** 형태이고,
  ⑧은 그 문장이 실제로 **없는 일감을 하나 만들어 낸** 사례다.
...
- **신데렐라 다단히트가 착륙했다 — 고칠 것은 불릿2 하나였고, 불릿1은 건드리지
  ...
  뒤**라 10히트가 전부 창 안인데도 **불릿1은 FB 보너스를 안 받는다**(두 불릿의
  배수 차 1스택 **2.044** · 12스택 **2.085**, 받았다면 1.0이어야 한다) — 즉
  **버스트 대미지는 캐스트 시점에 확정**되고, 히트를 캐스트 시각에 기록하는
  현행이 그 사실 그대로다. 바뀐 것은 불릿2: 1회 넉 → **10틱**(각 틱이 자기 시각의
  아름다움 스택을 읽고 FB 보너스를 받는다), 엔진 확장은 `resource_scaled_nukes`의
  **`fire_delay` 한 필드**뿐이고 없으면 0.0이라 기존 스펙 전부 불변.
  캘리 **1.043x → 1.050x · 19/25 불변**(신데렐라 0.980x → **1.095x**, RL 평균
  1.038x → 1.057x, **나머지 24유닛 미동**), 백엔드 **2411 passed / 3 skipped**.
  원본 `docs/measurements/cinderella-glass-slippers.md`.
- **미하라 0.895x가 닫혔다 — 모델 결함이 아니었다 (2026-08-15).** 같은 날 사격장
  측정 **세 벌**(1인 · 5인 지속딜 · 5인 평타)이 그녀의 **모든 크기 축**을 덮었고
  전부 0.3% 이내로 맞는다: 체인 50.06% · 지속딜 25.08%/스택 · 스택 선형성 10.0000 ·
  **캡 20.0000** · 버프 곱 4.2664 vs 4.2792 · 평타 FB밖 0.998x · FB중 코어 1.003x.
  **남은 것은 시간 축(발수 8,162 · 틱 179)과 실기록의 유닛별 배분 판독뿐**이고,
  덱5가 전체로는 1.013x인데 그녀만 0.895x라 미달과 덱메이트 과대가 상쇄되는 모양이다.
  총딜의 2.8%짜리 유닛이라 **여기서 닫는다.** 이 판독이 확정한 부수 사실 셋:
  - **게임 표기 ATK에는 오버로드 공격력증가가 안 들어 있다**(389,164 × 1.3951 = 542,924).
  - **사격장에는 적정거리 보너스가 없다** — 대조할 때 밴드는 `None`이어야 하고,
    실기록 보스가 `mid`라고 그 설정을 가져오면 1.30x/1.12x로 어긋난다.
  - **창 안의 값은 최빈값을 써야 한다** — 버프가 감쇠해 한 구간에 값이 여럿이라
    `max()`를 뽑으면 「엔진이 1.88배 과대」라는 잘못된 결론이 나온다(내가 밟았다).
  - 기각된 가설 셋(FB밖 틱 고정 · 방출 타이밍 · 버스트 누락)은 `engine-gaps.md`와
    `docs/measurements/mihara-ensnaring-chains.md`에 근거째로 남겼다.
...
- **레이드 회차 보스 가져오기 착륙**: 솔로/유니온 공지에서 읽은 이번 회차 보스를
  ...
  - **그 뒤로 카드가 채우는 값이 다섯으로 늘었다**: 약점 속성 · `range_band`(유니온
    ...
    `spawns_adds`(2026-08-20, 잡몹이 나오면 홀드 파이어 택틱을 기본적으로 뺀다) ·
    `hold_fire_despite_adds`(2026-08-22, 잡몹을 감당하고 홀드하겠다는 플레이어의
    선언 — 위 사실을 판정이 아니라 기본값으로 만든다).
...
- `burst_gauge.py` — 사이클 간격의 나머지 절반. 게이지 채움 시간을 **덱마다** 그
  발사 타임라인에서 계산하고(`fill_times`), `simulate_raid`의 고정점이 사이클 길이·
  재장전 위치와 함께 **세 번째 수렴축**으로 푼다 (2026-08-22).
...
### 마스트 3중첩 버스트 택틱 + 행오버 스턴 잔여 (2026-08-23, Fienn 요청 · 이번 브랜치 범위 밖)

시즌40 추천을 훑다가 Fienn이 「덱2의 마스트+민트가 비효율적으로 보인다」고 짚은 데서
나왔다. 셋은 같은 뿌리(행오버 스턴)를 공유한다.

- [ ] **★★ 「N사이클마다 한 번만 버스트」 형태가 없다 — Fienn의 택틱이 요구한다.**
      (1,2,2) 덱에서 마스트를 **Drunken 3중첩일 때만** 버스트시키는 운영
      (B2-B2-마스트-B2-B2-마스트…). 그녀의 버스트 A Pirate's Romance는 **스택당 ATK**를
      주므로 3중첩에서 쓰는 것이 최대이고, 스턴이 그 풀 버스트 **종료 시점**에 스택을
      지우므로 「직전에 쓴다」가 정확히 최적 타이밍이다.
      오늘 `burst_delay`가 아는 것은 `skip_cycles`(개전 N사이클, **1회성**) ·
      `not_before`(시각) · `min_interval`(유효 쿨다운, 시간 기반)뿐이다. **반복되는
      사이클 카운트 형태가 없다.** `min_interval`로 흉내내면 안 된다 — 사이클 길이는
      덱의 쿨감에 따라 변하고, 그 모듈 독스트링이 이미 「사이클 수 형태와 시간 형태는
      서로 대체 불가」라고 못박아 두었다.
      **택틱이지 유닛 속성이 아니다** — `hold_fire`·`max_bursts`와 같은 범주라
      레지스트리에서 오면 안 되고, 후보를 내는 게이트가 필요하다: 덱에 마스트가 있고 ·
      **앵커가 없고**(있으면 3중첩을 늘 유지하므로 택틱이 무의미) · 티어메이트가 있어
      그녀가 거르는 사이클을 커버할 수 있을 것.
- [ ] **★★★ 스턴이 스쿼드 버프를 안 끄는 것을 고친다 — 측정된 크기 5.94%.**
      A Pirate's Heart의 스쿼드 crit_rate + flat_atk이 `duration=None`으로 전투 내내
      걸린다. 근거로 적힌 *"the brief solo post-stun gap is ignored"* 가 안 맞는다:
      스턴이 스택을 전부 지우고 그 구간은 **전투의 22.2%**(10초 x 4회)다.
      반사실 측정(시즌40 덱2, `docs/engine-gaps.md` 자기 스턴 항목 잔여 3):
      그 지속 버프의 총 값어치가 **덱 총딜의 25.30%**이고, 지속률로 선형 보정하면
      **5.94%**가 사라진다. **부호가 뒤집힌다** — 민트 덱이 앵커 덱보다 −2.57% 앞서던
      것이 보정 후 **+3.58%로 뒤진다.** 즉 이 근사가 Fienn의 「마스트는 앵커와 가야
      한다」를 가리고 있었다. (보정은 선형이라 크기는 추정, 방향은 확정.)
- [ ] **★ 스턴이 평타를 안 멈춘다.** `self_stun`은 `burst_cycle`에만 닿고
      `attack_rate`·`raid_simulator`는 그 필드를 안 읽는다. 같은 덱에서 덱 총딜의
      **약 1.00%**가 기절 중 평타로 실린다. 위 항목과 같은 방향(마스트 과대평가)이다.

### 아인 실측 — 차지대미지의 형태와 자동/수동 멈춤 (2026-08-20, Fienn 실측)

`docs/measurements/ein-tap-fire.md`. 아인의 톡톡이 가부를 묻다가 시작해서, 정작
**차지 대미지 공식의 형태**와 **멈춤 표가 무엇을 재고 있었는지**를 정했다.

- [x] **★★ 차지 대미지는 게이지에 가산된다 — 형태 확정.** 아인의 Feather Shot(lv7)이
      자기에게 Charge Damage +69.09%를 걸어 풀차지 게이지가 **274% → 343%**가 된다.
      가산 예측 343.09 ✅ / 승산 예측 394.2 ❌(51포인트 어긋남). 브래디 판독이
      「두 점으로는 형태가 안 갈린다, 차지대미지 0% 세팅이 한 벌 더 필요」로 남긴
      구멍을 닫았다 — **필요했던 건 0% 세팅이 아니라 더 큰 지렛대였다**(브래디의
      3.5포인트 대 아인의 69포인트). `docs/measurements/bready-charge-damage.md` 갱신.
- [x] **톡톡이는 Full Charge 트리거를 주지 않는다 — 실측 확인.** 풀차지 → 톡톡이 →
      풀차지에서 3발째 게이지가 343%가 아니라 **274%로 복귀**한다. 엔진의 가정이 맞았다.
      `build_ein_per_shot_rules` docstring에 「후보로 켜면 같은 변경에서 게이팅해야
      한다」를 못박음(지금은 후보가 아니라 코드는 그대로 — YAGNI).
- [x] **아인 모션 딜레이 = 22프레임(자동사격), stand-in에서 실측으로.**
      `TIMED_CHARGE_MOTION_DELAY["ein"]`에 등록하고
      `STAND_IN_ACCEPTED_CHARGE_MOTION_DELAY`에서 제거. **값이 같아 딜은 한 자리도 안
      움직인다**(밀크가 2026-08-15에 지난 자리와 같다). stand-in 22프레임의 **네 번째**
      독립 확인이다(브래디·센티·밀크·아인).
- [x] **★★ 멈춤 표가 「자동사격 표」라는 것을 명시.** 전투 중 플레이어는 5명 중 1명만
      손으로 치므로 자동이 옳은 기본값이고, **수동 판독은 톡톡이 후보에게만** 들어간다
      (Fienn의 규칙). 아인이 같은 계정에서 둘 다 재서 **8.4프레임** 차이를 보였다
      (자동 22.524f / 수동 14.143f). 이것이 「밀크의 16시그마 반례」의 정체다 — 유닛별
      사실이 아니라 자동 대 수동이었고, **앨리스의 일치는 우연이 아니라 그녀만 수동으로
      쟀기 때문**이다(그녀는 톡톡이 후보다). 레지스트리 세 곳의 서술을 정정.
- [x] **표에 넣을 값은 실측 딜레이가 아니라 `발간격 − 파일차지`다.** 판독은 딜레이를
      **실측** 차지와 짝짓는데 엔진은 **파일** 차지와 짝짓는다. 자동에선 0.57프레임이라
      안 보이고 수동에선 3.5프레임 벌어진다(육안 확인 반응시간).
- [x] **낡은 문구 정정** — `TAP_FIRE_CANDIDATES` 주석이 아직 「톡톡이 샷의 배율은
      **정확히 100%**」라고 적고 있었다(103% 정정 때 안 따라온 문구).
- [x] **아인은 재고도 톡톡이 후보로 켜지 않는다.** 그녀의 풀차지는 **스스로 사슬을 잇고**
      톡톡이 한 발이 그걸 끊는다 — 사슬 유지 시 풀차지 +4.1%, 버스트 창 +44.8%, 버프
      없는 상태는 전투 첫 발에만 존재. 게다가 판정이 장탄에 극도로 민감해
      (뒤집힘 지점 6발 30.19f · 8발 21.14f · 10발 15.70f) 계정별로 갈린다.
- [x] **★ 밀크가 자동 멈춤을 쓰고 있었다 — 수동 값으로 옮겼다 (Fienn 결정, 2026-08-20).**
      그녀는 톡톡이 후보(=손으로 치는 좌석)인데 `optimal_full_charges`에 넘어가는 풀차지
      멈춤이 **자동** 22프레임이었다. 수동 멈춤 실측이 없으므로 아인의 수동 환산값
      **17.643f**를 새 표 `registry.TAP_FIRE_MANUAL_MOTION_DELAY`로 옮겼다 — 이 표는
      `_CHARGE_MOTION_DELAY` 병합의 **마지막**에 와서 자동 실측을 이긴다(조작이 틀린
      측정보다 조작이 맞는 값이 낫다). 결과: 정상상태 DPS **160.5569 → 163.0417**,
      톡톡이 이득 **10.76% → 7.74%**, 창 초과 9개 조합 전부 0건(최악 간격 5.76s →
      5.69s로 여유가 늘었다). **케이던스 선택은 안 바뀐다** — Pierce 창이 매거진당
      풀차지 1발을 강제한다. 그녀의 자동 판독 22프레임은 `TIMED_CHARGE_MOTION_DELAY`에
      **그대로 남는다**(stand-in 22프레임의 네 근거 중 하나이고, 틀린 측정이 아니라 다른
      조작의 측정이다).
      - **14.143f가 아니라 17.643f인 이유**: 엔진이 `간격 = 파일차지 + 딜레이`로 **파일**
        차지와 짝짓기 때문이고, 그 3.5프레임이 **풀차지 도달을 눈으로 확인하는
        반응시간**이다. 톡톡이엔 그 항이 없어 실측 15f를 그대로 쓰므로, 풀차지에만
        잠금값을 주면 **양쪽이 다른 손을 가정**하게 되어 비교가 풀차지에 불리해진다.
- [x] **검산 스크립트가 멈춤을 하드코딩하고 있었다** — `audit_milk_tap_fire.py`가
      `charge_motion_delay: 22 * FRAME_SECONDS`를 직접 들고 있어 레지스트리가 바뀌어도
      **옛 값으로 계속 초록을 찍었다**. 멈춤·톡톡이 간격·창을 전부 레지스트리에 묻게
      고쳤다([[measurement-scripts-must-ask-the-module]]의 재발).
- [ ] **밀크의 수동 멈춤 실측** — 지금 값은 아인에서 옮겨 온 stand-in이다. M3/N1과 같은
      프로토콜, 조작만 수동. 재면 `TAP_FIRE_MANUAL_MOTION_DELAY`에서 그녀가 빠진다.
- [ ] **앨리스의 값도 같은 검사를 통과하는가.** 그녀의 15프레임은 수동 실측이라 조작은
      맞지만 「실측 딜레이가 아니라 `발간격 − 파일차지`」 규칙과 어긋난다(101.73 − 90 =
      11.73f). 실측 차지 87.92f가 파일 90f보다 **작은** 것도 수동 반응시간과 방향이
      반대다 — 그 판독 때 차지속도 오버로드가 켜져 있었는지에 답이 걸려 있다(켜져
      있었다면 파일차지 82f · 반응시간 5.92f · 재현용 딜레이 19.73f). 지금은 안 건드렸다.
- [ ] **`tap_fire_slugs`가 덱 안 후보 **전원**을 톡톡이로 계산한다.** 앨리스와 밀크가
      같은 덱에 앉으면 둘 다 손으로 치는 것이 되는데, 5명 중 1명 규칙과 어긋난다.
      지금 후보가 둘뿐이라 드물게만 닿지만 규칙이 명문화된 이상 판단이 필요하다.
- [x] **감사가 「TIMED 0.29」로 거짓말하고 있었다** — 상태는 실측이라 하면서 값은 빌려온
      것을 찍었다. `status_for`에 **`manual stand-in`** 상태를 넣고(실측을 덮을 수 있는
      유일한 상태라 TIMED보다 **먼저** 본다) 왜 빌렸는지·무엇이 움직이는지를 출력에
      적었다. **exit 코드는 안 건드렸다** — 빨간불로 만들지는 Fienn 판단이다.
- [x] 검증: 백엔드 **2600 passed / 3 skipped**(기준선 2597에서 +3 — 이 변경이 더한
      회귀 테스트 셋, 값을 되돌려 **실제로 빨개지는 것까지 확인**) ·
...
- [ ] **보스 이미지 필드 — 여전히 백로그**(Fienn 판단으로 `guide`와 분리,
      ...
      커밋하는 방식이어야 한다.
- [x] **가이드 데이터가 처음으로 실렸다 — 사치스러운 거미 10줄**(2026-08-20,
      Fienn 판독, 커밋 `e423fe42`). `data/raid-rotations.json`의 `guide`에
      알집 생성 3 · 타수기믹 잡몹 소환 3 · M자/검은화면/오각형 3회/삼각형
      5회 저지 4를 적었다 — **릴리즈 앱에 담겨 나가는 첫 가이드.** 검증:
      프론트 **1030 passed / 80 files**, 백엔드 **2607 passed / 3 skipped**,
      타입 0, lint 에러 0, **릴리즈와 같은 빌드**(`scripts/build_app.py`) +
      `--selftest` 통과, 번들 안 `dist/RapiLab/_internal/data/raid-rotations.json`이
      가이드 10줄과 아래 새 파괴 시각을 값으로 담고 있음을 확인. 착륙과
      `v*.*.*` 태그는 Fienn이 직접 한다.
- [x] **파츠 파괴 시각을 재판독 — `[1, 61, 126]` → `[3, 63, 128]`초**
      (2026-08-20, Fienn, 커밋 `46f720a1`). 가이드 이벤트(알집 생성
      2·62·127초)를 세로 타임라인 같은 축에 채우다가 파괴가 생성보다 먼저
      오는 순서 모순이 드러났다 — 옛 판독은 2초 일렀다. `part_destroyed`
      트리거가 읽는 엔진 입력이므로 덱 추천 계산에 영향. 상세:
      `docs/decisions.md`·`docs/insights.md`.
- [ ] **백로그 — 레이븐·디젤 윈터 스위츠의 파츠 파괴 딜 퍼센트 재측정**
      (Fienn 판단으로 보류, 2026-08-20). `backend/app/skill_rules/raven.py`의
      Single Point Attack(+1.36%)과 `diesel_winter_sweets.py`의 지속댐
      (-2.91%)은 둘 다 옛 파괴 시각 `[1, 61, 126]`으로 잰 값이고, 새 시각
      `[3, 63, 128]`로는 재측정하지 않았다(독스트링에 명시). 창 셋의 총합
      45s와 전투 180초 안에 들어간다는 사실은 시각이 2초 밀려도 그대로지만,
      창이 밀리면 딜 분포와 겹치는 구간이 달라져 퍼센트는 움직일 수 있다.
...
- [x] **사고 ① — Mark-of-the-Web: 받아서 푼 `RapiLab.exe`가 실행 즉시 크래시.**
      브라우저로 받은 zip에 붙는 Zone.Identifier(인터넷 표시)를 **탐색기의
      압축 풀기가 안의 파일 전부에 전파**(실측 364개 중 363개)하고, .NET
      Framework의 어셈블리 로더가 그렇게 표시된 DLL을 거부해 pythonnet의
      `Python.Runtime.dll`을 못 읽는다. 빌드 산출물 차이·의존성 드리프트·
      .NET 런타임 부재는 전부 sha256 대조·버전 확인으로 배제됐다. 고침:
      `desktop.unblock_bundle()`이 `main()` 맨 앞(.NET을 건드리기 전)에서
      번들 아래 Zone.Identifier를 스스로 지운다. 상세: `docs/decisions.md`·
      `docs/insights.md`.
- [x] **사고 ② — `--selftest`가 창을 띄우는 경로를 아예 안 보고 있었다.**
      서버·번들 데이터·앱 셸·프로세스 풀 넷은 전부 초록인데, 창을 띄우는
      .NET 적재 경로는 다섯 번째로 안 보고 있어서 v0.1.2가 그대로 나갔다.
      `import clr`로 창은 안 열고 적재만 확인하는 ⑤ 추가(`gui backend: ok`).
      부수: `clr`이 직접 import가 되어 `pythonnet`을 `requirements-app.txt`에
      직접 선언 + `IMPORT_TO_PACKAGE`에 매핑 추가(전이 의존 → 직접 의존).
      상세: `docs/decisions.md`.
- [x] **사고 ③ — `v0.1.2` 태그가 이 브랜치 착륙 전 커밋(`6e858b38`)을
      가리켰다(기록만, 코드 변경 없음).** 착륙 자체는 정상(`1d8b2be5`)이었지만
      태그만 앞서 있어, 배포된 앱이 옛 `part_destruction_times: [1, 61, 126]`을
      내보내고 `guide` 키가 아예 없었다. 확인법:
      `git merge-base --is-ancestor <브랜치 마지막 커밋> <태그>`. 다음 태그부터
      **`origin/wip/scaffolding`의 최신 커밋**에만 찍는다. 상세: `docs/insights.md`.
- [x] **검증**: 빌드본 사본에 표시 740개 전부를 붙이고 exe만 풀어 사고 ①을
      재현 → 정상 실행 확인, 표시를 붙인 채 `--selftest`도 통과. 백엔드
      **2615 passed / 3 skipped**(MOTW 테스트 5개 추가). 엔진 능력은 안
      건드렸다(`engine-capabilities.md` 변경 없음 — 확인만).
- **릴리즈 절차 갱신(이 사고로 확정):** (1) 태그는 항상 착륙이 끝난 뒤
  `origin/wip/scaffolding`의 최신 커밋에 찍는다 — 로컬 브랜치나 미착륙
  커밋에 찍지 않는다. (2) 태그를 민 뒤에는 **실제로 GitHub 릴리스 zip을
  받아서 실행**해 값까지 확인한다(로컬 빌드 검증만으로는 안 된다 — CI가
  빌드하는 커밋과 로컬에서 빌드한 커밋이 다를 수 있다). 위 "가이드 데이터가
  처음으로 실렸다" 항목에서 Fienn이 남긴 **배포본(로컬 빌드 + `--selftest` +
  번들 데이터 값 확인)** 단계가 애초에 이 관행의 시작이었고, 이번 사고
  둘(Mark-of-the-Web 크래시·낡은 태그)을 실제로 잡은 것도 "받아서 실행해
  본다"는 바로 그 습관이었다 — 둘 다 자동화된 테스트나 CI로는 안 잡혔다.

### 톡톡이 배율 정정 — 103% + 차지대미지 게이팅 (2026-08-20, Fienn 실측)

- [x] **톡톡이 배율 100% → 103%** — 게이지 100%는 「차지 안 함」 판정이라 발사
      자체가 안 나간다. 발사에 필요한 관측된 최소 게이지는 103%다(브래디를 차지
      대미지 두 세팅으로 잰 판독, 두 세팅 모두 최소 게이지 103% —
      `docs/measurements/bready-charge-damage.md`). `attack_rate.
      TAP_FIRE_CHARGE_BONUS = 0.03`.
- [x] **톡톡이 샷은 차지 대미지 버프를 안 받는다** — `raid_simulator`의 기존
      주석("Charge Damage multiplies a fully-charged SHOT and nothing else")을
      코드가 이제 따른다. 같은 브래디 판독이 근거: 차지 대미지를 7.59%→11.11%로
      올려도 같은 게이지 샷의 대미지는 +0.06%만 달라졌다(직접 곱해진다면
      +3.27%여야 한다).
- [x] **풀차지/톡톡이 판별을 값에서 플래그로** — `ShotRecord.is_tap_fire`.
      배율이 0보다 커진 지금 값(`extra_charge_bonus > 0`)으로 판별하면 톡톡이가
      전부 풀차지로 잡힌다.
- [x] **검산 스크립트가 이 버그를 그대로 갖고 있었다** — `audit_milk_tap_fire.py`의
      「풀차지」 집계가 여전히 `extra_charge_bonus > 0`이라 배율 인상 뒤 모든
      톡톡이 샷을 풀차지로 셌다. `is_tap_fire`로 고치고 톡톡이배율 출력 칸을
      더함 — 실행값 **1.030**, 「창 초과」 0건(9개 장탄/재장전 조합 전부).
- [x] **★★ 두 번째 버그: 같은 스크립트가 이득 비율을 짧은 창(30초)으로도 재고
      있었다** — 리뷰에서 지적됨. 옛 모델(톡톡이 배율 0%)로 창 길이를 30/60/180초로
      스윕하면 각각 5.56%/8.33%/9.62%로 아직 수렴 전이다(정상상태가 아니다). 600초
      창도 확인해 보니 **참값이 아니었다** — 600·900·1200·2400초에서 우연히 같은
      10.79%를 내 수렴한 것처럼
      보였지만, 450초·4800초·120,000초·1,000,000초까지 스윕하면 10.76% 근처에서
      진동하며 수렴한다. 근본 수정: 이 무기는 버프가 전투 내내 상수라 케이던스가 **첫
      매거진부터 이미 완전히 주기적**이다(9개 장탄/재장전 조합 전부 확인) — 그래서
      창을 스윕할 필요 없이 **매거진 한 주기의 대미지 ÷ 그 주기의 길이**가 정확한
      정상상태 DPS다. `audit_milk_tap_fire.py`에 `steady_state_dps()`로 반영,
      「창위반검사」(30초, Pierce 창 초과 스캔용)와 「정상상태」(매거진 주기 대조,
      이득 비율용) 출력을 라벨로 분리.
- [x] **밀크 딜 이득 재계산(정상상태 방식, 두 번째 정정): +8.6% → +10.76%**(스톡
      장탄 6·재장전 2초, 정상상태 DPS 대조 — 매거진 주기, 창 길이 무관, 고친
      `audit_milk_tap_fire.py` 실측). 같은 정상상태 방식으로 잰 옛 모델(톡톡이
      배율 0%)도 **+8.59%**로 옛 문서값과 거의 일치해 대조가 사과 대 사과임을
      확인했다. **이득은 낮아진 게 아니라 올랐다** — 이 대조엔 애초에 차지대미지
      게이팅이 안 걸린다(밀크 단독 실측이라 외부 차지대미지 버프가 없다, 게이팅은
      `raid_simulator`의 버프 경로에서만 작동), 톡톡이 배율 인상(+3%)만 이 수치를
      움직인다.
- [x] **앨리스 측정 문서·인코딩의 「정확히 100%」 결론을 정정** —
      `docs/measurements/alice-tap-fire.md`의 「톡톡이 샷은 정확히 100%」 절,
      `backend/app/skill_rules/alice.py`의 「The frame a tap does bank.」
      보류 항목(3-7% understatement로 적혀 있었다)을 닫음.
- [x] `docs/engine-gaps.md` · `docs/encoded-nikkes.md` ·
      `.claude/skills/nikke-skill-encoding/references/engine-capabilities.md`
      갱신.
- [x] 검증: 백엔드 **2571 passed / 3 skipped**(옛 기준선 2565에서 +6 — 이
      정정이 더한 테스트) · 프론트 **949 passed / 74 files** · 타입에러 0
      (프론트·타입은 이 정정으로 변한 것 없음 — 기존 기준선 그대로).
...
- [x] **색·굵기·기울기는 샘플을 보고 골랐다** — `--text` → `--accent` 가로
      그라디언트, 굵기 **+40 units**, 기울기 **10°**(배포본 Oblique). 굵기가
      합성인 이유는 이 글자꼴에 굵은 웨이트가 없어서다(두 파일 다 weightClass
      400). **+90쯤에서 속공간이 메워지고**, 속 빈 변형은 헤더 크기에서 획이
      0.47px라 못 쓴다. ⚠ 그라디언트는 `index.css`의 「빨강은 드물게」 규칙을
      의도적으로 깬 것이고 **그 주석에 예외를 명시해 뒀다** — 안 적으면 다음
      세션이 되돌린다. 근거는 `docs/decisions.md`.
- [x] **크기는 교체 전과 맞췄다.** viewBox가 잉크에 딱 맞아 `height:1em`이면 글자
      보다 커진다. 예전 제목의 대문자가 20.0px였고, 덧댄 획 때문에 글자의 바깥
      위끝이 `675 + 획/2 = 695`라 **0.984em**에서 같아진다 — 브라우저에서 재서
      고른 값이다. **`--weight`를 바꾸면 이 값도 다시 뽑아야 한다.** 제목 자체를
      키우려면 `.app__title`의 `font-size`를 만질 것.
...
### 밀크: 블루밍 바니 톡톡이 — 매거진 안 혼합 케이던스 (2026-08-19/20)

- [x] **톡톡이 간격을 멈춤에서 분리** — `registry.TAP_FIRE_INTERVAL`. 앨리스에서
      「톡톡이 간격 = 멈춤」으로 읽혔던 건 우연이었다: 밀크의 멈춤은 21.889f(n=9)인데
      톡톡이 간격은 14.810f(n=21)로 약 16시그마 다르다. 실측 없는 유닛은 여전히
      멈춤으로 대신한다(`get_tap_fire_interval`이 `None`이면 `motion_delay`).
- [x] **매거진 안에서 풀차지 `k`발 + 톡톡이 `C-k`발을 섞는다** — `attack_rate.
      optimal_full_charges`. `tap_fire_wins`의 단조성 논증을 k축으로 확장 — 효율이
      `(p+ak)/(q+bk)` 꼴이라 답은 언제나 구간 끝, 탐색은 `k=0..C` 훑기(비용 없음).
      `full_charge_positions`가 균등 배치(첫 탄 항상 풀차지), `mixed_charge_round_offset`가
      누적 타임라인을 만든다.
- [x] **풀차지가 되살리는 창** — `registry.FULL_CHARGE_WINDOW`(밀크 6초, Pierce).
      `optimal_full_charges`가 연속 두 풀차지 사이 최악 간격이 창을 넘는 `k`를
      제외하고 남은 것 중 최고 효율을 고른다. 어떤 `k`도 못 지키면 `k=C`로
      물러난다 — 창을 가장 덜 잃는 배치이지 「지킨다」는 보장이 아니다.
- [x] **Pierce 영구 부여의 근거를 케이던스로 이전** — 「그녀는 SR라 매 샷이 풀차지」는
      톡톡이 등장으로 거짓이 됐다. 새 근거는 「그녀의 실제 수치(장탄 6·재장전 2초,
      강제재장 3초 포함)가 창을 못 지키는 분기에 안 닿는다」는 부등식. 밀크를
      `TAP_FIRE_CANDIDATES`에 등록.
- [x] **밀크 딜 약 +8.6%**(매거진 케이던스 축, 스톡 장탄 6·재장전 2초 — 등록 전
      `k=C` 고정 대비, 전투 600초 정상상태·엔진 실측 `audit_milk_tap_fire.py` 기준).
      Pierce 자체는 안 끊긴다(신규 테스트 2건이 못박는다). **정정(2026-08-20):
      톡톡이 배율 100%→103%로 이 수치는 +10.76%로 올랐다**(같은 정상상태 방식으로
      잰 옛 모델은 +8.59%로 이 문서값과 거의 일치 — 위 「톡톡이 배율 정정」 절.
      첫 정정 시도는 짧은 창(30초)을 써서 +7.56%로 잘못 적었었다).
- [x] **엔진에 직접 물어 검산, 손계산 아님** — `backend/scripts/audit_milk_tap_fire.py`가
      `generate_segmented_shots`가 실제로 만든 ShotRecord 타임라인을 세어 찍는다.
      장탄 6/10/14(재장전 2.0초)에서 k=1/1/2로 스펙의 손계산 표와 일치, 「창 초과」
      0건(전 조합 최대간격 ≤ 5.76s < 6.0s 창).
- [x] **Task 6 — 매거진당 풀차지 발수를 화면까지 나른다.** 「항상 톡톡이로
      계산했어요」는 밀크에게 거짓이다(그녀는 매거진마다 풀차지를 섞어야 Pierce가
      산다) — 그대로 두면 플레이어가 관통특화를 잃는다. `raid_simulator.
      full_charges_per_magazine`(매거진별 풀차지 발수의 최빈값, **톡톡이가 하나도
      없는 매거진은 후보에서 제외** — 안 그러면 차속 창에서 최빈값이 매거진
      크기가 되어 「풀차지 6회 + 톡톡이」라는 모순된 문구가 나온다) → API
      `partial_charge_full_rounds: dict[str, int]` → 프론트
      `helpText.tapFireMixed(name, fullRounds)`가 발수를 숫자로 말하는 세 번째
      문구. `partial_charge_slugs`에 있고 이 값이 1 이상인 좌석만 `DeckCard.tsx`가
      이 문구로 고른다.
- [x] HEAD 기준 최종 검증(전 태스크 완료 후): 백엔드 **2565 passed / 3 skipped**,
      프론트 **945 passed / 73 files**, 타입에러 0.

### 앨리스 인코딩 + 부분 차지 샷(톡톡이) (2026-08-19, Fienn 제보·실측)

- [x] **앨리스 수집·인코딩** — ShiftyPad 정본(rid 191) + lootandwaifus 원문. SR ·
      버스트 3 · Fire · 차지 1.5초 · 풀차지 **350%**(계정에 소장품이 붙으면 383%) ·
      6발 · 재장전 2초. Energizing Carrot(FB진입, top-2 ATK, **캐스터 기준** 차속
      0.17505초 + 차지댐 7%) · Healthy Carrot(`has_pierce` 영구) · Wonderland(자기
      차속 +80.15% / ATK +55.12%, 10초, 버스트 딜 없음).
- [x] **멈춤 15프레임 실측 등록** — `TIMED_CHARGE_MOTION_DELAY`. 딜레이 14.75f
      (n=12, sd 0.62)이고 `차지 + 딜레이 = 발간격`이 0.95프레임으로 검산된다.
      **표에서 가장 짧다** — 안 잰 차지 무기에 22프레임 stand-in을 주는 것의 반례.
- [x] **부분 차지 샷 프리미티브** — 대미지는 HUD 게이지 퍼센트 그 자체(무차지 100%
      에서 풀차지까지 선형)이고 톡톡이는 게이지를 안 채운다. 배율이 홀드에 선형이라
      **중간 지점은 항상 지배당하고** 판정이 두 값 비교로 닫힌다
      (`attack_rate.tap_fire_wins`, 탐색 없음). **정정(2026-08-20): 톡톡이가
      게이지를 「전혀」 안 채우는 게 아니라 103%까지는 채운다** — 발사 자체가
      게이지 3%를 요구한다(위 「톡톡이 배율 정정」 절). 도함수 부호 논증과
      「매거진마다 고른다」 결론은 안 바뀐다 — 103%도 여전히 h=0 끝점이다.
- [x] **매거진마다 고른다** — 승자가 창 안팎으로 갈리므로 런 단위 플래그로는
      **−27.6%**다(Fienn의 크라운+프리바티+큐브 덱). `_base_shot_records`가 차지속도를
      이미 샘플하는 그 granularity에서 고르고, 결과는 `tap_fire_used`로 화면까지 간다.
      옵트인은 `registry.TAP_FIRE_CANDIDATES`.
- [x] **앞선 floor 모델(17프레임 · 딜레이 0)은 틀렸었다** — 「자동 23발 / 수동 40발」을
      같은 축의 정도 차이로 읽고 사이에서 35발을 고른 위에 서 있었는데, 실제로는
      **자동 멈춤 26.1프레임 대 수동 멈춤 15프레임**이었다(버스트 창에서는 스킬1의
      캐스터 기준 감소까지 더해져 차지가 0이라 둘 다 풀차지다). 그 모델은 딜레이를
      0으로 둬서 창 밖에서 **17~83% 빠르게** 쏘고 있었다.
- [x] 백엔드 **2542 passed / 3 skipped** · 프론트 **942 passed / 73 files** · 타입에러 0.
- [x] **밀크: 블루밍 바니의 멈춤 재측정 — 위 「밀크: 블루밍 바니 톡톡이」 절로 완료**
      (2026-08-19/20). 멈춤 21.889f(n=9)·톡톡이 간격 14.810f(n=21)로 둘이 갈렸고,
      `TAP_FIRE_CANDIDATES`에 등록됐다.
- [ ] **아인의 멈춤 재측정** — 22프레임 stand-in인데 손익분기 탭 간격이 **19.9프레임**이라
      톡톡이 부호가 그 안 잰 값 위에서 갈린다. 재기 전까지 `TAP_FIRE_CANDIDATES`에
      안 넣는다.
- [ ] **차속이 걸린 상태의 딜레이 확인** — 15프레임 판독은 버스트를 안 쓴 구간이다.
      딜레이가 차속과 무관하다는 것은 아직 가정이고, 1차 판독의 23발이 그 가정 위에서
      맞아떨어진 것이 유일한 방증이다.
- [ ] **소장품 383%의 출처 확인** — 383/350 = +9.43%가 소장품 차지대미지 배율과 맞는지
      로스터 재동기화 후 대조. 엔진은 이미 그 배율을 `weapon` 스탯으로 갖고 있다.

### 홀드 파이어 택틱 (2026-08-18, Fienn 제보)

- [x] **`hold_fire_segments`** — 자기 버스트로 연 FB 창 동안 사격 정지. 창은
      `context.full_burst_windows`에서 읽어 확장분까지 반영. `release_shots=1`이면
      창 닫히기 **1프레임 전**에 한 발(반열린 구간이라 벨 위에 쏘면 FB 밖이다).
- [x] **런의 선택으로 받는다** — `evaluate_deck(hold_fire={slug})`, `max_bursts`와
      같은 범주. **유닛 속성으로 박으면 안 된다**: 라운드 버프 제공자 없는 덱에서
      홀드는 미하라 **−31.66%**·아인 **−27.39%**다.
- [x] **덱 게이트** — `evaluate_deck_hold_fire_options`가 「택틱 대상 유닛 + 라운드
      버프를 남에게 주는 유닛」이 다 있을 때만 대안을 낸다. 게이트는 **슬러그 목록이
      아니라 규칙 태그**(`grants_round_buff_to_allies`)로 읽어 새 제공자가 자동 포함된다.
      고르는 곳은 **리포트 경로**(`evaluate_deck_best_seating`), 탐색 핫패스 아님.
      부분집합을 다 낸다(「둘 다 홀드」가 최적이 아닌 사례가 있다).
- [x] **측정:** 미하라+미란다 덱 **+2.15%** · 아인+미란다 **+0.83%** ·
      **아인+미란다+타키나 +7.37%**. 스윕 101 중 **0 변경**, 벤치 무변화 — 끄면 오늘 그대로.
      **⚠ 크기는 그 픽스처에서의 값이다** — ATK 스프레드에 따라 부호까지 뒤집힌다(아래).
- [x] **에이다 해소 (2026-08-18)** — 특수개조를 **1발 세그먼트**로 다시 인코딩해
      차지 4.0초·차지댐 1750%가 둘 다 착지했다(`until_shots: 1`이 곧 「for 1 round(s)」).
      매거진 경계에서 샘플되는 스탯의 라운드 버프는 근사하지 말고 **선언**하면 된다는
      일반형이기도 하다. 스윕 **ada-wong +1.93%**. 그녀의 홀드는 그 세그먼트를
      **대체**한다(방출탄이 곧 그 샷) — 미란다 덱 **+4.00%**·개인 **+7.35%**.
- [x] **사이클 단위 탐색은 기각(2026-08-18, 측정)** — 그리디 1패스로 실제 구현해
      쟀는데 **5덱 전부에서 오늘의 최선을 못 이겼다**(−0.55%/+0.00%/+0.00%/
      −0.97%/−1.65%). top-ATK 타겟팅이 끈적해서 그녀는 거의 전 사이클을 먹거나
      거의 못 먹고(7/7·2/7·0/7·0/7·0/7), 섞이는 경우는 ATK가 근소차일 때뿐이라
      그때는 홀드 자체가 값을 못 낸다. `docs/decisions.md`에 재검토 조건과 함께 기록.
- [x] **부수 발견: 홀드의 값은 ATK 스프레드에 따라 부호가 뒤집힌다** — 앞서 보고한
      「아인+미란다 +0.83%」(95k/55k)가 110k/70k에서는 **−1.65%**다. 앞선 측정치들은
      「그 픽스처에서」의 값이고 법칙이 아니다. 덱마다 둘 다 재는 오늘 구조가 이걸
      흡수한다(`docs/insights.md`).
- [x] 백엔드 **2524 passed / 0 failed**(신규 11).

### 라운드 버프 창의 끝·인과·중첩 (2026-08-18, Fienn의 「왜 다음 평타까지 유지하나」에서)

- [x] **창은 N번째 소모탄 자신에서 닫힌다.** 그 탄은 덮고 그 뒤는 아니다. 종전엔
      「다음 샷」까지 갔는데, 그건 **틀린 것 둘이 상쇄된 결과**였다(아래).
- [x] **`from_own_shot`** — 캐스터의 평타가 트리거인 그랜트는 그 탄을 덮지도
      소모하지도 않는다(탄 k의 버프는 탄 k+1의 것, Fienn). 종전엔 엔진이
      `>= granted_at`으로 **그 탄 자신**을 소모자로 지목했고, 그래서 끝을 한 샷
      늘려야 팬텀이 맞았다 — 상쇄할 짝이 없는 미란다·츠바이 쪽에서만 창이 길었다.
      **타임스탬프로 추론하면 안 된다**: 질 발렌타인 Magnum Ammo는 **재장전** 발동이고
      탄창 첫 탄은 표식일 뿐이라 그 탄이 9발 중 하나다.
- [x] **캡 기본값 1** — 「stacks up to N」이 없으면 중첩하지 않는다(Fienn).
      이게 없으면 위 항목이 팬텀을 반대로(2중첩) 깨뜨린다.
- [x] **스윕 101 중 8 변경:** zwei +0.50% · zwei-signature +0.53% ·
      jill-valentine **−0.48%** · dorothy-serendipity −0.09% · d-killer-wife −0.05% ·
      ein −0.01% · phantom(·애장품) −0.00%(7,651/764M, 전투 첫 탄 하나).
- [x] **Fienn이 물었던 세 덱은 한 자리도 안 바뀌었다** — 과잉 구간이 비어 있었다
      (SW:헤비암즈 36.4초가 딜 0). 이제 그 구간이 **0.00초**다.
- [x] 백엔드 **2513 passed / 0 failed**(신규 11).

### 「N발 유지」 버프가 부여 시점부터 산다 + 캡은 순간 규칙 (2026-08-18, Fienn 택틱 확인에서)

- [x] **라운드 그랜트 창이 `granted_at`부터 열린다.** 종전엔 수신자의 다음 탄에
      앵커돼 있어, 같은 트리거에 터지는 그 유닛의 넉이 버프 생기기 **0.067초 전에**
      계산됐다. 평타에는 안 보이는 앵커였다(그랜트와 첫 탄 사이엔 그 유닛 샷이
      없다). Fienn 인게임 확인 둘: FB 진입 시 미란다 버프가 마르차나 넉보다 먼저
      걸리고, 「N발 유지」는 **스킬 딜에도** 적용된다.
- [x] **창의 끝은 안 건드렸다** — N번째 소모탄의 **다음 샷**. 「소모한 탄 직후」로
      조였다가 스윕이 **팬텀 −2.9%**로 잡았다: 탄과 탄 사이는 사각지대가 아니라
      보유 구간이고, 그녀의 자공댐 1발 버프는 그 사이에 터지는 자기 버스트에
      걸려 있어야 맞다.
- [x] **「stacks up to N」 캡을 순간 규칙으로 다시 짰다**(`_capped_round_grant_segments`).
      창이 사슬처럼 겹치자 종전 per-grant 규칙이 츠바이 3중첩을 **1**로 읽었다.
- [x] **측정:** 택틱 덱 넉 78.2M → **132.4M(+69.25%)** · 마르차나 +34.86% ·
      덱 총딜 **+21.81%**. **고정 셸 스윕은 101 중 0 변경** — 셸에 이 모양이 없다.
      부수효과로 츠바이(애장품)+그레이브 +2.37% → **+6.21%**.
- [x] 백엔드 **2502 passed / 0 failed**(신규 10).

### 「N발 유지」 버프가 [장탄 수 무한] 중에는 안 깎인다 (2026-08-18, Fienn 질문에서)

- [x] **라운드 그랜트의 계수를 「발사한 샷」에서 「소모한 탄」으로 바꿨다.**
      [장탄 수 무한] 창 안의 사격은 탄을 안 쓰므로 버프가 안 깎이고, **창이 끝날
      때** 소모된다(Fienn 인게임). 그전엔 미란다(애장품)의 크확 85.42%가
      나유타에게 **0.05초**, 그레이브에게 0.083초만 살았다.
      `_round_grant_shot_window` — 창이 없는 수신자에겐 종전과 같은 답을 낸다.
- [x] **[장탄 수 무한]을 상태 축으로 선언한다.** 유닛이 자기 버스트 슬롯에서 창
      길이를 내놓고(`_UNLIMITED_AMMO_DURATIONS` → `unlimited_ammo_durations`)
      변환 패스가 읽는다. **사격 생성은 안 건드린다** — 넷 다 재장전 쪽을 이미
      각자 모델하고 있어 이중 계상이 된다.
- [x] **인원 census(Fienn):** 그레이브 10초 · 나유타 10초 · 목단 10초 ·
      모더니아 15초. **레드후드는 아니다** — Red Wolf는 99발 탄창이 창보다 길 뿐
      원문에 「Unlimited ammunition」이 없고, [1발 유지]는 정상 소모된다.
- [x] **측정**(`scripts/measure_unlimited_ammo_round_grants.py`):
      미란다(애장품)+나유타 **+17.58%** · +목단 **+8.06%** ·
      츠바이(애장품)+그레이브 **+7.28%**. 나머지 쌍의 0.00%는 이유가 각각 다르다.
- [x] 백엔드 **2492 passed / 0 failed**(신규 22).

### 「제외한 니케」 필터 + 레이드 탭의 제외 안내 (2026-08-18)

- [x] **니케 검색 필터에 「제외한 니케」 칩이 생겼다.** 속성·단계와 같은 자리·같은
      모양이고, 아이콘이 없는 축이라 글자가 그 이름이다(`.unit-filter__chip--text`,
      높이는 아이콘 칩과 같은 `--filter-chip-box`). 규약도 같다 — 켜면 **제외한
      니케만** 남고, 안 켜면 아무것도 안 거른다(Fienn 결정: 「숨기기」도 「제외/사용
      2칩 그룹」도 아니다). `UnitFilterState.excludedOnly` + `UnitFacets.excluded`로
      들어가고, 「필터 해제」가 이 축도 같이 끈다. 제외라는 개념이 없는 화면(미란다
      계산기)에는 칩이 아예 안 나온다 — `excludable`은 `excludedSlugs !== undefined`,
      그래서 팔레트의 기본값 `= []`를 없앴다.
- [x] **솔로·유니온에서 제외된 칩을 누르면 어디서 푸는지 말해준다.** 「다시
      활성화하려면 니케 풀에서 활성화해주세요」가 그 칩 **오른쪽**으로 열린다
      (오버로드 팝오버는 아래로 열리는데, 누르는 동안 마우스가 칩 위에 있어 둘이
      반드시 같이 뜬다). 4초 뒤 스스로 사라지고 다른 칩을 누르면 곧바로 걷힌다.
      배치 화면의 제외된 칩은 이제 `disabled`가 아니라 `aria-disabled`다 — disabled면
      그 누름이 아예 안 와서 왜 안 되는지 말할 기회가 없다(앉은 칩은 그대로
      `disabled`: 앉았다는 테두리가 이미 이유를 말한다). 말풍선은 `aria-hidden`이고,
      읽히는 것은 팔레트 뿌리의 `aria-live="polite"` 한 줄이다 — `role="status"`를
      안 쓴 이유는 이 팔레트를 품은 화면이 제 진행 상황을 알리는 status를 이미
      갖고 있어서다.
      착륙 시점 프론트 922 → **939 passed** (73 files), 타입에러 0 · lint 에러 0.
      Vite dev + FastAPI를 Playwright로 띄워 실제 브라우저에서 확인했다(니케 풀
      필터 · 솔로 말풍선 · 유니온 말풍선 · 초상화 클릭 경로).
      **알려진 것:** 팔레트 마지막 열의 칩에서는 말풍선이 덱 컬럼 왼쪽 가장자리를
      ~60px 덮는다(불투명·4초·최상단이라 읽기에는 지장 없음).

### 갱신되는 스택 — 전부 닫힘 (2026-08-17)

`stacks up to N ... lasts for D sec`는 **스택 전체가 공유하는 타이머 하나**이고 새
스택마다 D초가 다시 시작한다(레이븐 판정 2026-07-17 · 메이든 사격장 2026-08-17).
전수 census는 `python scripts/audit_stack_lifetime_refresh.py`, 유닛별 카운트·딜은
같은 스크립트의 엔진 절반이 낸다.

- [x] **센티(애장품) · 로자나: 시그니처** — 상한 있는 `ResourceSpec`으로 재인코딩.
      덱 **+48.94% / +20.90%**. 상세는 `docs/decisions.md`(2026-08-17).
- [x] **안 걸리는 것 확인** — 아스카 WILLE · 줄리아 시그니처(두 모델 다 상한 도달) ·
      레오나(영구 근사가 정확히 같음을 실측: 5초 0.00%, 대조 0.5초 −3.91%) ·
      헬름 아쿠아마린 · 퀜시 · 레드후드 · 토브 · 사쿠라 · 레이븐 ·
      「continuously」로 적힌 것들(아르카나 · 신데렐라 · 기요틴 · 소다 · 미하라 · 엘레그).
- [x] **팬텀 두 빌드 다 손댈 것 없음** — base의 보류는 갱신 규칙으로도 안 열린다(대거
      소스가 자기 지속과 같은 5초 카드에 막혀, 갱신할 살아 있는 스택이 없다. 적이
      하나인 레이드에서만 성립하는 교착). 시그니처는 갱신 규칙으로 따라가도 상한이
      같은 **60발**에 떨어지고 딜은 양쪽 0.00%.
- [x] **줄리아 · 모더니아 · 즈바이 시그 · 디젤 ×2에 `lifetime_refreshes`** — 코어
      48.89px · DEF 8000 · 180초, 셸은 후보 티어별 고정에서
      **줄리아 +9.22% / 덱 +1.86%** · 모더니아 +2.95% / +1.33% ·
      즈바이 시그 +2.14% / +1.70% · 디젤 인트로 +2.52% / +1.14% ·
      디젤 하이라이트 +2.17% / +1.06%. 헬퍼 `linear_resource_buff`가 플래그를 받도록
      확장(소비자가 다섯이라). **옛 테스트가 이 의미론을 하나도 안 잡고 있었다** —
      전부 통과했다. 유닛마다 「공유 시계 vs 스택별 만료」를 가르는 시각을 골라
      카운트를 대조하는 테스트를 새로 넣었다.
- [x] **라플라스 ±시그** — 아래 엔진 항목(수명을 자원으로)을 하고 나서 닫혔다.
      게이트가 열린다: 카운터가 전투의 **69%**(시그 96%)를 상한에 머문다. 스택별
      만료로 되돌리면 **본인 −11.61% / 덱 −2.12%**(base) · −2.51% / −0.63%(시그).
      2026-08-14 판정의 「0 of 322 ticks」 근거 산수가 뒤집혔다 — `docs/decisions.md`.
- [x] **★ 엔진: 수명을 `ResourceSpec`으로 올렸다** (Fienn이 옵션 B 선택, 2026-08-17).
      `ResourceBuff`에서 **옮겼고**(복사 아님), 시뮬 셋업이 `register_resource`로
      컨텍스트에 등록하며 `resource_count`는 등록된 의미론이 인자를 이긴다. 게이트는
      4-튜플 → **3-튜플**(lifetime 원소 제거), 스펙 딕트의 미사용 `"lifetime"` 선택키도
      삭제. 이제 다섯 읽기 경로가 구조적으로 한 값을 본다. 벤치 152.51ms(회귀 없음).
- [ ] **실기록 5덱에 해당 유닛이 하나도 없다** — 캘리는 안 움직이고, 그래서 이
      계열 수정은 **인게임 확인 외에 검증 수단이 없다**. 스윕은 반드시 움직인다.

### 메이든의 Meditation 스택 + 나중 패스의 Max HP를 환산이 못 읽던 순서 갭 (2026-08-17, A7 1·2단계)

- [x] **버그: 「상한이 안 걸린다」는 근거가 틀렸다 (Fienn 정정).** 독스트링은
      「10스택은 **한** 15초 창 안에 풀차지 60발이 필요한데 RL 케이던스가 못 미친다」로
      cap 10을 미모델로 뒀는데, **창이 고정이 아니다** — 스택이 하나 붙을 때마다 15초가
      새로 카운트되므로 15초 안에 6풀차지를 계속 성공하면 **상한까지 쌓인다.**
      옛 모델(독립 15초 타이머·cap 없음)은 그녀를 **약 2스택에 묶어** 과소평가했다.
- [x] **신규 자원 의미론 `lifetime_refreshes`.** `resource_count`의 세 번째 규칙 —
      fill마다 **전체 스택의 시계가 리셋**되고, 마지막 fill + lifetime에 **한꺼번에**
      사라진다. 연속 fill 간격이 lifetime을 넘으면 사슬이 끊겨 다시 1부터.
      Meditation을 per-shot 룰에서 **cap 10짜리 `ResourceSpec`**으로 옮겼다.
- [x] **★ 옛 테스트가 틀린 모델을 못박고 있었다** — 「첫 스택이 15초에 만료되고 둘째는
      살아 있다」를 단언하고 있었다. 그게 이 버그의 화석이다. 새 테스트가 그 반대를 고정한다.
- [x] **⚠ 1단계 시점엔 딜이 0.0000% 움직였다 — 이 스택엔 대미지 소비자가 없었다.**
      per-stack 값을 **100배**로 키워도 그녀 딜도 덱 총합도 **한 자리도 안 변하지
      않았다**. 원인은 순서다: Blessings Upon You의 **Max HP → ATK 환산은
      `own_burst_activate` 룰**이라 **버스트 사이클 안**에서 돌고, 이 스택은 **그 뒤에
      도는 패스**가 낸다. 독스트링의 「Blessings를 통해 딜에 닿는다」는 **두 빌더를
      손으로 순서대로 부른 유닛 테스트에서만 참**이었다.
- [x] **★★ 그 0.0000%를 처음 잰 프로브는 아무것도 재고 있지 않았다.** 정의 모듈의
      빌더를 monkeypatch했는데 `registry.py`가 `from x import build_y`로 **자기 바인딩을
      들고 있어** 패치가 조용히 무시됐다. 숫자는 우연히 맞았지만 **근거는 없었다** —
      2단계에서 `registry` 네임스페이스를 패치하고 **VISIBLE로 뜨는 컨트롤 둘**을 함께
      돌려서야 진짜로 재는 프로브가 됐다. **측정 스크립트는 소비자가 실제로 부르는
      바인딩을 패치해야 한다.**
- [x] **갭을 닫았다: 환산이 나중 패스의 `flat_max_hp`를 읽는다.** `simulate_raid`의
      기존 고정점 루프(소다 풀버스트 확장용)에 **두 번째 수렴량**을 얹었다 — 패스 1이
      **버스트 사이클 뒤에** 쓰인 `flat_max_hp` 효과를 모아 돌려주고, 패스 2가 그걸
      **읽기 전용 그림자 레지스트리**로 받아 `SquadContext.live_max_hp`에서 라이브
      레지스트리와 함께 읽는다. Max HP는 타임라인을 안 바꾸므로 **항상 두 패스 안에
      멈춘다.** 환산이 버스트 사이클 안에 그대로 남으므로 **top-ATK 랭킹**(미란다·나가·
      레오나·마나·맥스웰)도 교정된 ATK를 본다 — 후처리 패스로 옮겼다면 그게 틀어졌다.
- [x] **★ 메이든만의 문제가 아니었다 — Rouge도 같은 갭에 걸려 있었다.** Card Throw의
      per-shot squad Max HP는 **샷 루프**에서 붙어서 역시 환산에 안 보였다. 그녀
      독스트링은 「Max HP 부여엔 순서 제약이 없다 — 각 대미지 인스턴스 시점에 읽힌다」고
      적어 두었는데, **환산은 인스턴스 시점이 아니라 버스트 시점의 스냅샷**이라 그
      논리가 틀렸다. 인코딩 이래 계속 0이었다.
- [x] **실측 이득.** 메이든 덱(리터·크라운·블랑·헬름) **868,251,562 → 893,901,458
      = +2.95%**(그녀 본인 **+5.23%**) · Rouge+맥스웰+라플라스UH 덱 **677,018,878 →
      680,370,360 = +0.50%**. 백엔드 **2453 passed**.
- [ ] **남은 것: 「환산 소비자는 반드시 버스트 사이클 안에 있어야 한다」가 규칙이 됐다.**
      후처리 패스에 환산을 놓으면 라이브 + 그림자로 **이중 계상**된다. 카탈로그에 적었고
      테스트는 없다 — 소비자가 넷뿐이라 지금은 규칙으로 둔다.

### 애니힐리오 기록을 채점지표에서 내린다 — 다음 지표는 곧 나올 레이드 (2026-08-17)

- [x] **Fienn 결정.** 신데렐라:CW 오차(1.237x, 절대오차 1위 +0.516B)는 **보류**하고,
      애니힐리오 기록은 **기록으로만** 남긴다. 더 살펴보지 않는다.
- [x] **이유가 그녀 하나에 그치지 않는다.** 전투는 **2026-07-06**, 채점 로스터는
      **2026-08-07 동기화본**이다(6주). 오버로드는 장비 투자라 늘기만 하는데 시뮬은
      **오늘 것**을 읽으므로, 유닛마다 **알 수 없는 크기의 과대 항**이 얹혀 있다.
      **되돌릴 수 없다** — `tools/collect-blablalink/`의 가장 이른 스냅샷이 07-31로
      전투보다 3주 뒤다.
- [x] **크기의 감**: 신데렐라:CW의 오버로드는 이 전투에서 그녀 딜의 **21.7%**다
      (덱2를 그녀 오버로드만 빼고 재채점: 1.237x → **0.969x**, 즉 빼면 과소가 된다).
      그녀만의 수치가 아니라 **모든 유닛이 지고 있는 항의 크기**다.
- [x] **배너를 출력에 박았다.** `measure_record_calibration.py`가 맨 위에
      「채점지표에서 내렸다」를 찍는다 — 파일 안에만 적으면 아무도 안 본다
      (`full_burst_passes.converged`가 그렇게 됐다). 콘솔 cp949 깨짐도 같이 고쳤다.
- [ ] **★ 새 레이드에서 지켜야 할 단 하나: 로스터를 기록과 같은 날 동기화할 것.**
      이 실패는 모델이 아니라 **절차**에서 왔다. 기록 시점 로스터가 없으면 유닛별
      잔차는 원리적으로 귀속 불가다.
- [ ] **이 잔차에 기대고 있던 결론들** — 새 기록으로 다시 재기 전까지 확정으로 읽지 말 것:
      합계 1.075x·18/25 · 무기군별 평균 · **갭 #21의 코어 비중 상관(r=+0.677, 22유닛)**.
      갭 #21 항목 머리에 경고를 달았다. 결론을 폐기하는 건 아니고 **재확인 대기**다.
- [ ] **`docs/decisions.md` 항목은 `/document`로 남겨야 한다** — 이 세션은 서브에이전트를
      못 부른다(사용자 지시). 위 내용이 ADR 재료다.

### 확률 발동 트리거는 기대값으로 (A9, 2026-08-17)

- [x] **Fienn 규칙: 확률은 굴리지 않고 기대값으로 처리한다.** 엔진은 이미 다른 확률을
      전부 그렇게 다루고 있었다 — 크리는 히트마다 `crit_rate`로 스케일되고, 코어히트는
      면적비이고, `per_critical_hit_every`는 실제 크리를 세는 대신 **라이브 크리율을
      누적**한다. 「p% 확률로 발동」 = **100/p 발마다**
      (신규 `_helpers.expected_shots_per_proc`, 발 단위로 반올림).
- [x] **★ 범위가 생각보다 훨씬 작았다 — 전수 census로 확인.** 인코딩 103슬러그의 원문을
      전부 훑으니 확률 **트리거**를 가진 건 **2유닛(×2빌드)뿐**이다(그물을 `random`·
      `odds`·`% of the time`까지 넓혀도 나머지 12건은 전부 「무작위 **적** 대상」이라
      단일보스 레이드에선 무의미).
      · **`tove`(base)** — 「공격 시 5% 확률」 → 20발마다. **이것만이 확률이 유일한
        장애물이었다.**
      · **`sugar`·`sugar-signature`** — 「엄폐물 피격 시 20% 확률」. **막는 건 확률이
        아니다** — 기대값 규칙은 이벤트 스트림을 솎아낼 뿐인데 솎을 스트림이 없다.
        애장품 빌드는 확률이 아예 없는데도 같은 이유로 여전히 보류다(갭 #14).
- [x] **★ 같은 슬롯이 빌드마다 다른 뜻이라 빌더를 나눠야 했다.**
      `description_value_01`이 기저는 **확률**, 애장품은 **발수**다. 하나로 공유했으면
      기저가 **5발마다** 재장전해 4배 과대인데, 숫자가 그럴듯해서 리뷰를 통과했을 것이다.
      두 방향 다 테스트로 못박았다.
- [x] **실측.** Tove 기저 딜 **+1.07%** · 덱(크라운·헬름·레드후드·블랑) **+0.20%**.
      자기 탄창만 건드리는 self 환급이라 작은 게 맞다.

### 플로라의 「중첩 수 ▲1」 (A7) — 착륙 완료 (2026-08-17)

원문: **「Activates after landing 100 normal attacks. Affects all Electric Code
allies. Increases the stack count of stackable buffs by 1.」**

- [x] **Fienn 판정 둘.** ⓐ **현재 중첩이 +1**이다(상한을 올리는 게 아니다) ·
      ⓑ 범위는 **그 유닛이 받는 모든 중첩 버프**(출처 무관).
      ⓐ의 귀결이 크다 — 엔진이 **정상상태(최댓값)로 근사한 중첩 버프는 이미 상한**이라
      **+1이 inert**다. 그러니 값이 나는 건 **진짜 카운터(`ResourceSpec`)가 상한
      아래일 때**뿐이다.
- [x] **대상 인구를 셌다.** 인코딩 103슬러그 중 **Electric은 20**, 그중 `ResourceSpec`을
      가진 건 **셋**: `cinderella.beautiful`(cap 12) · `maiden-ice-rose.mp`(cap 12) ·
      `zwei(-signature).pierce_attacks_101`(cap 3).
- [x] **★ 작은 항목이 아니다.** 플로라는 **B2 · Electric · MG**라 180초에 평타 **8,560발**
      → 100발마다면 **85회 프록**(약 2초에 한 번). cap 12짜리에 85회 +1을 부으면
      **전투 내내 상한에 박힌다** — 신데렐라의 아름다움은 Glass Slippers 라이더를
      스케일하고 메이든의 MP는 그녀 버프를 굴리므로, 넓고 큰 변경이다.
      (내가 A7을 「가장 작다」로 분류했던 건 틀렸다.)
- [x] **★ 내 오독 정정 (Fienn).** `pierce_attacks_101`은 「관통 공격 101회」를 세는
      카운터가 **아니다** — **츠바이 버스트 버프의 이름**이고(10초 전체 아군 버프),
      그 이름의 자원은 그 버프가 떠 있는 동안 평타마다 쌓이는 **크리율 스택**
      (스쿼드 스코프)이다. 즉 셋 다 중첩형 **버프**가 맞다.
      **자원 이름을 문구로 읽지 말 것 — 스킬 효과의 이름일 수 있다.**
- [x] **Fienn 사격장 실측이 대상을 갈랐다 (2026-08-17).** 「그 유닛이 자기에게 쌓는
      스택에도 닿는가」의 답은 **자원마다 다르다** — 스펙의 성질로는 구분이 안 되므로
      `ResourceSpec.stackable_buff`로 **선언**한다.
      · **신데렐라 아름다움 = 받는다.** t=16에 12스택 도달. 플로라 없이 3초마다 1스택이면
        풀스택은 t=33이다. 「[12 중첩]」 문구도 이 판정을 보강한다.
      · **츠바이 크리율 스택 = 받는다.** 본인이 평타를 안 쏴도 스택이 는다.
      · **메이든 Meditation = 받는다.** 1스킬 3번불릿이 t=16에 6스택. 플로라 없이
...
- [x] **막고 있던 것이 이미 없었다.** 독스트링은 「periodic-nuke 경로엔 창 검사
      프리미티브가 없다」고 적었지만, **periodic 넉은 버스트 사이클이 끝난 뒤 도는
      후처리 패스**라 그 자리에서 `full_burst_windows`가 보인다(같은 스펙의
      `"during_full_burst"`가 이미 그걸 쓴다). 조건을 걸 자리가 없던 게 아니라
      **안 걸었을 뿐**이다. 그 독스트링이 인용한 제약은 **`periodic_rules`의 것**이고
      (그 패스는 버스트 사이클보다 **먼저** 돌아 `burst_times`가 비어 있다),
      그게 이 경로에까지 확대 적용돼 있었다.
- [x] **신규 스펙 키 `full_burst_rider`.** 틱 자신의 시각이 창 안이면 버프를 남긴다.
      `add_refreshing`이라 쿨 < 지속인 스펙도 자기와 안 쌓인다. 키가 없으면 오늘
      동작 그대로.
      **★ `during_full_burst`와 혼동 금지** — 그건 창 밖 틱의 **대미지를 지운다**.
      원문은 넉이 아니라 **라이더**를 게이트한다.
- [x] **소비: 스노우화이트** `crit_rate` +0.261 / self / 10초.
      (`normal_attack_crit_rate`가 아니다 — 원문이 그냥 「Critical Rate」다.)
      스윕 **+1.03%**. 그녀는 실기록 25유닛에 없어 캘리는 불변.
      **이번엔 스윕이 본다** — 크리는 DEF와 무관하므로 [[sweep-boss-has-no-def]]에
      안 걸린다.

### 레이 아야나미의 Anti A.T. Field 넉 착륙 (2026-08-16, A3)

- [x] **신규 per-shot 모드 `every_during_ally_status_window`.** 창이 **아군의**
      버스트에 앵커되고 **창마다 카운터가 리셋**된다(Fienn 판정). 리셋이 맞는 이유는
      원문이 창 끝에서 상태를 **제거**하기 때문 — 세다 만 카운트가 이월될 곳이 없다.
      아군이 덱에 없으면 창이 0개라 룰이 한 번도 안 뜬다(그게 게임의 답이다).
- [x] **Rei의 590.64% 넉 배선.** 「18발마다, Anti A.T. Field 상태의 타깃에게」.
      창 길이는 **아스카가 소유**하므로 그녀 모듈이 `ANNIHILATION_STATE_DURATION`·
      `SLUG`로 내보내고 Rei가 import한다 — 9를 두 곳에 적지 않는다.
      `test_annihilation_state_duration_constant_matches_the_data`가 그 상수를
      스킬 데이터에 못박는다.
- [x] **「Anti A.T. Field stacks ▲10」도 배선했다 (Fienn 지시, 2026-08-16).**
      처음엔 「여유 0이라 inert」를 근거로 안 만들었는데, **효과가 실재하고 로스터는
      고정이 아니므로** 표현해 두기로 했다. 신규 fill 종류
      `per_shot_every_during_own_status_window_by_ally` + **기여(contribution) 배관** —
      스택은 아스카 자원의 것(그녀의 cap·소비자·reset)이고 숫자는 Rei의 것이라,
      **Rei 모듈이 선언하고 `roster._merge_resource_contributions`가 합친다.**
      대상이 덱에 없으면 기여는 버려지고, **cap은 절대 넓히지 않는다** —
      `resource_count`가 합친 총량을 그대로 30으로 클램프한다(테스트가 못박는다).
      **오늘 딜은 안 움직인다**(아스카가 이미 상한). 열리는 조건은 그녀가 창에서
      약 33발/초 아래로 느려지는 편성이다.
- [x] **셋째 불릿 「Annihilation State인 아군에게 ATK +17.6% of caster ATK」도 배선.**
      새 프리미티브 **0개** — `member_subset_buff_rule`(Rei 자신의 옆 불릿이 쓰던 것)
      + 신규 `ANNIHILATION_STATE_SLUGS`(`PERSONA_STATE_SLUGS`와 같은 모양).
      **Persona와 다른 점 하나:** Persona는 전투 시작에 자기가 걸고 안 풀리므로 멤버십만
      물으면 되는데, Annihilation State는 **자기 버스트가 연다** — 그래서 소비자는
      `context.burst_used_this_cycle`도 같이 봐야 한다. 같이 적힌 「대상 수 ▲1」·
      「사거리 ▲500%」는 단일 보스에서 inert라 뺐다.
- [x] **두 불릿의 캘리 여파:** 합계 1.069x → **1.075x**, ±15% 19/25 → **18/25**.
      밴드를 벗어난 건 **아스카**(1.106x → **1.171x**, Rei의 ATK를 받아서)다.
      덱3이 1.130x → **1.166x**로 최악 덱이 됐다 — 크라운 1.393·헬름시그 1.332가
      이미 그 안에 있어서, **덱3에 이 덱만의 별개 과대 항이 있다**는 신호로 읽힌다.
      **되돌릴 신호가 아니다**(원문이 정한다), 다음 조사 대상이다.
- [x] **결과: 레이 0.932x → 1.088x** (−0.096B → +0.124B). 합계 1.062x → **1.069x**,
...
아스카 버그 뒤 **같은 부류가 다른 유닛에도 있는지** 인코딩된 103 슬러그를 전부
스킬 원문과 대조했다. 전체 보고서는 `docs/encoding-audit-2026-08-17.md`.

- [x] **아스카형(지연 발동 오독)은 다른 곳에 없다.** 상태 이름을 참조하는 발동
      문구를 가진 유닛 12개를 개별 확인 — 전부 올바른 시점에 걸려 있다.
- [x] **F1 노아: 「같은 부대 아군이 전장에」 조건이 게이트 없이 항상 걸렸다.**
      부대는 777(Blanc·Noir·Rouge)이고, 덱에 Blanc/Rouge가 없으면 발동하지
      않아야 한다. 같은 문구를 쓰는 Blanc은 처음부터 옳게 게이트되어 있었다.
      부대원 없는 덱에서 그녀의 코어 명중률 0.0649 → **0.0501**.
- [x] **F2 퀜시: `Explore Route`가 매니페스트에 없어 스킬 값이 만렙으로 박혀
      있었다.** 스킬 2가 몇이든 항상 10레벨로 시뮬레이션됐다. 스킬 1이면 실제는
      ATK 64.8% / 명중률 35.9%인데 110.3% / 61.1%를 주고 있었고, 명중률 쪽은
      SMG 탄착군을 50px 코어 **안에** 넣느냐를 가른다(코어 명중률 0.4353 vs
      0.9996). **만렙 로스터에서는 숫자가 안 바뀐다.**
- [x] **F3 마나: Metal σ가 사이클을 건너 이월되는 상태였다** (Fienn 인게임
      타임라인 2026-08-17로 확정). 4번 불릿은 **Full Burst 종료 시점에 판정**되고,
      버프는 **그녀가 버스트한 사이클의 다음 사이클**에 붙는다.
      `own_burst_fired_this_cycle()`은 정확히 **반대 사이클**에 주고 있었다 —
      그녀의 버스트 딜(Fatal Error 10틱 DoT 포함)이 게임이 다음 창에 주는
      ATK +63.36%를 챙기고 있었다. 명시적 상태(`METAL_SIGMA_STATUS`)로 교체:
      battle_start가 세우고 · full_burst_enter가 `has_status` 게이트로 쓰고 지우며 ·
      full_burst_end가 `own_burst_fired_this_cycle()`일 때 다시 세운다.
      **스윕 tier-3 셸(B3 둘): 780.4M → 727.7M, −6.76%.**
      단일 B3 덱에서는 매 사이클 σ가 복구되어 불변 — 그래서 여태 안 드러났다.
      **모듈 docstring의 「티어당 공격수 한 명」 전제가 사실이 아니었다** —
      `burst_cycle`의 `chosen = eligible[0]`이 같은 티어 2인을 번갈아 발사시킨다.
- [x] 문서 결함 7건(코드는 옳고 서술이 틀린 것) 중 6건 수정 — 그중 셋은 같은
      docstring 안에서 코드와 **반대로** 적힌 것이었다(laplace-UH의 누적 여부,
      snow-white-heavy-arms의 트리거, privaty의 매니페스트 주석).
- [x] 재현용 스크립트 3종: `dump_skill_text.py`(원문+슬롯+**조립값**) ·
      `audit_trigger_phrases.py`(원문 발동 문구 ↔ 배선된 트리거) ·
      `audit_hardcoded_skill_values.py`(레벨 스케일 값이 리터럴로 박힌 곳).
      **슬롯 번호를 눈으로 세지 말 것** — 추출기는 값 슬롯이 아닌 숫자를 세지
      않아(“Activates once per battle”의 1) 수동 계산이 어긋난다.

### 아스카: WILLE의 긴급수복이 9초 일찍 터지고 있었다 (2026-08-16, Fienn 지적)

- [x] **버그: Emergency Repair 2번 불릿이 버스트 시점에 발동했다.** 원문은
      **"Activates when using Annihilation"**이고 Annihilation은
      **"After Annihilation State ends"** = 버스트 **+9초**다. 그런데 Effect 1
      (MG 예열 ▼100%/3초) · Effect 2(탄약 100% 제거) · Effect 4(재장전 60% 고정)가
      전부 `own_burst_activate`/`offset=0`으로 걸려 있었다. **`silent_reload_segments`엔
      `offset` 파라미터가 이미 있었는데 아스카만 안 넘기고 있었다.**
- [x] **고친 뒤 그녀의 스택이 25 → 30(상한)으로 붙는다.** 스택은 그 9초 창 **안의**
      그녀 발사로 쌓이는데, 창 **첫머리**에서 탄창을 버리고 예열을 −100%로 죽이고
      있었으니 자기 스킬이 자기 스택을 깎고 있었다. 창 안 발수 **250 → 505**
      (27.8 → 56.1발/초), 7사이클 전부 CAPPED.
- [x] **★ 이게 A3(레이 아야나미)의 근거를 바꿨다.** Rei의 「Anti A.T. Field stacks ▲10」은
      **여유가 0이라 무조건 inert**다 — 고치기 전엔 「Rei의 예열 버프가 아스카를 상한에
      올리니까」라는 조건부 논거였는데, 이제 **아스카 혼자** 상한에 붙으므로 조건이 사라졌다.
      A3에서 **쓰기 절반은 만들지 않는다**(크로스유닛 fill · 순환 의존 · 상한 초과 위험 전부 회피).
- [x] **⚠ 캘리는 1.050x → 1.062x로 1.0에서 멀어졌다** (19/25 불변). 움직인 건 **덱3
      전원**이다 — Anti A.T. Field의 Damage Taken +0.83%/스택이 **스쿼드 스코프**라
      25→30스택이 덱메이트 전부를 밀어 올린다: 크라운 1.342→**1.378** · 헬름시그
      1.28→**1.318** · 라피B1 1.069→**1.093** · 레이 0.923→**0.932** · 아스카 **1.106x**.
      **되돌릴 신호가 아니다** — 무엇이 맞는지는 원문이 정하지 비율이 정하지 않는다
      (신데렐라 차지 간격 판독과 같은 규약).
- [ ] **아스카의 캐비엇 근거를 재검토할 것.** 그녀가 캐비엇인 이유가 「탄약 덤프 +
      MG 재예열 비용을 플레이어는 컨트롤로 회수하는데 엔진은 모델링 안 한다」였는데,
      **그 비용이 이제 풀버스트 창 밖으로 나갔다.** 캐비엇이 인용하는 그 항이 사라졌으므로
      캐비엇 문구 자체가 낡았을 수 있다.

### 데미지 타이핑의 두 축 — 라플라스 버스터 (2026-08-16)

- [x] **평타의 전달 버킷이 트루뎀 타이핑에 먹히고 있었다 (Fienn 정정).** 「평타는
      트루뎀이든 아니든 발사체폭발 대미지 버프를 받는다」인데, 엔진은 `damage_type`
      **한 태그**로 버킷을 골라 `true`인 동안 `projectile_explosion_damage_up`을
      통째로 버렸다. 이제 `_damage_instance`가 평타에 한해 무기의 전달 버킷
      (`weapon_delivery_type`, RL만 갖는다)을 태그가 고른 버킷 **위에** 얹는다.
      스킬 대미지는 안 받는다 — Fienn의 말은 평타에 대한 것이었다.
- [x] **세그먼트의 `damage_type`을 자원으로 게이트할 수 있게 됐다.** 프로필의
      신규 `damage_type_gate`가 `resource_gate`와 **같은 4튜플**을 싣고, 게이트가
      `ShotRecord`를 타고 페이즈 2에서 답해진다. 라플라스: 시그니처의 버스터가 첫
      소비자이고 라이더가 쓰던 `hero_vision_max_stack_gate`를 **그대로 재사용**하므로
      한 카운터가 두 경로를 답한다. 닫힌 틱은 무타입이 아니라 자기 무기의 전달
      타입이 되고 DEF를 문다.
- [x] **측정 — 두 결함은 반대 방향이었고 버킷 쪽이 컸다.**
      신규 `scripts/measure_laplace_buster_typing.py`(DEF 8,000 · Mint 낀 덱):
      게이트 **−3.08%** · 전달 버킷 **+11.85%** → 순 **+8.40%**(덱 총합 +4.33%).
      틱 `true` 519 / `projectile_explosion` 170.
- [x] **★ 스윕은 이 변경에 대해 「101슬러그 중 0개 변경」을 냈다 — 맞지만 무의미하다.**
      스윕 보스가 `enemy_def=0`이고 티어3 셸에 타입 버킷을 주는 유닛이 없어
      **구조적으로 못 본다.** [[sweep-boss-has-no-def]]의 두 번째 사례 —
      **타이핑·명중률·DEF 변경은 스윕으로 판정하지 말 것.**
- [ ] **`laplace-ultimate-hero`도 같은 축을 가졌는지 미확인.** 그녀는 RL이고
      변형 세그먼트를 갖는데, 트루뎀 전환이 있는지는 안 봤다. 있으면 같은 두 축
      문제가 붙는다.

### 오버로드 옵션 상세 모드 (2026-08-15)

- [x] **니케 풀 탭이 인게임 장비 화면을 되돌려 보여준다.** 필터 툴바 아래
      「오버로드 옵션 상세」 버튼 하나로 탭 전체가 합계 줄 → 부위별 2×2 격자로
      바뀐다(좌상 머리·우상 몸통·좌하 팔·우하 다리, 부위마다 옵션 행 셋, 롤이 없는
      행은 `—`로 자리 유지). 이름은 요약의 축약(`우코`)이 아니라 인게임처럼
      `[우월코드 대미지]`. 강조는 인게임과 같이 **단계**로 — 15단계는 행 반전,
      12~14단계는 수치 강조(색 선택 근거는 `docs/decisions.md`). 그 표기 규칙은
      화면 어디에도 안 적혀 있어서 버튼 옆 `HelpTip`이 답한다. 버튼은 눌린 쪽이
      흰색·안 눌린 쪽이 검은색으로 채워지고 테두리는 양쪽 다 흰색이며, 상태는
      `aria-pressed`가 따로 말한다(색만으로는 상태가 없는 사람이 있다). 선택은
      localStorage에 남는다. 백엔드가 롤마다 `level`·`index`를 싣게 됐다
      (`assemble_overload`). 착륙 시점 트렁크 기준 백엔드 2411 → **2415 passed**,
      프론트 884 → **922 passed** (73 files), 타입에러 0·lint 에러 0. Vite dev +
      FastAPI를 Playwright로 띄워 실제 브라우저에서 네 경로(전체 롤 / 부분 롤 /
      오버로드 없음 / 롤 없는 옛 로스터) 모두 확인했다.
      15단계 수치는 반전된 바탕에서 `--accent`가 대비 3.39로 AA 미달이라 신규 토큰
      `--accent-on-light`(`#cc0010`, 5.25)를 쓴다 — 12~14단계는 어두운 행이라
      `--accent` 그대로(4.58). **방어력 롤이 드롭되면 옵션 행에 구멍이 생기는 것이
      상례**라 `gearRows`가 행 번호대로 자리를 지킨다.
...
- [x] **차지 모션 딜레이 — 36정 전부가 답을 가졌다, audit exit 0 (2026-08-15).**
      마지막은 신데렐라였다. 그녀의 0은 클록이 아니라 `input_type`·`rate_of_fire`에서
      **유도한** 값이었는데, 한 탄창 37발을 재서 확인됐다: 발-발 간격 **19.583프레임**
      (36간격, sd 0.937)이 **22프레임 멈춤 가설을 15.5σ로 기각**한다. 멈춤이 있었다면
      그녀는 1.095x → 1.046x였을 것이라(평타 503→461발) **이 판독은 캘리를 1.0에서
      멀리 두는 쪽을 택한 것**이고, 그래도 되는 이유는 판독이 그렇게 말하기 때문이다.
      **딜 불변**(모델이 이미 0), `INFERRED` → `NO_CHARGE_MOTION_DELAY`.
      `docs/measurements/cinderella-charge-interval.md`.
- [ ] **★★★★ 답 나옴 (2026-08-16): 양자화는 엔진이 맞고 리베랄리오의 데이터
      `chargeTime`이 틀렸다 — 1.5초가 아니라 88프레임(1.4667초).** 같은 날 확인 둘이
      이 조사를 닫았다.
      - **브래디 판독 오프셋은 없다** — Fienn: 「1966프레임에서 차지게이지 0%」.
        1965·1966이 0%이고 1967이 최초 양수인데 RAW에 실린 값이 **1967**이므로
        `bready-charge.md`의 정의가 맞다. 차지 57f 유지, `⌊base×(1−s)⌋` 모델은 다시 기각.
      - **흑련 차분이 리베랄리오 base를 쟀다** (본계정, 흑련 차속 없음, **편성만 바꾼
        측정이라 오버로드 불변**): with 리베 32.4219f(n=64) · without 43.5679f(n=81)
        → **grant 11.1460 ± 0.1742 프레임**. Calm Depths가 그녀 base의 12.74%이므로
        base를 직접 준다.

        | base | grant σ | 차속 6% → 엔진 | 간격 실측 83.000과 |
        |---:|---:|---:|---:|
        | 87 | −0.36 | 82f | 6.3σ ❌ |
        | **88** | **+0.37** | **83f** | **0.0σ** ✅ |
        | 89 | +1.11 | 84f | 6.3σ ❌ |
        | 90(데이터) | +1.84 | 85f | 12.6σ ❌ |

        **`base=88f`만 두 독립 측정을 동시에 만족한다.** grant 단독으로는 87.5±1.4f로
        87·88이 동등하지만 **간격 실측이 88f를 못박는다.** 선례 둘(스칼렛·레이븐)과
        같은 형태의 수집 데이터 오류다.
      - **★ 결정: 엔진은 1.5초를 그대로 쓴다 — 반영 보류** (Fienn, 2026-08-16:
        「실측으로 얻은 결론이 공식제공 데이터를 갈아엎는 건 조심스럽다」). 두 측정이
        수렴하나 **둘 다 간접**이고(grant는 `12.74%`라는 또 다른 수집 값에 의존,
        간격은 양자화 모델 전제 위), **캘리브레이션도 같은 방향**이다 — SR이 이미
        **1.122x 과대**인데 85f→83f면 발수가 약 2.4% 늘어 **더 과대**가 된다.
        근거·재검토 조건은 `docs/decisions.md`. **따라서 리베랄리오는 실측과 12.6σ
        어긋난 채로 남는다 — 모르는 게 아니라 보류한 것이다.**
      - **남은 것:** ① **네온**은 같은 형태로 base 59f면 0.7σ가 되지만 **grant 같은
        독립 측정이 없어 추정으로만** 둔다(엔진 50f · 실측 49.115f, 5.5σ로 역시 어긋난
        채). ② **신데렐라 −2.09%**는 floor(`rate_of_fire`) 경로라 이 축과 별개.
        ③ **재검토 조건**: 리베랄리오 차속 0 판독, 또는 같은 형태의 수집 `chargeTime`
        오류가 다른 유닛에서 **독립적으로 또** 확인될 때(선례 둘 — 스칼렛·레이븐).
      - **죽은 가설 열 개, 다시 쫓지 말 것:** 0.01초 격자 · 차속 큐브 · 편성/버스트 ·
        계정 전역 · 굴림 오독 · 표기값 오류 · 편성 스킬 원문 · 프레임 드랍 ·
        판독 오프셋 · `input_type`별 양자화. 각 기각 근거는
        `docs/measurements/charge-speed-scaling.md`.
      - 아래는 그 답에 이르는 경위다.
- [ ] **~~차지속도 판독이 요구하는 총량이 엔진보다 정확히 +2%p 크다~~ (경위).**
      신데렐라의 2.7σ가 판독 편향인지 가르려고 넷을 쟀고,
      **편향은 기각**됐다. 갈린 축은 판독이 아니라 **차지속도**다:
      - **차속 0인 판독 셋이 전부 맞는다** — 아니스: 스타 **±0.000f**(n=16,
        그녀 버프는 차지를 0.7초로 **고정**해 스케일링을 안 탄다) · 라플라스UH
        첫 차지 **60프레임 정각 4연속** · 밀크 차지 +0.61%(반대 방향).
      - **차속을 타는 쪽만 빠르다** — 리베랄리오(6.09%) **8.2σ** ·
        네온(17.03%) **5.5σ** · 신데렐라 2.7σ. (이 σ는 **엔진 기준**이다 —
        문서가 처음 적은 6.0/4.2σ는 연속값 `차지 × (1 − 차속)`으로 잰 것이고
        엔진은 정수 % 반올림 + 프레임 내림을 더 밟는다. 어긋남은 더 크다.)
      - **★ 형태가 정해졌다 — 연속값이 아니라 정수 프레임으로 역산하면 답이
        유일하다** (`scripts/solve_charge_speed_from_reading.py`). 실측이 가리키는
        정수 프레임 하나가 그것을 사는 차속 구간을 유일하게 정한다: 리베랄리오
        83f ← [7.778%, 8.889%) → 정수 **8%**(엔진 6%), 네온 49f ←
        [18.333%, 20.000%) → 정수 **19%**(엔진 17%). **둘 다 정확히 +2%p**이고,
        차지가 1.0초·1.5초로 다른데 같은 **%p**가 맞으므로 비례 인자(×1.33 대
        ×1.12)도 고정 초 오프셋(2프레임 대 1프레임)도 아니다 — **가산 상수**다.
      - **즉 `charge_time_with_speed`는 무죄다.** 공식도 격자도 맞고(브래디가
        13.5σ로 확정), 엔진이 그 두 유닛에게 **넣어 주는 차속이 2%p 모자란다.**
      - **출처 후보 넷이 죽었다.** ① **차속 큐브 — 기각**(Fienn, 2026-08-16: 안 꼈다).
        ② **편성 아군 버프 — 기각**: 공통 편성 아니스·마스트·앵커 중 아니스의 차속만
        존재하는데 **`"self"` 대상**이고(`anis_star.py`), 나머지 넷은
        `charge_speed_percent`를 아예 안 다룬다. 게다가 리베랄리오는 차속 면역이다.
        ③ **계정 전역(연구·도크 등) — 기각**: 차속 0인 밀크가 60.364f로 그대로고
        브래디도 0%p다. ④ **굴림 분해 오독 — 기각, 그리고 이것이 답을 지목한다**:
        실측 프레임을 내는 굴림 **합계** 범위가 리베랄리오 **7.13~8.89%** ·
        네온 **17.27~20.48%**인데 문서에 적힌 6.09% · 17.03%는 **둘 다 밖**이고
        대조군 브래디만 안(4.55~6.61%)이다. 분해를 어떻게 골라도 안 맞으니
        **합계 자체가 달라야 한다.**
        ⑤ **측정 당시 오버로드가 문서 값이 아니었다 — 기각**(Fienn: 「차속 수치는
        측정 때 전달한 값이 맞다」). 로스터가 네온을 9.47%로 들고 있는 것은 **로스터가
        낡은 것**이지 판독 조건이 흔들린 게 아니다(재동기화 대상). ⑥ **편성 유닛의
        스킬 텍스트 — 기각**: 원문에 마스트·앵커는 `charge`가 **0회**, 아니스는
        「Charge time is fixed at 0.7 sec」로 퍼센트가 아닌 고정이다(걸렸다면 42f).
      - **★★★★ 축은 `input_type`이다 — 신데렐라의 미해결이 같이 닫힌다 (2026-08-16).**
        Fienn: 「직전 재측정은 단독편성이었음」 → **리베랄리오는 단독에서도 83.000f**라
        아래 「단독 대 편성 2×2」는 **철회**. 대신 어긋나는 유닛을 늘어놓으니:

        | 유닛 | `input_type` | 판독한 것 | 엔진 | 실측 | 어긋남 |
        |---|---|---|---:|---:|---:|
        | 브래디 | **UP** | 차지시작→발사 | 57 | 57.000 | **0.00%** |
        | 프리카 | **UP** | 차지시작→발사 | 57 | 57.000 | **0.00%** |
        | 리베랄리오 | **DOWN_Charge** | 발-발 | 85 | 83.000 | **−2.35%** |
        | 네온 | **DOWN_Charge** | 발-발 | 50 | 49.115 | **−1.77%** |
        | 신데렐라 | **DOWN_Charge** | 발-발 | 20 | 19.583 | **−2.09%** |
        | 아니스 | DOWN_Charge (차지 **고정**) | 발-발 | 42 | 42.000 | 0.00% |

        **차속을 타는 DOWN_Charge 셋이 −2.07%(폭 0.58%p)로 모이고 UP 둘은 0이다.**
        단일 비례 −2.06%가 셋을 0.24프레임 이내로 맞추고 **신데렐라는 0.003f**다 —
        그리고 그 −2.09%는 `cinderella-charge-interval.md`에 **「floor 자체가 2.7σ
        빠르다」는 미해결**로 적혀 있던 바로 그 값이다. **이것이 「+2%p 가산」보다
        낫다: 신데렐라는 floor에 걸려 차속 퍼센트 경로를 안 타므로 가산으로는
        건드릴 수 없다.** 커뮤니티도 같은 것을 말한다(arca 175948581, 「영」:
        「차지류가 미약하게 **서브프레임 단위로 선차지**가 있어서」) — 누른 채 반복하는
        `DOWN_Charge`는 발사 모션과 다음 차지가 **겹칠** 자리가 있고 `UP`은 없다.
      - **★ 조건이 좁혀졌다 — 「`DOWN_Charge` **이면서** 차속을 탄다」 (Fienn 정보,
        2026-08-16).** Fienn: 「딜레이 없는 니케들은 발사와 동시에 다음 차지가 진행돼.
        n번째 발사와 n+1번째 차지시작이 **동일 프레임**에 담겨 있어」. 따라서
        **발-발 간격이 곧 차지 시간**이고 **판독 방식 축은 죽는다**(게이지를 봐도 같은
        값). 내가 적었던 「겹쳐서 간격이 차지보다 짧다」도 틀렸다 — 「간격 = 차지 −
        1프레임」은 리베랄리오가 **6.3σ로 기각**한다. **차지 자체가 짧다.**
        그리고 **라플라스UH 첫 차지가 네 번째 대조군**이다: `DOWN_Charge`인데
        **차속 0**이고 차지시작→발사가 **60프레임 정각 4연속**으로 정확하다. 즉
        면제 넷은 UP 둘 · 차속 0인 라플라스UH · 차지가 **고정**이라 경로 밖인 아니스이고,
        어긋난 셋은 전부 **DOWN_Charge + 차속 적용**이다. `input_type`만으로도
        차속만으로도 부족하고 **둘 다** 필요하다.
      - **★ 반영 전 충돌을 먼저 풀 것: 이 −2%를 넣으면 캘리브레이션이 나빠진다.**
        DOWN_Charge 5정 중 넷이 실기록 덱에 있어 차지가 2% 짧아지면 딜이 오르는데,
        신데렐라는 이미 「반영하면 1.12x로 **더** 과대」로 기록돼 있다. 판독이 이기지만
        (판독이 정하지 비율이 정하지 않는다) **과대의 다른 원인이 같이 있다는 뜻**이다.
      - **Fienn의 프레임 드랍 가설은 리베랄리오에 대해 기각**(메커니즘은 실재 —
        「30프레임에서 모션 짧아지는 건 개노 오피셜 버그」 — 이나 ① 같은 단독 조건의
        브래디·프리카가 0.00%이고 ② 5인 영상의 아니스가 **풀버스트 구간에서**
        42.000f로 정확하다). **네온은 5인 판독뿐이라 아직 열려 있다**(실험 B).
      - **리베랄리오 차지 데이터 1.5초는 확증됐다(5.96σ)** — 88프레임이면 그녀의
        Calm Depths가 주는 양이 0.18685초여야 하는데 흑련 실측이 0.19019±0.00056이다.
      - (아래는 철회된 2×2 기록) **~~축이 확정됐다 — 단독 대 편성~~ (2026-08-16).**
        Fienn 재측정 1차(리베랄리오 n=35, 분포 81×2·82×9·83×11·84×13)가 **평균
        83.000**으로 첫 판독(83.174)을 재현했고 **85f는 여전히 0회**다. 그리고
        **프리카가 네 번째 칸을 채웠다** — `prika-charge.md`는 **단독 편성** 4.92%
        1.00초에서 57.0000±0.1606을 재 뒀고, 가산 +2%p라면 56f여야 하므로 그 판독이
        이미 **6.2σ로 기각**하고 있었다.
        ...
        차지 시간(1.0/1.5초)도 차속 값(5/6/17%)도 섞여 있는데 **갈리는 축은 편성
        하나**다. 게다가 리베랄리오 RAW의 시퀀스가 **길이 8.3초 · 시작 간격
        16.8~21.0초**로 **풀버스트 10초·주기 20초에 대응**한다 — 판독이 FB 구간만
        잰 것으로 보이고, 엔진의 차지는 FB 안팎이 같으므로 데이터가 말하는 것은
        **게임이 풀버스트 중 차지를 빠르게 한다**는 것이다. **엔진에 그 항이 없다 —
        유력한 신규 엔진 갭.**
      - **남은 질문 하나: 「풀버스트 중에만」인가 「아군이 있으면 항상」인가.**
        단독 판독 둘이 **버스트를 못 쓰는** 편성이라 두 축이 겹쳐 있다.
        `charge-speed-scaling.md`의 **실험 A**가 가른다 — 기존 5인 영상의 **FB 밖**
        구간을 읽기만 하면 되고(새로 찍을 필요 없음), 85f면 FB 한정 · 83f면 상시다.
      - **★★ 위 판정은 평균에 기대고 있고 `bready-charge.md`가 그걸 하지 말라고
        적어 뒀다.** 분포로 보면 리베랄리오는 `81×2 · 82×5 · 83×3 · 84×13`으로
        **85f가 0회**(최빈 84, 위쪽이 잘림)이고, 네온은 `48×7 · 49×9 · 50×10`으로
        참값 49에서 **꼬리가 대칭**이다. 같은 영상의 아니스가 `41×2 · 42×12 · 43×2`로
        대칭인 것이 「판독 오차는 양방향」임을 보인다. **분포 품질로는 네온이 깨끗하고
        리베랄리오가 지저분하다** — 굴림 합계 여유 기준의 반대다.
      - **★ 앞으로 차지 판독을 받을 때는 인게임 오버로드 표기값·큐브·FB 안팎을 같이
        받고, 간격은 평균이 아니라 분포로 남길 것.**
      - **★ 격자를 다시 의심하지 말 것.** 커뮤니티의 「0.01초 단위 반올림」은
        브래디 실측이 **이미 13.5σ로 기각**했다(`bready-charge.md`). 게다가 그
        격자로 바꿔도 어긋남의 22~23%만 회수돼 **닫히지도 않는다.**
      원본: `docs/measurements/charge-speed-scaling.md`.
      - **★ 앞서 여기 적혀 있던 「floor를 가진 넷을 재서 가른다」는 성립하지
        않았다.** floor는 차지가 그 아래로 내려가야 물리는데 넷 중 셋은 구조적으로
        못 내려간다(아니스 42f/floor 30f · 리베랄리오는 **차속 면역**이라 90f/18f ·
        네온 60f/12f). 실제로 floor에 걸리는 건 **신데렐라뿐**(459/459). 그래도
        그 판독들이 차지를 재면서 위 축을 갈라 줬다.
- [x] **라플라스: 얼티메이트 히어로 — 판독 계정이 Lv1이었다, 엔진은 그대로 (2026-08-17).**
      스택당 차속은 **레벨 의존**이고 `description_value_02`가 **Lv1 = 5% ·
      Lv10 = 10%**다. 부계정의 그녀가 스킬레벨 전부 1이라 실측이 표기의 절반으로
      읽혔다(Fienn 확인). **엔진은 최대 레벨을 시뮬레이션하므로 Lv10 값 =
      한 발당 10% = Warm Up 빌드 4.0초를 그대로 유지한다** (Fienn 결정).
      독립 확인 둘: 스택당 실효 7.08/5.63/5.56/5.14%가 Lv1의 5%에 앉고, 실측 빌드
      약 4.4초가 Lv1 램프 4.50초와 0.1초 차이다(Lv10 램프는 4.00초).
      **변형 주기도 같은 날 닫혔다** — 16.1초는 **사격장 보스가 잠깐씩 사라져 연사가
      끊긴** 구간을 포함한 값이었다. 연속 사격 구간만 재면 한 바퀴 **11.883초**이고,
      그 판독이 변형 연사를 **초당 24발**(무기군 SMG 상수 20이 아니다)로, 무기 변경
      모션을 37프레임으로 정했다. 엔진 반영 후 그녀 **+8.56% / 덱 +3.22%**.
      `docs/measurements/laplace-uh-transform-loop.md`.
      **판독 재측정은 이 질문엔 불필요**(오염 가설의 근거였던 어긋남이 설명됐다).
      아래 옛 분석은 죽은 가설 기록으로 남긴다:
      - **원문이 메커니즘을 확정한다:** skills[0] 2번 불릿 —
        「풀차지 공격 시 자신에게 **Warm Up: 차지속도 ▲10% 지속, 5중첩**」,
        5스택에서 스택을 제거하며 변형(`_02=10` · `_03=5`). **엔진에는 이 버프가
        없고**, 인코딩은 그 구간을 「Warm Up 빌드 **4.0초**」 통짜 상수로 근사했다
        (실측 빌드 구간 약 **4.4초**).
      - **실측 스택당 실효가 ~5%로 읽혔다**(7.08 / 5.63 / 5.56 / 5.14%). 당시엔
        원문 10%의 절반이라 어긋남으로 봤는데, **`_02`가 레벨 의존이고 Lv1이
        정확히 5%**였다 — 채점 기준이 틀렸던 것이다. 그때 세운 가설 셋(Warm Up만
        다르게 동작 · 스택형 차속이 비합산 · 스택 시점 오독)은 **전부 죽었다.**
      - 변형 **주기**는 아직 열려 있다 — 차지 시작 간격 평균 **16.1초** 대 엔진
        **12.5초**. 변형 탄약 120발(`_05`)도 스택 상한 5(`_03`)도 레벨 무관이라
        스킬레벨이 이쪽은 설명하지 못한다. `[Max Ammo Increase]` 차이가 남은 후보.
      - **프레임드랍 오염 가설도 같이 죽었다** — 그 가설의 근거가 이 어긋남이었고,
        판독이 Lv1 데이터와 0.1초 안에서 스스로 설명된다. 첫 차지 60프레임 정각
        4연속이라는 반대 증거도 그대로다.
- [x] **차지 모션 딜레이 17유닛 — 16은 민감도로, 밀크는 실측으로 (2026-08-15).** **밀크 실측이 stand-in을 확인했다**: 사격장 프레임 판독
      9회 평균 **21.889프레임**(sd 1.27)이 정수 격자의 **22프레임**에 앉고,
      stand-in과 **−0.51%** 차이다. 판독이 스스로를 검산한다 — 차지 60.364f
      (무기 파일 1.00초)와 합치면 82.253f로, 실측 발-발 간격 82.556f와 0.303f
      차이다. **딜은 한 자리도 안 움직인다**(값이 같고 지위만 추측 → 측정으로
      바뀐다). `docs/measurements/milk-blooming-bunny-charge.md`.
      - **stand-in 22프레임의 근거가 셋이 됐다** — 브래디(SR, 49판독) · 센티(RL) ·
        밀크(SR, 9판독). 그리고 밀크는 **stand-in을 짊어졌다는 이유로 측정된
        유일한 유닛**(민감도 1위 3.07%)이라, 가장 민감한 유닛이 stand-in 위로
        돌아온 것이 나머지 16을 accepted로 둔 결정의 가장 강한 증거다.
      - audit은 이제 **15 timed · 0 assumed · 16 accepted**이고, 남은 질문은
        `cinderella`(inferred 0 — 아무도 그녀를 직접 재지 않았다) **하나**다.
      - 백엔드 **2415 → 2422 passed / 3 skipped**, 캘리 **1.050x · 19/25 불변**.
      - **경위 — 16을 먼저 닫은 방법:**
      「멈춤이 있는가」는 `input_type`이 닫았고(17유닛 전부 `UP`), 남은 값 질문은
      **신규 `scripts/measure_charge_delay_sensitivity.py`가 16개를 닫았다** —
      실측 딜레이가 실제로 걸치는 범위(0.34~0.43, 잰 14유닛의 양끝)에서 각 유닛의
      딜이 얼마나 움직이는지 재니 **전부 2% 미만이고 열하나는 1% 미만**이다.
      stand-in이 그 범위 **중앙 근처**라 실제 오차는 그 절반쯤이고, 배분 탐색 자체의
      눈금 폭이 **±5%**라 추천을 못 가른다. 그래서 16유닛은
      `registry.STAND_IN_ACCEPTED_CHARGE_MOTION_DELAY`로 옮겨 **stand-in을 답으로
      받아들였다**(Fienn 결정) — 딜은 한 자리도 안 움직인다(값이 같은 집합을 둘로
      나눈 것뿐, 캘리 **1.050x·19/25 불변**).
        그때 남은 실측 대상이 `milk-blooming-bunny` 하나(3.07%)였고, audit은
        「stand-in (accepted)」를 별도 보고하며 **exit 코드에서 빼도록** 고쳤다 —
        항상 빨간 도구는 읽히지 않으므로.
      - **음수 둘**(`diesel-winter-sweets-intro` −0.13% · `laplace-signature` −0.15%)은
        **비단조**다 — 딜레이가 길어져 발사 위상이 풀버스트 창에 대해 밀린 결과이고,
        택티컬 베어 환급과 같은 형태다. 이득이 아니다.
...
- [x] **mihara-bonding-chain 0.895x — 사격장 3회로 닫혔다: 모델 결함이 아니다
      (2026-08-15).** 계수·스택 캡·버프 곱·평타가 전부 0.3% 이내로 맞고, 남은 것은
      시간 축과 실기록 배분 판독뿐이다(아래 상세). 그녀 딜은
      ...
      **확인된 것 (2026-07-31):**
      ...
      - **파츠 가설 기각 (Fienn, 2026-07-31).** 원문이 타겟을 복수형으로 쓰지만
        ...
        모델링할 이유가 없다.
      **사격장 실측 완료 (2026-08-15) — 계수는 전부 맞았다.** 1인 사격장에서
      실제 ATK 542,924(표기 389,164 × 오버로드 39.51%, **표기 ATK에는 오버로드
      공격력증가가 안 들어 있다**) 기준으로 체인 어택 **271,738** · 지속딜 1스택
      **136,140** · 10스택 **1,361,403**이 전부 **1.0000x**로 맞는다. 스택 선형성도
      10.0000. 따라서 **부족분 100.7M은 계수에 없다** —
      `docs/measurements/mihara-ensnaring-chains.md`.
      **그리고 그녀는 이 덱에서 버스트를 못 쓴다.** `max_bursts` 캡을 완전히 풀어도
      0회다 — deck5의 Burst 3가 셋(네온·아크레인저·미하라, 전부 쿨 40초)인데 15개
      사이클 슬롯을 **네온 8 + 아크 7**로 둘이 가져간다. 기록의 「0회」는 제약이
      아니라 이 사실과의 일치이고, 그녀 딜은 **100% 패시브**(지속딜 58.6% + 평타
      41.4%)다.
      **기각된 가설 셋 — 다시 쫓지 말 것:** (1) 「FB 창 밖 지속딜 틱이 갱신 시점
      값으로 고정」은 그녀만 보면 0.895x → **0.998x**로 크기가 맞지만, 25유닛에
      같은 산술을 돌리면 아크레인저 1.050→1.330 · 신데렐라CW 1.237→1.285로 **이미
      과대인 쪽을 더 민다**. (2) 「체인 방출 타이밍」은 원문이 `at a specific timing`
      이라 인코딩이 가정한 게 맞지만, 충전이 1회뿐이라 **총 10발이 시각과 무관하게
      불변**(최대 이득 2.5M). (3) 「실제로는 버스트를 썼다」는 위 사실이 기각.
      **5인 편성 실측도 들어왔다 (같은 날) — 지속딜 경로는 전부 검증됐다.**
      싱크로 682 사격장, 실기록 덱5와 같은 다섯. 표기 스킬딜 **580,832**와
      **11,616,638**의 비가 **정확히 20.0000**이라 **스택 캡 20이 직접 확인**됐고
      (「필요 캡 23.6」은 캡으로 설명 불가), 버프 곱은 **실측 4.2664x vs 엔진
      4.2792x로 0.3% 차이**다. 이 대조는 정의를 맞춰야 성립한다 — 분모는 아군이
      없는 **self-only**여야 하고(엔진 t=1 틱을 그대로 쓰면 **에이드의 배틀스타트
      스쿼드 flat ATK**가 섞여 3.93으로 낮아진다), 분자는 **논크리**여야 한다
      (엔진은 크리 기대값이라 `base_crit_rate=0`으로 돌려야 판독과 같은 축).
      **평타 실측까지 들어와 이 조사는 닫혔다 (2026-08-15).** FB 밖 논코어
      **33,005** · FB 중 코어히트 **155,021**을 싱크로 배수 3.0812로 대조하면
      엔진이 **0.998x · 1.003x**다. **그녀의 단발 대미지는 이제 전 축이 검증됐다** —
      계수 · 스택 캡 · 버프 곱 · 평타 안팎이 전부 0.3% 이내다.
      - **부수 소득: 사격장에는 적정거리 보너스가 없다.** 같은 대조를 `mid`로
        돌리면 1.298x · 1.123x로 둘 다 어긋나고 `None`에서만 맞는다. **사격장
        판독을 엔진과 대조할 때 밴드는 `None`** — 실기록 보스가 `mid`라고 그
        설정을 가져오면 안 된다.
      - **함정 하나 (내가 밟았다): 창 안의 값은 최빈값을 써야 한다.** 버프가
        감쇠해 한 구간에 값이 여럿 나오는데(FB 밖 1,556발 중 1,436발이 10,692)
        `max()`를 뽑으면 「엔진이 1.88배 과대」라는 잘못된 결론이 나온다.
      **남은 것은 시간 축(발수 8,162 · 틱 179)과 실기록의 유닛별 배분 판독뿐이고,
      둘 다 사격장으로는 못 잰다.** 덱5는 **전체로 1.013x**인데 그 안에서 그녀만
      0.895x라, 미달과 덱메이트의 과대가 상쇄되고 있다 — `raid_record.py`가 이미
      적어 둔 「25유닛 합계 34.896B vs 기록 총합 34.768B, 0.37% 어긋남」과 같은
      성격이다. **총딜의 2.8%짜리 유닛이고 모델은 맞았으니 여기서 닫는다.**
...
- [x] **게이지 충전이 덱 속성이 됐다 — 해소 (2026-08-22).** 게이지가 보스 상수가
      아니라 그 덱의 발사 타임라인에서 계산되고, `simulate_raid`의 고정점이 그것을
      **세 번째 수렴축**으로 푼다. **착륙 내용(채움원·수렴축·남는 보류 셋)은 한 곳에만
      적는다** — `docs/engine-gaps.md`의 「버스트 게이지 충전」 행. 결정 넷과 그 대가는
      `docs/decisions.md`(2026-08-22).
      **값의 근거(실측, 2026-08-05)는 그대로 유효하다:** CDR 7.48초 유닛이 든 덱을
      게이지까지 최대한 빠르게 컨트롤하면 **15번째 풀버스트가 t≈179**다. 게이지가
      병목인 동안 사이클 간격은 CDR과 무관하게 `FB 10.0 + 게이지 + 티어갭`이므로 그 한
      시각이 값을 유일하게 정한다: `143.0 + 15g = 179` → **g = 2.4**.
      `test_default_gauge_reproduces_the_measured_fifteenth_full_burst`가 이 산술을
      못박는다. **2.467부터는 절벽** — 15번째가 전투 밖으로 나가 사이클을 통째로 잃는다.
      **상수는 안 지웠다 — 뜻이 바뀌었다.** 2.4는 이제 답이 아니라 고정점의 **시드**다.
      덱이 아직 자기 채움 시간을 안 낸 첫 패스가 여기서 출발하고, 반복이 그 덱 자신의
      값으로 옮겨간다. 그래서 위 실측과 그 테스트는 지금도 그대로 성립한다.
      **틀렸던 전제 — 지우지 않는다.** 이 항목은 오래 「실제 게이지는 **가한 대미지**로
      찬다」고 적고 있었고, 그래서 「대미지에서 채움 속도를 유도하는 모델이 필요한데
      순환 의존이라 별도 설계 건」으로 미뤄 두었다. **그 전제가 틀렸기 때문에 접근이
      처음부터 다른 곳을 팠다** — 실제로 필요했던 것은 대미지 모델이 아니라 「언제 몇
      번 때렸나」뿐이었고, 엔진은 그것을 이미 갖고 있었다. 아래 정정이 그 내력이다.
      두 실측이 어긋났던 것(볼륨 편성 180초에 **14회**, 위 CDR 편성 **15회**)도 이제
      이상하지 않다 — **덱마다 다른 값**이 정답이었고, 상수 하나는 빠른 쪽을 따르느라
      볼륨 편성을 한 사이클 많게 모델하고 있었다.
      `burst_gauge_fill_speed_percent`(Anis: Star)의 "의도된 미구현"도 같은 뿌리였고,
      같이 소비자를 얻었다.
      > **채움 규칙이 확정됐다 (2026-08-21) — 위 「대미지에서 유도」는 틀렸다.**
      > 게이지는 **대미지와 무관**하다(코어 히트조차 무관). **타격 수 × 무기별
      > 상수**이고, 그 상수는 `shot_detail.burst_energy_pershot`으로 **데이터에
      > 이미 있다**(무기군별 500~29,000, 56배 차이 · 풀차지면
      > `× full_charge_burst_energy/10000` · 펠릿은 개별로 세고 빗나가면 0).
      > **게이지 총량 = 500,000**을 단독편성 6유닛이 독립적으로 확정했다.
      > 엔진은 발사 타임라인을 이미 정확히 갖고 있으므로 남은 순환은
      > 사이클 길이 ↔ 발수뿐이고 1패스 반복으로 닫힌다.
      > (실제로는 1패스로 안 닫혀서 **고정점**이 됐다 — 위 참조.)
      > **상수를 올리는 우회로는 죽었다** — 2.4→6.0에서 실측 5덱의 순위가 한 번도
      > 안 바뀌고 덱2가 오히려 가장 둔감하다(−18.4%). 덱마다 다른 값이어야 한다.
      > 실측 덱2는 3.7~4.0초 · 덱3은 2.55~3.5초로, **2.4초는 실제의 절반**이다.
      > 원본 판독: `docs/measurements/burst-gauge-fill.md`
      > (여기 「남은 미해결」로 적혀 있던 셋 — 덱3 1.8배 과대 · 헬름 · 헤비암즈 —
      > 은 전부 그 파일 안에서 닫혔다.)
- [x] **게이지 착륙 후 열린 결정 둘 — 둘 다 Fienn이 2026-08-22에 승인했고 착륙했다.**
      (「시간 비용 늘어나는건 일단은 허용할게. 히스테리시스도 추가하자.」)
      - **① 비용 승인 — 받았다.** A1 승인 당시 추정은 **290초**였는데 실제는
        **693초(11.5분)**, **2.4배**다(측정 방법과 ms/패스 표는 이 문서 맨 위
        「버스트 게이지가 덱 속성이 됐다」). 근사(사이클 수 상한·조기 종료)는 안 얹는다.
      - **② 히스테리시스 — 착륙했고, 오늘 실덱에서는 한 번도 안 걸린다.**
        `simulate_raid`가 게이지 축의 **주기 2를 감지한 뒤에만** 사이클마다 느린
        쪽으로 감쇠하고(`gauge_oscillation_damped`), 두 표가 격자 한 칸보다 크게
        다르면 **감쇠하지 않고** 감지 사실과 폭만 남긴다(`gauge_oscillation_detected`
        · `max_gap_quanta`) — 그 덱은 예전대로 경고를 받는다. 처방으로 적혀 있던
        「새 값이 한 격자 이내면 현재 값 유지」는 **기각됐다** — 고정점을 향해 0.1초씩
        걸어 내려가는 덱이 첫 값에서 얼어붙어 1.0초 오차를 「수렴」이라 답한다.
      - **★ 그 과정에서 이 항목의 전제가 틀렸다는 것이 드러났다.** 「사이클 두 개의
        값이 격자 한 칸 0.1초씩 번갈아 뛴다」가 아니다. 주기 2인 것은 맞지만 모양이
        **대체로 한 칸인데 꼬리의 한두 사이클이 4~9칸(0.4~0.9초)까지 벌어지는** 것이고
        (셋 중 하나는 일곱 사이클 중 다섯이 정확히 한 칸이고 마지막 둘만 4칸·2칸),
        그 꼬리 때문에 **3덱 전부가 가드에 걸려 감쇠 경로는 0덱**이다(수치도 비용도
        착륙 전과 동일: 397 수렴 / 0 감쇠 / 3 정지, 최대 25패스, 4.59배).
        폭이 뒤로 갈수록 커지는 것은 접두사 안정화의 뒷면이다 — 이른 사이클이 0.1초
        밀리면 창 종료·재장전 위상이 밀려 뒤 사이클에서 한 매거진이 통째로 경계를
        넘나든다. 현재 폭은 `scripts/check_gauge_convergence.py`가 정지마다 찍는다.
- [ ] **⚠ 새로 열린 결정: 감쇠 가드를 넓힐 것인가 — Fienn의 판단 대기 (2026-08-22).**
      위 ②가 남긴 것이다. 오늘 안 멈추는 3덱은 꼬리가 4~9칸이라 감쇠가 안 걸리고,
      **가드를 그만큼 넓히면 셋 다 결정적인 답을 받는다.** 대가는 오차가 격자
      한 칸이 아니라 **0.9초**로 묶이는 것 — 이 기능이 사려던 성질을 그만큼 잃는다.
      리뷰의 정리는 이렇다: 「이 세 덱은 찾을 고정점이 없다. 넓히는 건 격자 인공물을
      결정론적으로 푸는 게 아니라 **실재하는 두 타임라인 중 하나를 고르고 질문을
      멈추는 것**이고, 그건 다른 기능이다.」 숫자를 움직이려고 구현자가 임의로
      넓히지는 않았다. 넓힌다면 폭을 무엇으로 정할지가 새 질문이다(관측 최댓값 9칸?
      덱마다 다른데?).
- [x] **신데렐라 다단히트 — 착륙 (2026-08-15). 고친 것은 불릿2 하나이고, 불릿1은
      건드리지 않는 것이 정답이었다.** Fienn의 표기값 실측(1스택·12스택 두 벌)이
      「10히트를 실제 착탄 시각으로 흩뿌린다」를 기각했다. 첫 히트가 **FB 진입
      0.95초 뒤**라 10히트 전부 FB 창 안인데도 **불릿1은 FB 보너스를 안 받는다** —
      두 불릿이 받는 배수 차가 1스택 **2.044** · 12스택 **2.085**로 일관되고,
      불릿1도 받았다면 1.0 근처여야 한다. 즉 **버스트 대미지는 캐스트 시점에
      확정**되고, 히트를 캐스트 시각에 기록하는 현행이 그 사실 그대로다.
      흩뿌렸다면 게임이 안 주는 보너스를 줬을 것이다.
      - **불릿2만 바뀐다:** 1회 넉 → **10틱**(`tick_count=10`·`tick_interval=0.2`·
        신규 `fire_delay=0.95`). 각 틱이 자기 시각의 아름다움 스택을 읽고, 시각이
        창 안이라 FB 보너스는 엔진의 창 검사가 준다. 엔진 확장은 `fire_delay`
        한 필드뿐이고 **없으면 0.0이라 기존 스펙 전부 불변**이다.
      - **캘리 1.043x → 1.050x · 19/25 불변.** 신데렐라 0.980x → **1.095x**,
        덱4 0.987x → 1.041x, RL 평균 1.038x → 1.057x. **나머지 24유닛은 한 자리도
        안 움직였다** — `fire_delay`가 opt-in이라는 배선의 확인이다.
        백엔드 **2410 → 2411 passed / 3 skipped**.
      - 골든: `test_glass_slippers_riders_read_the_stack_count_at_their_own_hit_time`이
        0스택 캐스팅 사례를 못박는다(라이더 10개 중 t=3.15/3.35/3.55/3.75 **넷만**
        대미지). 원본: `docs/measurements/cinderella-glass-slippers.md`.
