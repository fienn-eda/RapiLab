...
- [x] **니케를 별명으로 찾는다.** 「홍련: 흑영」을 흑련으로, 「리틀 머메이드」를
      ...
      규율로 관리하고, 이름과 같은 규칙(부분일치 + 초성)으로 본다. 키가 오타면
      그 별명은 조용히 아무것도 안 맞히므로 `nikkeAliases.test.ts`가 모든 키를
      실재 슬러그와 대조하고 별명 충돌도 막는다.
...
- [x] **유니온이 싱크로 레벨 스탯으로 잰다 — 레벨 400 보정은 솔로만의 규칙이다.**
      ...
      **남은 것:** 실기록 캘리브레이션(1.060x·18/25)은 솔로 기록으로 잰
