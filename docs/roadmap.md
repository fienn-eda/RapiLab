...
- [ ] **도로시: 세렌디피티의 Flash는 보류 유지 — 펠릿 카운터 갭 (Fienn 판정
      2026-08-07).** 설계문서가 그녀를 최대 수혜자로 적은 근거가 이 불릿인데
      (+98.18%가 Radiant Wings와 합쳐 110% 특이점을 넘는다), 트리거가 「펠릿 80발」
      이고 10펠릿 중 몇 발이 맞는지는 플레이 조건이다. **인게임에서 Flash가 몇
      초마다 터지는지 한 번 재면** 주기 상수 + per-shot 룰로 끝난다.
- [ ] **디젤: 윈터 스위츠(Highlight)가 어떤 덱에서도 버스트를 안 한다.** 명중 작업
      중 발견 — Noise Pollution(아군 명중 −100%/1초)을 인코딩했는데 E2E로 발동을
      한 번도 못 봤다. 그녀가 유일한 B3인 셸에서는 **풀 버스트가 180초 동안 0회**
      열리고(`burst_delay`의 `skip_cycles: 1`이 `cycle_index`를 못 넘긴다),
      티어메이트를 넣은 덱에서도 좌석이 매번 상대에게 간다(노와르·슈가·스칼렛:
      블랙섀도우·레드후드·맥스웰 전부). 2026-07-19 E2E는 그녀가 짝수 사이클에
      버스트한다고 기록했으므로, 그 사이에 좌석 규칙이 바뀌었을 수 있다.
      **그녀의 버스트 DoT(199.63%×9틱)와 받댐 디버프도 같이 안 걸린다** —
      명중과 무관한, 더 큰 문제다.
...
- [ ] **SMG가 이제 가장 높다 (1.145x).** MG 예열이 걷히면서 남은 최대 무기군 항.
      SMG에도 예열이 있는지는 미측정이고 MG 숫자를 빌려 쓰지 않았다 — 같은 방식의
      판독(단독 편성, 탄창 하나, 프레임+잔탄) 한 번이면 갈린다.
- [ ] ~~**고정 연사 무기(MG·SMG)의 실제 발사 수 실측 — 지금 가장 큰 단일 항.**~~ 2026-08-07
