...
- **버스트 게이지가 덱 속성이 됐다 (2026-08-22).** 엔진의 타이밍 축 하나가 통째로
  ...
  - **★ 사람의 결정 둘이 아직 열려 있다** — 아래 To-Do의 「게이지 착륙 후 열린 결정
    둘」 항목.
...
- [x] **게이지 충전이 덱 속성이 됐다 — 해소 (2026-08-22).** 게이지가 보스 상수가
      ...
      **틀렸던 전제 — 지우지 않는다.** 이 항목은 오래 「실제 게이지는 **가한 대미지**로
      ...
      > 은 전부 그 파일 안에서 닫혔다.)
- [ ] **⚠ 게이지 착륙 후 열린 결정 둘 — 둘 다 Fienn의 판단 대기 (2026-08-22).**
      엔진 갭이 아니다. 코드로 답할 수 있는 것은 이미 다 재 놓았고, 남은 것은
      「그 대가를 치를 것인가」다.
      - **① 비용 승인.** A1 승인 당시 추정은 **290초**였는데 실제는 **693초(11.5분)**,
        **2.4배**다(측정 방법과 ms/패스 표는 이 문서 맨 위 「버스트 게이지가 덱
        속성이 됐다」). 승인의 전제가 어긋났으므로 다시
        묻는다: 이 탐색 시간을 받아들일 것인가, 아니면 사이클 수 상한·조기 종료 같은
        근사를 얹을 것인가. **아직 답을 못 받았다.**
      - **② 히스테리시스.** 400덱 중 **4덱**이 고정점에 안 앉고 **주기 2로 진동**한다
        (사이클 두 개의 값이 격자 한 칸 0.1초씩 번갈아 뛴다). 진짜 고정점이 격자점
        둘 사이에 있으면 양자화된 사상에는 고정점이 **아예 없다** — 격자를 바꾸면
        어느 덱이 경계에 앉는지만 바뀐다(0.2/0.25로 확인). 처방은
        **「새 값이 현재 값과 한 격자 이내면 현재 값을 유지」, `simulate_raid` 4줄**이고
        오차는 0.1초로 실측 폭(덱3 ~1초)의 1/10이다. 고정점의 **의미**를 바꾸는 설계
        결정이라 구현자 권한 밖으로 두었다. 오늘 문제는 아니다(상한 32패스 안, 최대 25).
