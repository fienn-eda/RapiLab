...
- [x] **밀크 딜 약 +10.5%**(매거진 케이던스 축, 스톡 장탄 6·재장전 2초 — 등록 전
      `k=C` 고정 대비). Pierce 자체는 안 끊긴다(신규 테스트 2건이 못박는다).
...
- [x] 백엔드 **2561 passed / 3 skipped**(신규 테스트 없음, 검산 스크립트만 추가).
      프론트는 코드를 안 건드렸다(Task 6이 문구를 고친다) — 이 워크트리엔
      `frontend/node_modules`가 없어 회귀 실행은 생략(회귀 아님, 워크트리
      공통 함정).
