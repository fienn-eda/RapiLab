...
- [ ] **라플라스 ±시그만 남는다** — 본인 **+13.13% / 덱 +2.17%**(base) ·
      +2.57% / +0.63%(시그). 그녀는 Hero Vision에 **버프가 없고 게이트만** 있어서
      플래그가 닿지 않는다. 아래 엔진 항목이 선행 조건이다.
      감사 스크립트가 지금 **이 둘만 0이 아닌 값으로** 보고한다.
- [ ] **★ 엔진: 의미론이 자원이 아니라 버프에 달려 있다.** `lifetime_refreshes`는
      `ResourceBuff` 계단함수 경로(`raid_simulator.py:1871`)만 읽는다. 게이트 4-튜플
      (`:1200` 넉 · `:1216` 타이핑)과 스펙 딕트(`:1947` 지연 버프 · `:720` 조건부 FB
      델타)는 플래그를 못 받는다. **라플라스는 버프가 없고 게이트만 있어서 지금
      구조로는 고칠 방법이 자체가 없다.** `lifetime`/`lifetime_refreshes`를
      `ResourceSpec`으로 올리면 다섯 경로가 구조적으로 일치한다 — 설계 변경이라
      착수 전 Fienn과 상의.
