...
- [ ] **잔여 편차 0.87~1.35x의 정체를 가르지 못했다.** 위 수정 뒤에도 덱1은 0.87x(과소)
      덱2는 1.35x(과대)다. 두 갈래가 섞여 있고 분리되지 않았다: (a) 실기록이 **두 시즌 전**
      이라 그 사이 늘어난 투자만큼 시뮬이 높게 나오는 게 정상 — 즉 1.08x의 일부는 오차가
      아니다, (b) "실제 플레이가 코어를 얼마나 자주 맞히는가"는 여전히 미모델(적격 히트는
      항상 맞힌 것으로 계산). 가르려면 **최근 시즌** 기록이 필요하다.
- [x] **덱 데미지 내역이 총합의 22~72%만 설명하던 문제 — 완료 (2026-07-26).**
      세 번째 줄 **`Skill`**(시뮬레이터의 여덟 소스 중 burst·normal_attack이 아닌
      여섯 개의 합)을 추가해 **세 값의 합 = `total_damage`** 를 계약으로 고정했다.
      ...
      (덱1 스킬 7.65B vs 평타 4.43B vs 버스트 0.10B). `Burst: 0`은 그대로지만 옆에
      Skill이 있어 이제 "버스트 넉이 없는 덱"으로 읽힌다. 백엔드 1413 → **1415 passed /
