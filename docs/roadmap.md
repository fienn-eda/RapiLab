...
- **신데렐라 다단히트가 착륙했다 — 고칠 것은 불릿2 하나였고, 불릿1은 건드리지
  ...
  **`fire_delay` 한 필드**뿐이고 없으면 0.0이라 기존 스펙 전부 불변.
  ...
  - **같은 날 미하라 사격장 실측도 들어왔다 — 계수는 전부 맞다**(체인 50.06% ·
    지속딜 25.08%/스택 · 스택 선형성 10.0000, 전부 1.0000x). 부족분 100.7M은
    계수가 아니라 **버프 곱 또는 스택 수**에 있고, 가설 셋(FB밖 틱 고정 · 방출
    타이밍 · 버스트 누락)은 기각됐다. **표기 ATK에 오버로드 공격력증가가 안 들어
    있다**는 것도 이 판독이 확정했다. `docs/measurements/mihara-ensnaring-chains.md`.
...
- [ ] **mihara-bonding-chain 0.895x — 지속딜이 캡에 포화된 채 부족하다.** 그녀 딜은
      ...
      **남은 것은 평타와 시간 축 둘뿐이다.** 지속딜이 맞다면 실기록이 요구하는
      평타는 964M − 506M = **458M**인데 엔진은 357M이라 **0.78x**가 된다 — MG
      무기군이 1.164x **과대**인 것과 방향이 반대라, 맞다면 그것대로 새 사실이다.
      **다음 측정: 같은 5인 사격장에서 미하라 평타 한 발의 논크리 표기값**
      (코어 미명중/명중 각각 하나면 코어 배율까지 같이 갈린다).
      실기록 총딜의 2.8%짜리 유닛이라 우선순위는 여전히 낮다.
