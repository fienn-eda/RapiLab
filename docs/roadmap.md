...
- 브랜치: `worktree-result-view-and-saved-runs` — 결과 화면 정리와 결과 보관.
  ...
  **⑤ 결과를 이름 붙여 보관.** `Profile.savedRuns`에 `tab: 'solo' | 'union'`으로
  ...
  육안 확인에서 결함 하나를 잡았다: 보관 목록 상자가 `align-items: flex-start`
  때문에 내용 크기(428px)로 줄어, 펼친 보관물 안의 덱 그리드가 한 열에 갇혔다.
  `.saved-runs-group { width: 100% }`로 고쳐 1336px·3열이 됐다. **CSS는 앱을
  띄워야 보인다**는 것이 다시 확인됐다(Vitest는 `css: false`).
...
- [ ] **소다의 FB 확장을 인코딩할 것 — 보류 사유가 관측과 어긋난다 (2026-08-06).**
      Fienn 실측(덱 `tove-signature,arcana-fortune-mate,soda-twinkling-bunny,
      dorothy-serendipity,drake-signature`): **풀 버스트가 15초**이고 칩이
      **50→33→40/41→49/50→33으로 순환**한다(고갈이 아니다). 그녀는 B3 셋 중
      **격 사이클로** 버스트하고, 다른 B3가 연 사이클에도 평타 3회당 1스택이 쌓인다.
      같은 로테이션에 `FULL_BURST_DURATION_DELTA` +5초를 주입하면 시뮬이 그 궤적을
      스택 단위로 재현한다(엔진 그대로면 50→33→39→45→28→…로 고갈 — 실측과 다르다).
      - **순환에는 안정된 고정점이 있다**: 칩 ≥20 → +5초 → 창당 fill 9~10 → 격번으로
        17을 써도 2사이클 수지 +1~2 → 20 아래로 안 내려감. 자기일관적이라 **그 운영에서는
        상수로 접힌다.** 붕괴하는 것은 매 사이클 버스트할 때뿐(-17+9 = -8/사이클).
      - **미모델의 대가**: 이 덱 총딜 4.429B → **4.841B (+9.3%)**. 소다 혼자가 아니라
        덱 전원이 풀 버스트 창을 50% 더 갖는다.
      - 원문이 `Activates when entering Burst Stage 3. Affects all allies.`이므로
        **소다가 버스트할 필요가 없다** — 덱에 있고 칩이 임계 위면 누가 B3를 쏘든 걸린다.
        지금의 `FULL_BURST_DURATION_DELTA`는 「그 사이클을 연 B3」에서 읽으므로 이 모양이
        아니다. 설계가 필요하다(덱 보유 기반 + 칩 임계 조건).
      - 남은 미지수는 `docs/measurements/`의 「결론짓지 않는 것」절 — 특히 창당 fill이
        실측 8인데 시뮬 9~10인 15% 차이(소다의 발수 모델이 빠를 가능성).
