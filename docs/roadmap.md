...
- [ ] **②-c 앱이 인박스를 집어오게 배선 — 남음.** 프론트가 동기화 화면에서 폴링해
      `GET /api/sync-inbox`를 확인하고, 받으면 기존 후보 선택 → `assemble-roster`
      흐름으로 넘긴다. 기존 `useBookmarkletImport`(postMessage)는 개발용으로 남아
      있으므로, 둘 중 무엇을 유저에게 보여줄지도 같이 정해야 한다.
