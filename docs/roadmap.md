...
- [x] **★★ 두 번째 버그: 같은 스크립트가 이득 비율을 짧은 창(30초)으로도 재고
      있었다** — 리뷰에서 지적됨. 30초에서는 매거진 재장전 위상이 안 맞아 이득이
      5.56~9.62%로 진동한다(정상상태가 아니다). 600초 창도 확인해 보니 **참값이
      아니었다** — 600·900·1200·2400초에서 우연히 같은 10.79%를 내 수렴한 것처럼
      보였지만, 450초·4800초·120,000초·1,000,000초까지 스윕하면 10.76%로 계속
      미끄러진다. 근본 수정: 이 무기는 버프가 전투 내내 상수라 케이던스가 **첫
...
- [x] `docs/engine-gaps.md` · `docs/encoded-nikkes.md` ·
      ...
      갱신.
- [x] 검증: 백엔드 **2571 passed / 3 skipped** · 프론트 **949 passed / 74
      files** · 타입에러 0 (이 정정으로 변한 것 없음 — 기존 기준선 그대로).
