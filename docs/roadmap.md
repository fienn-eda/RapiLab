...
- **버스트 게이지가 덱 속성이 됐다 (2026-08-22).** 엔진의 타이밍 축 하나가 통째로
  ...
  - **★ 덱1 실측이 상수를 확증하고 정확도 갭 셋을 열었다 (2026-08-22).** Fienn이
    ...
    **F4가 주범이다 (2026-08-22).** 천장 논증이 「무기 밖 채움원이 반드시
    있다」를 부등식으로 확정했고, Fienn 판독(창 밖 착탄 1~3개)이 F1을 확정하되
    **크기가 갭의 1/3 이하**임을 보였다. 나머지는 **F4 — 아니스: 스타의 발당
    게이지 에너지가 27% 낮다**(같은 RL 스탯인데 단독편성 완충이 아니스 9발 ·
    디젤 14발, 디젤만 맞는다). 아니스를 2타로 놓으면 천장이 실측 요구에
    **1.008**로 닿는다.
    - [ ] **F4를 고치기 전에 갈라야 할 것**: 「2타인가 배율 373.675%인가」.
      게이지 축엔 같지만 딜 축엔 대미지 2회 대 1.37배로 다르다. 가르는 실측은
      아니스 **톡톡이 단독편성 완충 발수**(2타 **17발째** · 배율 **34발째**;
      Starfall +6% 포함한 값이다).
