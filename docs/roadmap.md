...
### 메이든의 Meditation 스택 — 모델은 고쳤고, 딜에는 안 닿는다 (2026-08-17, A7 1단계)
...
- [x] **⚠ 그런데 딜은 0.0000% 움직인다 — 이 스택엔 대미지 소비자가 없다.**
      per-stack 값을 **100배**로 키워도 그녀 딜도 덱 총합도 **한 자리도 안 변한다**(실측).
      원인은 순서다: Blessings Upon You의 **Max HP → ATK 환산은 `own_burst_activate`
      룰**이라 **버스트 사이클 안**에서 돌고, 이 스택은 **그 뒤에 도는 패스**가 낸다.
      독스트링의 「Blessings를 통해 딜에 닿는다」는 **두 빌더를 손으로 순서대로 부른
      유닛 테스트에서만 참**이었다.
- [ ] **새 갭: 버스트 룰이 나중 패스의 스탯을 못 읽는다.** A4/A8과 **같은 순서 계열**이다
      (자원은 샷 루프 뒤·페이즈 2 앞에 풀린다 → 대미지 속성엔 닿지만 **버스트 시점에
      읽는 값**엔 못 닿는다). 이게 열려 있는 한 **메이든의 Meditation과, A7에서 그녀에게
      갈 플로라의 +1은 둘 다 inert**다. 신데렐라·츠바이는 페이즈 2 소비자라 무관하다.
...
- [ ] **⚠ 막힌 지점 — 인게임 확인 필요 (Fienn).** 플로라의 「중첩 수 ▲1」이
      **신데렐라의 아름다움 · 메이든의 MP처럼 그 유닛이 자기에게 쌓는 스택에도 닿는지**가
      미확인이다. 츠바이의 크리율 스택은 명백한 「버프」라 어느 해석에서도 들어오지만,
      나머지 둘은 자원/자기 스택이라 갈린다.
      **확인 방법:** 플로라를 신데렐라(또는 메이든)와 같이 편성하고, 플로라 평타
      100발마다 그 스택 수가 1씩 튀는지 본다.
      **확인 전에는 배선하지 않는다** — 85회 × cap 12면 틀렸을 때 조용히 크게 틀린다.
