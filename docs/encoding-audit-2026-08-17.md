...
| | 건수 | 상태 |
|---|---|---|
| **실제 결함** | 3 | 수정 완료 (F1 noir · F2 quency · F3 mana) |
| **문서 결함** (코드는 옳고 서술이 틀림) | 7 | 6건 수정, 1건 기록만 |
| 원문과 일치 | 100 | — |

F3은 처음에 「원문만으로는 안 갈린다」고 판정 보류로 올렸다가, Fienn의 인게임
타임라인(2026-08-17)으로 확정되어 같은 날 고쳤다.
...
## F3. Mana — Metal σ가 사이클을 건너 이월되는 상태였다 ★수정
...
**Fienn의 인게임 타임라인 (2026-08-17)이 확정했다:**

```
전투 시작            -> σ ON
사이클 1, 마나가 버스트  -> FB 진입: σ 있음, 버프 발동, σ OFF
                     -> FB 종료: 이 창에서 캐스트함 -> σ 다시 ON
사이클 2, 다른 B3      -> FB 진입: σ 있음, 버프 발동, σ OFF
                     -> FB 종료: 캐스트 안 함 -> σ OFF 유지
사이클 3, 마나가 버스트  -> FB 진입: σ 없음, 버프 없음
```

즉 4번 불릿은 **Full Burst 종료 시점에 판정**되고, σ는 사이클을 건너 이월된다.
**버프는 그녀가 버스트한 사이클의 다음 사이클에 붙는다.**
`own_burst_fired_this_cycle()`은 정확히 **반대 사이클**에 주고 있었다 — 그녀의
버스트 딜(Fatal Error의 10틱 DoT 포함)이 게임이 다음 창에 주는 ATK 버프를
챙기고 있었다.

**영향.** 스윕의 tier-3 셸은 `liter + crown + blanc + helm` + 대상이라 **B3가
둘**이다(helm + mana). 그 셸에서 마나 **780.4M → 727.7M (−6.76%)**.
단일 B3 덱에서는 그녀가 매 사이클 캐스트하므로 σ가 매 사이클 복구되어 두 읽기가
일치한다 — 그래서 여태 드러나지 않았다.

**수정.** 명시적 상태로 모델링했다(`METAL_SIGMA_STATUS`):
`battle_start`가 세우고, `full_burst_enter`가 `has_status` 게이트로 쓰고 지우며,
`full_burst_end`가 `own_burst_fired_this_cycle()`일 때 다시 세운다
(`burst_used_this_cycle`은 그 트리거가 돈 **뒤에** 비워지므로 그 시점에 읽힌다).
회귀 테스트: `test_metal_sigma_is_spent_at_full_burst_and_restored_only_if_she_cast`
(4사이클 시퀀스를 그대로 재현) ·
`test_metal_sigma_fires_every_cycle_when_she_is_the_only_burst_3`(단일 B3 불변).
...
- **수치의 실전 크기.** F1·F2는 명중률을 통해 데미지에 닿는데 스윕 셸의 보스는
  코어가 없어 **둘 다 스윕에서는 0으로 보인다**. 실제 크기는 실기록
  캘리브레이션에서 읽어야 한다. (F3은 명중률과 무관해서 스윕이 잡았다 — −6.76%.)

## 검증

- 백엔드 **2439 passed / 3 skipped 실패 0** (트렁크 `ca4d1c4a` 위 리베이스 후,
  신규 회귀 테스트 5건 포함)
- 기계 audit 6종 재실행 전부 exit 0
- F3의 스윕 측정: 마나 780.4M → 727.7M (**−6.76%**), tier-3 셸(B3 둘)
