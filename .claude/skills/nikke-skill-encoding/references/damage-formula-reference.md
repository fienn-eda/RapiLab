...
- *Possible unintended error:* weapon-transforming Nikkes don't use their equipped collection item's unique stat modifier (negatively affects Zwei, Nayuta, K, Snow White, E.H.). **Refuted — Fienn measured in-game (2026-08-03, deck Anis/Brid/Soline/Helm: Aquamarine/Snow White: Heavy Arms, 작열-element firing range, NEAR position) that a weapon-transforming Nikke's collectible bonus DOES apply during the transform**: Snow White: Heavy Arms's Seven Dwarves: I transformed burst shot carries her AR collectible's Core Damage +17.04% into the major-modifier bucket, matching prediction to a residual of 4.9e-08 both outside and inside Full Burst; dropping the collectible term from the prediction misses by 6.8%. See `docs/measurements/collectible-during-weapon-transform.md`. One narrower case is NOT refuted by this measurement: RL/SR collectibles placed on the weapon's own base stat (`charge_damage_percent`, "weapon" placement) are read from a static per-segment profile a weapon-mode segment never re-derives from — that gap is real and still open, see `docs/engine-gaps.md`.
