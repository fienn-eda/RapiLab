...
섞여 나오면 실제 크기(`naturalWidth`/`naturalHeight`)로 가른다 — arca.live는 본문에
이모지(예: ✅)를 쓰면 에디터가 그걸 `<img class="twemoji" ...>`(150×150, 화면
표시는 17×17)로 바꿔 넣는다(union-2026-07-31 공지에서 관측). 진짜 본문 이미지는
1280×1707이었다. `class="twemoji"`나 폭 100px 미만인 img는 걸러낸다. 그렇게
걸러낸 뒤에도 2장 이상이면 컨테이너를 잘못 잡은 것이다.
