...
describe('hashRecommendInputs', () => {
  it('동일 입력은 동일 해시를 낸다', () => {
    ...
    expect(hashRecommendInputs(r, boss, null, 5)).toBe(hashRecommendInputs(r, boss, null, 5))
  })

  it('로스터 순서가 달라도 같은 해시(정규화)', () => {
    const a = [nikke('liter'), nikke('crown')]
    const b = [nikke('crown'), nikke('liter')]
    expect(hashRecommendInputs(a, boss, null, 5)).toBe(hashRecommendInputs(b, boss, null, 5))
  })

  it('boss 한 필드만 바뀌어도 해시가 다르다', () => {
    const r = [nikke('liter')]
    expect(hashRecommendInputs(r, boss, null, 5)).not.toBe(
      hashRecommendInputs(r, { ...boss, enemy_def: 999 }, null, 5),
    )
  })
  ...
  it('roster 투자 데이터(레벨 등) 한 필드만 바뀌어도 해시가 다르다', () => {
    ...
    expect(hashRecommendInputs(r, boss, null, 5)).not.toBe(hashRecommendInputs(r2, boss, null, 5))
  })
  ...
  it('draft=null(zero-base)과 빈 decks 배열을 구분한다', () => {
    ...
    expect(hashRecommendInputs(r, boss, null, 5)).not.toBe(
      hashRecommendInputs(r, boss, emptyDraft, 5),
    )
  })
  ...
  it('draft가 다르면 해시가 다르다', () => {
    ...
    expect(hashRecommendInputs(r, boss, draftA, 5)).not.toBe(
      hashRecommendInputs(r, boss, draftB, 5),
    )
  })
  ...
  it('덱 내 시트 순서가 달라도 같은 해시(정규화)', () => {
    ...
    expect(hashRecommendInputs(r, boss, draftA, 5)).toBe(hashRecommendInputs(r, boss, draftB, 5))
  })

  it('numDecks만 달라도 해시가 다르다', () => {
    const r = [nikke('liter'), nikke('crown')]
    expect(hashRecommendInputs(r, boss, null, 3)).not.toBe(hashRecommendInputs(r, boss, null, 5))
  })
})
