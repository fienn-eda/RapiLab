...
describe('RecommendPanel raid mode', () => {
  ...
  it('renders the allocated decks, combined total, and leftover slugs — not as a ranked list', async () => {
    ...
    vi.mocked(recommendRaidDecks).mockResolvedValue({
      decks: [
        { deck: ['a', 'b', 'c', 'd', 'e'], total_damage: 100, burst_damage: 60, normal_attack_damage: 40, skill_damage: 0, hold_burst_slugs: [], tap_fire_slugs: [], seating: {}, pinned_slugs: [] },
        { deck: ['f', 'g', 'h', 'i', 'j'], total_damage: 80, burst_damage: 50, normal_attack_damage: 30, skill_damage: 0, hold_burst_slugs: [], tap_fire_slugs: [], seating: {}, pinned_slugs: [] },
      ],
...
describe('RecommendPanel evaluate mode', () => {
  ...
  it('로스터가 최소 인원 밑으로 줄어도 평가는 제출된다 - 평가는 편성된 유닛만 채점하지 로스터 크기를 보지 않는다', async () => {
    ...
    vi.mocked(evaluateDecks).mockResolvedValue({
      decks: [
        { deck: ['a', 'b', 'c', 'd', 'e'], total_damage: 100, burst_damage: 60, normal_attack_damage: 40, skill_damage: 0, hold_burst_slugs: [], tap_fire_slugs: [], seating: {} },
      ],
...
describe('RecommendPanel mode switch', () => {
  ...
  it('싱글 결과의 파훼 불가 배지는 다른 모드에서 나중에 제출해도 흔들리지 않는다', async () => {
    ...
    vi.mocked(recommendRaidDecks).mockResolvedValue({
      decks: [
        { deck: ['a', 'b', 'c', 'd', 'e'], total_damage: 200, burst_damage: 120, normal_attack_damage: 80, skill_damage: 0, hold_burst_slugs: [], tap_fire_slugs: [], seating: {}, pinned_slugs: [] },
      ],
              ...
              decks: [
                {
                  ...
                  skill_damage: 77, hold_burst_slugs: [], tap_fire_slugs: [], seating: {},
                },
              ],
              excludedSlugs: [],
            },
...
describe('RecommendPanel — 편성 초기화와 가져오기', () => {
  ...
  it('초기화는 떠 있는 배분 제출도 멈춰, 응답이 늦게 와도 결과가 되살아나지 않는다', async () => {
    ...
    resolveRequest({
      decks: [
        { deck: ['a', 'b', 'c', 'd', 'e'], total_damage: 999, burst_damage: 0, normal_attack_damage: 0, skill_damage: 0, hold_burst_slugs: [], tap_fire_slugs: [], seating: {}, pinned_slugs: [] },
      ],
