...
const withinDraft: DraftAllocation = {
  decks: [
    { deck: ['a', 'b', 'c', 'd', 'e'], total_damage: 110, burst_damage: 70, normal_attack_damage: 40, skill_damage: 0, pinned_slugs: [] },
  ],
...
describe('DraftResults', () => {
  ...
  it('shows an empty per-deck diff when the result is a permutation of the submitted decks (content-matched, not index-matched)', () => {
    const permutedRecommended: RaidDeck[] = [
      { deck: ['f', 'g', 'h', 'i', 'j'], total_damage: 60, burst_damage: 40, normal_attack_damage: 20, skill_damage: 0, pinned_slugs: [] },
      { deck: ['a', 'b', 'c', 'd', 'e'], total_damage: 70, burst_damage: 45, normal_attack_damage: 25, skill_damage: 0, pinned_slugs: [] },
    ]
    render(
      <DraftResults
  ...
  it('shows exactly one add/remove pair when a permuted result has one real unit swap', () => {
    const permutedWithOneSwap: RaidDeck[] = [
      { deck: ['f', 'g', 'h', 'i', 'j'], total_damage: 60, burst_damage: 40, normal_attack_damage: 20, skill_damage: 0, pinned_slugs: [] },
      { deck: ['a', 'b', 'c', 'd', 'z'], total_damage: 70, burst_damage: 45, normal_attack_damage: 25, skill_damage: 0, pinned_slugs: [] },
    ]
    render(
      <DraftResults
  ...
  it('shows no diff when a drafted character comes back as the mode the engine chose', () => {
    ...
    const resolvedDecks: RaidDeck[] = [
      { deck: ['bready-lingering', 'b', 'c', 'd', 'e'], total_damage: 70, burst_damage: 45, normal_attack_damage: 25, skill_damage: 0, pinned_slugs: ['bready-lingering'] },
    ]
    const drafted: Draft = {
      decks: [
        [
