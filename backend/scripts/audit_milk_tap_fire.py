...
★★ 이 스크립트가 한 번 거짓을 찍은 전력이 있다(2026-08-20). 「풀차지」 집계가
`extra_charge_bonus > 0`이었는데, 톡톡이 배율이 0.03으로 오른 뒤 모든 톡톡이 샷이
그 조건을 만족해 풀차지로 오분류됐다 - `is_tap_fire` 플래그로 고쳤다.

★★★ 같은 날 두 번째로 거짓을 찍었다 - 이번엔 **창 길이**. `FIGHT = 30.0`짜리 창으로
「누적배율」(이득 비율)까지 같이 재고 있었는데, 30초에서는 매거진 재장전 위상이 안 맞아
이득이 5.56~9.62%로 진동한다(장탄/재장전 조합별) - 정상상태가 아니다. 그래서 이득
비율은 **창을 아예 안 쓴다.** 이 무기는 버프가 전투 내내 상수라 케이던스가 첫
매거진부터 이미 완전히 주기적이고(2·3번째 매거진의 시각차가 100·101번째, 500·501번째와
...
900·1200·2400초 여러 배수에서 우연히 같은 값이 나와 「수렴했다」는 착시를 준다),
120,000초까지 창을 늘리면 10.763%로 계속 미끄러지고 500,000초에서야 매거진-주기
방식과 소수점 4자리까지 맞아떨어진다(10.7627% 대 10.7628%). **600초도 정상상태가
아니었다** - 창을 어디서 끊든 마지막 미완성 매거진이 편향을 남기고, 두 케이던스의
주기가 서로 약분되지 않아 그 편향이 창 길이에 따라 들쭉날쭉 없어졌다 나타났다 한다.
"""
import sys
