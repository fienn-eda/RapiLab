...
# 갈린다:
#
#   UP           26정 — **놓을 때** 발사. 방아쇠를 놓아 쏘고 다시 눌러야 차지가
#                       시작되므로 발사모션↔차지모션 **멈춤이 있다**. 잰 11명 전원
#                       0이 아니다. 이 26정의 `rate_of_fire`는 **전원 60**이고 아무도
#                       안 읽는다 — 자리채움이다.
#   DOWN_Charge   5정 — **누르고 있는 동안** 차지·발사를 반복. 놓지 않으니 멈춤이
#                       없고(넷 실측 0), 대신 그 반복 루프의 속도인 `rate_of_fire`가
#                       바닥을 만든다. 이 다섯이 아래 표의 전부다.
#
# 즉 「멈춤이 있느냐」와 「연사가 60을 넘느냐」는 같은 사실의 두 얼굴이고, 그 사실은
# 발사가 누름인가 놓음인가다. `scripts/audit_rate_of_fire.py`가 이 함의를 양방향으로
# 검사한다.
#
...
#
# **바닥값이지 케이던스가 아니라는 것은 실기록이 정한다.** 이 다섯 중 넷이 실기록 덱에
# 있는데, `rate_of_fire`를 지속 케이던스로 읽으면 리베랄리오 1.072x → 약 2.2x ·
# 네온 1.023x → 약 1.95x · 아니스: 스타 1.055x → 약 1.40x가 된다. 케이던스는 차지가
# 정하고 이 값은 차지가 사라졌을 때만 보인다.
#
...
#
# 유닛별 표가 아니라 상수인 이유: 바닥 판독이 있는 것은 브래디 하나뿐이다(앨리스의
# RAW 최소값은 104%로, 103% 바닥과 양립할 뿐 그 증거는 아니다 - 손이 한두 프레임
# 더 붙든 경우로 문서화됨). 유닛별로 가를 근거가 없으므로 없는 정밀도를 주장하지 않는다.
#
