...
**모드 변형은 여기서 구분한다.** 접미사→한글 매핑 같은 파생 로직은 없다. 한 캐릭터가
여러 슬러그로 나뉘면 각 줄에 통째로 다르게 쓴다 (예: "브레디 (잔류)" / "브레디 (권장)").
같은 이름을 두 슬러그에 쓰면 test_display_names.py가 잡는다.

**애장품은 이름에 쓰지 않는다.** 유저가 실제로 장착한 유닛은 UI가 초상화 뱃지와 이름
옆 하트로 표시한다 — 이 표에는 맨 이름만 쓴다.
