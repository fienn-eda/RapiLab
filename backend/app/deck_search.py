...
def _summarize(ordered_deck, result):
    ...
    return {
        ...
        # 분모는 이 전투에서 **버스트가 터진** 사이클 전부다 - 스케줄러는 발동
        # 시각만 `fight_duration`과 견주므로 마지막 사이클의 풀 버스트 창은 전투
        # 종료를 넘겨 닫힐 수 있다 - 실측 5덱은 180초 전투에서 마지막 종료가
        # 180.24 ~ 189.28초로 **다섯 덱 모두** 전투 뒤에 닫힌다.
        # 그 사이클을 빼면 분모가 분자와 다른 모집단을 세게 된다: 분자는
        # `burst` 이벤트를 세는데 창의 닫힘은 안 본다. 분자는 그중 첫 사이클을 뺀
