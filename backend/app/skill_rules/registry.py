...
# 안 된다** - 그 덱에서 구간별 최적이 최선의 단일 모드보다 +27.6%다.
# 판정은 `attack_rate.optimal_full_charges`가 매거진 시작마다 답한다 - 매거진 안에
# 풀차지 몇 발을 섞을지까지 함께 정하므로, 두 끝만 비교하던 `tap_fire_wins`를 한 축
# 위로 올린 것이다(그쪽은 이제 창이 없는 유닛에서 두 판정이 동치임을 못박는 테스트의
# 기준으로 남아 있다).
#
# **다만 「창 안에서는 풀차지」를 플레이 지시로 읽으면 안 된다.** 차속이 세게 걸린
# 구간에서는 차지가 0프레임까지 내려가므로(앨리스: 버스트 80.15% + 스킬1의 캐스터 기준
# 0.17505초) 거기서는 톡톡이와 풀차지가 **같은 동작**이다 - 누르는 순간 이미 풀차지다.
# 엔진이 그 구간을 풀차지로 세는 것은 배율 이야기이지 기다리라는 뜻이 아니고, 실제로
