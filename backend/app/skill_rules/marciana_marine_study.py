...
- "Elemental Advantage Attack Damage" (Penguin Emergency Dispatch, continuous
  +20.41%; Penguin Spiral burst, +30.97% for 10 sec): Attack Damage that only
  applies with elemental advantage (Iron > Electric), so modeled as
  attack_damage_up gated on an Electric boss.
...
- High-Risk 20-normal nuke (Emergency Whistle): 152.68% of final ATK as
  ...
  Target state (a 20-sec window from her burst, only on an Electric boss).
  Modeled via per_shot mode "every_during_own_status_window" (gap #7), gated on
  an Electric boss; "as additional damage" -> Full Burst Bonus eligible.
...
HIGH_RISK_DURATION = 20.0  # High-Risk Target state (DEF debuff) lasts 20 sec
...
def build_marciana_rules(values):
    ...
    return [
        ...
        buff_rule(
            "battle_start",
            [("attack_damage_up", elem_adv_continuous, "self", None)],
        ...
        buff_rule(
            "own_burst_activate",
            [("attack_damage_up", burst_elem_adv, "self", BURST_BUFF_DURATION)],
            condition=boss_is_element(ELECTRIC),
        ),
        buff_rule(
            "own_burst_activate",
            [("enemy_def_percent", -def_debuff, "squad", HIGH_RISK_DURATION)],
            condition=boss_is_element(ELECTRIC),
        ),
...
def build_marciana_per_shot_rules(values):
    ...
    High-Risk Target state - a 20-sec window from her burst, only against an
    Electric boss (gap #7 window-gated per-shot trigger)."""
    high_risk_nuke = float(values["emergency_whistle"]["description_value_03"])
    return [
        (
            (HIGH_RISK_NUKE_SHOT_COUNT, HIGH_RISK_DURATION),
