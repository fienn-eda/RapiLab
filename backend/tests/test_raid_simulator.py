...
def test_battle_start_buff_is_active_by_the_time_the_burst_fires():
    ...
    assert entry == {"slug": "attacker", "time": 5.0, "damage": 150000.0, "source": "burst",
                     "damage_type": "attack", "gauge_hits": 1, "gauge_energy": None}
...
def test_instant_damage_pulse_deals_damage_at_battle_start():
    ...
    assert instant_hits == [{"slug": "buffer", "time": 0.0, "damage": 500.0,
                             ...
                             "gauge_hits": 1, "gauge_energy": None}]
...
def test_instant_damage_pulse_from_own_burst_activate_stacks_with_burst_nuke():
    ...
    assert burst_hits == [{"slug": "attacker", "time": 5.0, "damage": 5000.0,
                           "source": "burst", "damage_type": "attack",
                           "gauge_hits": 1, "gauge_energy": None}]
...
def test_a_deliberate_pass_cap_stops_early_and_says_so(monkeypatch):
    ...
    (실측: 실제 로스터 400덱에서 평균 5.96패스, 덱당 887ms). 54%의 덱은 두 패스에
    진짜로 수렴하므로 캡이 그들에게는 아무 일도 안 하고, 근사는 게이지가 무는 꼬리
    덱에만 갇힌다.
