...
def test_charge_shots_fire_after_charge_time_then_reload_and_repeat():
    # charge_time=1, reload_time=2 -> cycle=3s, first shot at t=1, then t=4, t=7
    shots = generate_charge_shot_times(charge_time=1.0, reload_time=2.0, fight_duration=8.0)
    assert shots == [1.0, 4.0, 7.0]


def test_charge_shots_stop_at_fight_duration():
    shots = generate_charge_shot_times(charge_time=1.0, reload_time=2.0, fight_duration=4.0)
    assert shots == [1.0]
...
def test_generate_shot_times_dispatches_to_charge_for_charge_weapons():
    shots = generate_shot_times(
        weapon="RL", max_ammo=6, reload_time=2.0, charge_time=1.0, fight_duration=8.0,
    )
    assert shots == [1.0, 4.0, 7.0]
