...
def test_the_pellet_bonus_reaches_only_shotgun_allies():
    """Pellet count moves no damage (measured on Arcana: Fortune Mate) - it is
    encoded because it is an INPUT to Dorothy: Serendipity's Flash counter."""
    reg = EffectRegistry()
    fire_trigger("full_burst_enter", {"leona": build_leona_rules(LEONA)}, _ctx(), reg, 12.0)
    assert reg.total_for("pellet_count_bonus", SHOTGUN_ALLY, 12.0) == 5.0
    assert reg.total_for("pellet_count_bonus", SELF, 12.0) == 5.0  # she carries one too
