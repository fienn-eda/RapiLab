...
닫힌형(격차 나누기 표시공격력)은 필요치를 **과소평가한다** - 그것도 2.5~3배
차이로(Task 7, 2026-08-08 실측). 경계가 「전 사이클에서 받는가」이고 사이클마다
이기는 쪽이 갈리므로, 가장 빡센 사이클이 값을 정하기 때문이다. 그래서 실제로
돌려서 이진탐색한다. 이 파일이 보는 것은 「보고된 값에서 실제로 받고, 한 눈금
아래에서는 못 받는가」다.
"""
